...
Production has one route: MediaPipe Pose Landmarker Lite float16 `/1/`, `@mediapipe/tasks-vision` `1.0.1`, CPU-WASM in the dedicated MediaPipe Worker, `0.5/0.5/0.5`, Fast tracking, direct full input with no resize, measured-current gameplay only, and a 15 FPS submission ceiling (`<=15.01` measurement tolerance). There is no backend/provider selector and no replay/provider fallback; permission, model, runtime/execution-identity, and tracking-freshness failures fail closed. Experimental prediction fixtures remain outside the production graph and release.
...
The landing and coder→QA evidence agree: the drawer has exactly Gameplay, Visuals, Music, and Info; scoped native radios deterministically select Flow and `aero.visual.default`; product scopes omit development/scoring/converter/bundle text; missing playable Music focuses one concise prerequisite and makes zero camera requests; playable content is selected/configured before camera, T-pose, countdown, and play. The targeted mobile validator passed once. Lifecycle/reconnect, direct/iframe privacy, profile APIs, and no-production-Boxing-winner coverage remain represented by the unchanged accepted suites. **Historical frozen-release evidence (superseded; not current production):** the exact 2026-08-29 post-repair release proof recorded MediaPipe Lite float16 `/1/`, tasks-vision `1.0.1`, GPU-WebGL, measured input, and 15 FPS, with proof SHA-256 `88807550b23fae13dbd26f420c68ca20b679b3d8ca45225bd2e0e392b3fca3fd` and source fingerprint `e15922ff6eb9e09f244d7e9a6443424206f992ee2c14b2c05b83695057e65817`; this paragraph preserves that old proof without describing the current locked CPU-WASM worker route.
...
The targeted mobile validator was run exactly once for this audit and passed its 390×844 and 844×390 four-section, populated current/first radio, action/select typing, zero-camera gate, playable camera/T-pose/countdown/play, menu-pause, reconnect, and console-noise assertions. **Historical frozen-release evidence (superseded; not current production):** the exact 2026-08-29 product-radio audit release proof recorded MediaPipe Pose Landmarker Lite float16 `/1/`, tasks-vision `1.0.1`, GPU-WebGL, measured gameplay input, and 15 FPS; its source fingerprint is `83a0c64515fd9e2e80922f0f47875889ff383441c0078c78523dbe4c5582ea9d`, proof SHA-256 is `895b14ccfb0213da7f36d9234f05d8313a1cccfceccef6ed6430a8eccf9568d6`, and pre-manifest size is `3966471` bytes. This preserves that old proof only; the current product route is the locked CPU-WASM worker route. Local direct and documented Android HTTPS routes both returned HTTP 200.
