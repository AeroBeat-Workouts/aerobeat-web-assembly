...
## Corrected Diagnosis: Locked Eight-Way Flow and Frozen Release
...
The authoritative `aerobeat-docs/docs/architecture/beatsaver-flow-v1-conversion.md` locks arrow matching in 8-way shoulder-relative motion space at lines 165–185 and locks source head cut direction preservation at lines 285–291. Therefore authoring pass-through `0..7` plus direction-free dot `8` is correct and must not be normalized. The exact live assembly path found the stale downstream boundary: current `53F26` contains valid diagonal Beat Saber cuts `4..7`, but gameplay accepted only cardinal `0..3` and input still derived a cardinal direction from one predecessor instead of rolling shoulder-relative eight-way motion. A first authoring-side normalization at `13cc663` made the live map select but violated Flow preservation ownership. It was forward-reverted at `fa824fc`; authoring's full check/unit/security/real/browser gates pass with exact direction preservation restored. Gameplay P0 `aerobeat-web-gameplay-uau` at `2326ccb` instead extends only Flow validation/evidence matching to all eight Beat Saber directions while keeping dot `8` direction-free and Boxing cardinal semantics unchanged. The exact live package then selects five variants without camera. Assembly `02a1671` also replaces its legacy four-way/rotated render mapping with exact Beat Saber `0..7` → eight-way renderer directions.

**Plan correction:** the original plan statements “Four cardinal directions only” and “expected cardinal directions” predated the locked Flow v1 architecture and are explicitly superseded. They now state eight-way shoulder-relative evidence and exact BeatSaver `0..7` preservation/dot `8` semantics. Contracts P0 `aerobeat-web-contracts-vd2` and QA `aerobeat-web-contracts-fy7` are closed; implementation `5470c90` and final QA closure `c1e14f9` verify all eight exact strings, v1 compatibility, validator rejection, downstream compatibility, browser, pack, and diff gates.

Assembly's normal gate now locks actual-service hard-coded v4 acquisition/UI authoring/persistence/selection, strict tamper rejection, v2/v3 local ZIP regression, bounded public data, and no winner. The optional network gate locks exact live `53F26` / `addd9d6f8e7340ad6f5633947136d8475a7a99b5`. Contracts `1673f13`/`0c8fba8`, input `f23b372`/QA closure `829c85f`, and renderer `3cd09ce`/`a59d930`/QA closure `bc59641` complete the locked directionless-entry and eight-way evidence/cue boundary; full direct/iframe/live gates pass on the clean synchronized graph. After all owner QA closures (contracts `0c8fba8`, input `829c85f`, gameplay `e0f194f`, renderer `bc59641`, vendor `b9d19a6`), the full normal/live/direct/iframe matrix and two fresh recursive release rounds pass again. Their manifests match exactly; fingerprint/proof/build/cache identity is `6c913fcd1b17aa115e2112c0dd76ff0b327fcf0089fd45267d17ccdeec28fa97`, proof SHA-256 `dfdae0d909ee47b48f9c886e8210a0b133d5711a552b083bb5b4867bca165e31`, and pre-manifest bytes `4004042`. Two final dry-run pack JSON documents match byte-for-byte at SHA-256 `15a4a0e3adfbf8b3ef5051ac5f0a44b596abab5cc2e2ba445a8871c9f74af274` (60 files, packed `189146`, unpacked `679878`).
...
Root cause: Reused dependency prebundle plus stale input/gameplay/render boundaries lagging the locked eight-way authored Flow contract.
Evidence: Canonical Flow v1 lines 165–185 require rolling shoulder-relative eight-way arrow matching and lines 285–291 preserve head cut direction; live stack rejected a valid diagonal; input used cardinal single-transition evidence; authoring correctly preserved 0..7/8.
Failed approaches: Fixing browse result shape and increasing async waits did not address dependency cache; authoring-side cardinalization made selection pass but violated locked architecture and was reverted at `fa824fc`.
