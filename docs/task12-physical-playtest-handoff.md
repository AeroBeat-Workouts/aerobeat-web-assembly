...
4. Confirm the bounded source/status truth identifies MediaPipe Pose Landmarker Lite float16 `/1/`, tasks-vision `1.0.1`, CPU-WASM in the dedicated MediaPipe Worker, thresholds `0.5/0.5/0.5`, Fast tracking, direct full input with no resize, measured-current gameplay only, and a 15 FPS submission ceiling (`<=15.01` measurement tolerance). There is no selectable provider or replay/provider fallback; permission, model, runtime/execution-identity, and tracking-freshness failures must fail closed.
...
On final docs HEAD `99be070`, the exact clean linked heads are contracts `0c8fba8`, input `829c85f`, gameplay `e0f194f`, renderer `bc59641`, vendor `b9d19a6`, and authoring `a34de05`. Assembly test, direct/real-iframe browser matrix, exact live `53F26`, build, two recursive release rounds, and two packs pass. Both recursive manifests are byte-identical; proof/build/cache identity remains exact. Renderer browser evidence proves all eight cues have distinct pixel distributions and required luminance contrast.
...
### Close/camera implementation ready for independent QA and phone retest

`p4p` now renders a font-independent close X from two explicit white 24×4 bars in the sole 48px button. The new native Environment group under Visuals has two 42px choices: Aero (default) and Camera. It is independent of Default/Compact, remains child-local, and resets to Aero on reconnect.

Calibrate/Start explicitly plays the attached retained mirrored video. Initial T-pose and tracking-loss recalibration force the full-opacity preview while the WebGL background is transparent, leaving grid/guides above the athlete. Fresh readiness switches directly to opaque Aero for countdown/play by default. Camera selection keeps the same retained playing feed under WebGL during calibrated play; switching back to Aero hides it immediately without a new camera request. Pause/resume, document visibility, lease transfer, destroy, and reconnect preserve explicit pause/play/retention semantics.

A moving child-local color stream proves frame updates and mirror direction through the real compositor. The fresh and actual served direct/iframe phone matrix at DPR 1/3 proves 150/1,500 high-luminance X-mask pixels, all quadrants, center fill, >=3 CSS px stroke, one control, and explicit white-on-dark paint. Automated state matrices prove default hide at countdown/play, forced recovery preview, Camera/Aero switching, one cue, and no frame/pixel/screenshot serialization. Frozen source fingerprint is `01b535c6eab264203e88c343e6f46e1093c628559924838da07553864ba4de0e`, proof SHA-256 is `9d1801b4c5fe2bc805b58b10ffc6f7e7e8b7f0330dc4cffccc3755cb2eaccb8c`, and pre-manifest bytes are `3286234`.

### Final independent close/camera automated QA passed

At exact `37a35a6`, the unchanged existing Tailscale route and fresh source were independently sampled in direct and real cross-origin iframe contexts at 390×844/844×390 and DPR 1/3. Flood-fill analysis proves exactly one connected center-filled X component from two explicit 24×4 white bars, with no font glyph or second control. The actual served route also passes moving asymmetric mirrored-frame composition during initial calibration; the fresh state matrix proves T-pose/hold and tracking recovery, Aero's immediate ready/countdown/play hide with opaque restoration, Camera's retained playing stream, zero toggle reacquisition, explicit pause/resume/document/lease/reconnect/destroy, one cue/control, no menu leak, bounded private iframe messages, recursive raw-media/frame/pixel/screenshot privacy, and no winner.
