...
For the current mobile integration checkpoint, confirm the phone page loads over the Tailscale HTTPS URL and shows `Secure context: ready`. MoveNet is the no-query default. Use the visible Pose backend/provider controls or the stable query combinations documented in the README to select MoveNet WebGL, MediaPipe CPU-WASM/GPU-WebGL, or ONNX Runtime WASM/WebGPU. Run `npm run model:prepare:onnx` before starting Vite when real ONNX inference is required. The deterministic MoveNet replay source remains the explicit fallback for blocked camera access or runtime/model failure.
