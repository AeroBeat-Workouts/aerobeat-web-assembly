...
PASS at exact assembly `37a35a6`. The unchanged actual Tailscale route and fresh direct/real-cross-origin iframe contexts all pass 390×844/844×390 DPR1/3 pixel-mask checks. An independent flood fill proves the high-luminance interior is exactly one connected X component spanning all quadrants and the center; DOM/style evidence proves it comes from two explicit white 24×4 bars with no font glyph or second control. The actual served route passes moving asymmetric mirrored-frame composition, and fresh matrices cover calibration/T-pose, paused-tracking recovery, Aero/Camera transitions, retained-stream playback without reacquisition, pause/resume/document/lease/reconnect/destroy, one cue/control, no menu leak, private iframe messages, recursive raw-media/frame/pixel/screenshot privacy, and no winner. Full unit/browser/live/build and stable release/pack gates pass. Automated `h8z`, `p4p`, and `9y6` are closed; physical `kdf` and `3h7` remain open for Derrick's Android camera/calibration/play retest.
