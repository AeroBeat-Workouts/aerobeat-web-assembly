...
The current deterministic raw release proof is `0.0.28`. Task 0 and Task 2 received independent QA and audit PASS before release ownership began. The release uses PlayCanvas plus DOM as the sole gameplay presentation path for Flow, Boxing Lanes, and Boxing Grid; preserves both Balanced Height and Source Height conversions; and adds the timestamp-driven 3D timing floor, duration-aware Flow obstacle volumes, and Visual Test-only free-fly camera while retaining Play/Pause/live scrubbing. Owner release gates and repeated raw/pack construction establish semantic byte-for-byte determinism; exact final npm-pack metadata is kept only in the external release Bead because this README is itself packed. Automated evidence does not replace the explicitly Pending physical-phone checks in `docs/task16-flow-perspective-obstacles-test-transport-physical-handoff.md`, and no gameplay or conversion winner is selected.
