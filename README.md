...
The live phone route now reports WebGL2 preview fidelity and CV latency diagnostics in the Inference panel: selected CV performance preset, worker/main-thread execution status, inference input size, granted camera size, measured video FPS where available, frame prep/downscale milliseconds, adapter inference milliseconds, total CV milliseconds, cheap running averages, submitted/output ages, dropped frames, rendered pose frames, the seven-landmark calibration subset count, selected tracking profile, and the comparable media-time minus pose-frame timestamp delta. The visible overlay is limited to `nose`, `left_wrist`, `left_elbow`, `left_shoulder`, `right_shoulder`, `right_elbow`, and `right_wrist`, with the nose connected to both shoulders. The first-viewport phone controls include a camera selector populated from available browser `videoinput` devices, a tracking selector with `Smoother` and `Fast` profiles, and a CV performance selector that defaults to `Direct full (recommended)` with explicit `Worker downscale 256`, `Worker downscale 192`, and `Worker downscale 160` comparison presets.

The default phone build uses Fast tracking and the direct full-input CV path because Derrick's first Android Chrome telemetry measured that path faster than worker/downscale preprocessing. Downscale presets reduce camera constraints where the browser honors them and shrink the MoveNet inference input before it reaches the adapter; worker-backed MoveNet receives the smaller `ImageBitmap` when module workers and transferable frame support are available. These presets are comparison tools, not low-quality promises of lower cost, and the app visibly reports worker/main-thread fallback details so the next phone pass can compare execution paths truthfully.
