...
The live phone route now reports WebGL2 preview fidelity in the Inference panel: selected CV performance preset, worker/main-thread execution status, inference input size, rendered pose frames, the seven-landmark calibration subset count, selected tracking profile, and the comparable media-time minus pose-frame timestamp delta. The visible overlay is limited to `nose`, `left_wrist`, `left_elbow`, `left_shoulder`, `right_shoulder`, `right_elbow`, and `right_wrist`, with the nose connected to both shoulders. The first-viewport phone controls include a camera selector populated from available browser `videoinput` devices, a tracking selector with `Smoother` and `Fast` profiles, and a CV performance selector with `Full quality`, `Balanced phone`, `Fast phone`, and `Low-end rescue` presets so phone testing can compare quality against latency.
