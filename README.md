...
### Owned alien-moon runtime payload

The versioned `assets/environments/alien-moon-icescape/1.0.0/` payload contains the exact owned local self-contained GLB, its identity-transform YAML, and a strict provenance/structure manifest. The GLB is `6,149,400` bytes with SHA-256 `40e38a7bdce9eab4266d8bb19510a95bb4e0410534f3f14a500f36fac2b65077`; the YAML is `100` bytes with SHA-256 `1e50a9416dc2e506284919947088df812da54e537df69be5ec002c4cc167e788`. It embeds its PNG, has no external URI, uses `KHR_materials_unlit`, preserves nominal ±30 bounds and an identity transform, and was copied locally without third-party acquisition for this assembly. `npm run sync:environment-assets` copies only the two pinned source files after verifying source identity; `npm run verify:environment-assets` fails closed on source/payload/hash/byte/config/GLB/manifest/inventory drift. The frozen descriptor remains internal and package-relative; no environment asset identity, URL, hash, byte count, or payload enters public host contracts or snapshots. Renderer environment wiring is deliberately deferred.
