...
Backend/provider/tuning changes synchronize the URL with `history.replaceState`, terminally dispose the old CV service exactly once, retain latest-selection-wins behavior during rapid switches, and restart an active camera route. MediaPipe `standard` is the unchanged 0.5 detection / 0.5 presence / 0.5 tracking default; `responsive` is the measured experiment at 0.5 / 0.4 / 0.3. The tuning selector is disabled and labeled not applicable on other backends while its URL selection is retained for switching back. Invalid backend/provider/tuning values remain visible as requested values in telemetry while selection falls back to MoveNet, that backend's supported provider, or standard tuning. MoveNet-only worker performance presets are removed from the performance control while MediaPipe or ONNX Runtime is selected; direct presets remain orthogonal.
