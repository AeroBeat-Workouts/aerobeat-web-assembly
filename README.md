...
The verifier accepts only npm's canonical block-framed tar shape: every member has an unambiguous safe descendant path under `package/`, member padding is zero, and the stream ends with exactly the two consecutive 512-byte zero blocks emitted by the supported npm toolchain with no member, byte, or extra block after them. It applies path safety before directory/PAX/type handling, keeps directory mode exactly `0755`, requires every npm file member to match the detached target bytes and its Git-index-derived mode, rejects PAX and unsupported member types, and reports archive, decompressed-tar, and canonical path/mode/size/content-SHA256 manifest identities. The CLI accepts only each documented command's exact required/optional option schema. This release tooling does not rebuild or version raw output.
