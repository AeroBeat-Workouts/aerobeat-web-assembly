...
The live phone route reports WebGL2 preview fidelity and CV latency diagnostics in the Inference panel: selected preset, actual execution location and detail, actual resize path, inference input dimensions, granted camera size, measured video FPS where available, prep cost, adapter cost, total CV cost, running averages, submitted/output ages, dropped frames, rendered pose frames, the seven-landmark calibration subset count, selected tracking profile, and media-time minus pose-frame timestamp delta. The visible overlay remains limited to `nose`, `left_wrist`, `left_elbow`, `left_shoulder`, `right_shoulder`, `right_elbow`, and `right_wrist`, with the nose connected to both shoulders. Camera switching and the default `Fast` tracking profile remain available.
...
When Android screenshots fail while the live camera is active, use the runtime Telemetry controls beside the status panels. `Copy telemetry` captures the current version/build/cache, route URL, secure-context state, selected camera, tracking profile, CV preset, execution location/detail, resize path, input dimensions, prep/adapter/total CV costs, output age, media-pose delta, Camera/Media/Inference/Calibration panel strings, and service summary into the clipboard when browser policy allows it. The same snapshot is always rendered in a selectable text block for mobile share/copy fallback, and `Download telemetry` saves a text file from the current snapshot.
