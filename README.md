...
npm pack --json --pack-destination /absolute/output > /absolute/output/package.pack.json # run inside that detached worktree
node /path/to/main/scripts/release-pack-policy.js verify \
  --target /absolute/detached/worktree --commit <exact-commit> \
  --archive /absolute/output/package.tgz --metadata /absolute/output/package.pack.json \
  --manifest /absolute/output/package.manifest.tsv
npm run test:release-pack-policy
```

The verifier requires the matching trusted `npm pack --json` record and accepts only the pinned Node 22/npm 10.9.8 archive grammar. It binds package identity, exact metadata schema/inventory, archive byte length, SHA-1, and SHA-512 integrity; requires one canonical gzip member with its fixed header, complete deflate boundary, CRC32/ISIZE trailer, and exact EOF; then byte-compares every complete npm USTAR regular-file header against the detached target, Git/bin-derived mode, canonical ASCII path split, size, portable zero ownership/name/link fields, pinned mtime, checksum, and reserved bytes. Content and padding must match exactly, membership is neither empty nor partial/reordered/extended under the trusted archive hash, PAX/directories/links/base-256/noncanonical fields are rejected, and the tar ends with exactly two zero blocks. The CLI accepts only each documented command's exact option schema. This release tooling does not rebuild or version raw output.
