...
The current deterministic raw release proof is `0.0.30`. This control iteration preserves PlayCanvas plus DOM as the sole gameplay presentation path for Flow, Boxing Lanes, and Boxing Grid, retains both Balanced Height and Source Height conversions, and keeps the corrected scene-facing perspective camera. Desktop Visual Test now uses right-click toggle capture, smooth captured mouse look, and gradual held camera-relative `WASD`/`Q`/`E` movement; touch Visual Test uses a bounded two-finger capture toggle, captured one-finger look, and an accessible Forward/Back/Left/Right/Up/Down, Normal/Boost, Reset panel without help prose. Scored Play remains fixed-camera. Strict direct/iframe portrait/landscape DPR1/3 evidence proves the complete GUI/capture/movement lifecycle and rejects invalid canvas buffers and the old yaw-zero failure. The timestamp-driven 3D timing floor, duration-aware Flow obstacle volumes, Play/Pause/live scrubbing, exact 4×3 scoring/input, lifecycle, and privacy boundaries remain unchanged. Owner release gates and repeated raw/pack construction establish semantic byte-for-byte determinism; exact final npm-pack metadata is kept only in the external release Bead because this README is itself packed. Derrick's `0.0.29` camera-direction PASS and superseded control failures remain recorded, every `0.0.30` physical retest remains explicitly Pending, and no gameplay or conversion winner is selected.
