...
The current deterministic raw release proof is `0.0.29`. The hotfix preserves PlayCanvas plus DOM as the sole gameplay presentation path for Flow, Boxing Lanes, and Boxing Grid, retains both Balanced Height and Source Height conversions, and corrects the desktop Visual Test free-fly camera's stored forward orientation so active gameplay remains visible before input and across Reset and pause/resume. Strict direct/iframe portrait/landscape DPR1/3 canvas evidence rejects invalid buffers and proves the old yaw-zero failure against the corrected scene-facing pose. The timestamp-driven 3D timing floor, duration-aware Flow obstacle volumes, Play/Pause/live scrubbing, exact 4×3 scoring/input, lifecycle, and privacy boundaries remain unchanged. Owner release gates and repeated raw/pack construction establish semantic byte-for-byte determinism; exact final npm-pack metadata is kept only in the external release Bead because this README is itself packed. Derrick's `0.0.28` desktop Flow blue-screen observation remains recorded as Failed/superseded, every `0.0.29` physical retest remains explicitly Pending, and no gameplay or conversion winner is selected.
