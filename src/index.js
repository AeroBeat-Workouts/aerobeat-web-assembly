...
import { canonicalWorldUnitsPerMs, gameplayFlowColliderSettings, rendererGameplayVisualConfig, sanitizedNoseCameraDeflection, selectedNormalSpawnDistanceWorldUnits } from "./gameplay-visual-runtime.js";
...
export class AeroGame extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.latestPoseTimestampMs = -1; this.lastFreshPoseAtMs = -Infinity; this.lastInputAdvanceAtMs = -Infinity; this.lastContentSyncAtMs = -Infinity; this.runtimeUiSignature = ""; this.contentPresenterSignature = ""; this.desiredGameSetup=getGameSetupSnapshot(); this.activeSessionSetup=null; this.gameSetupDrafts.clear();
  ...
  rendererFrame() {
    ...
    const selected = content.selectedVariant; const nowMs = Number(session.timelinePositionMs ?? 0),setup=this.activeSessionSetup??this.desiredGameSetup;
    const presentation = rendererPresentationForVariant(selected),timingWindowMs=selected?.rulesetId===gameplayRulesetIds.flowColliders?setup.timingWindowMs:prototypeJudgementDefaults.timingWindowAfterMs;
  ...
  installGameSetupControls() {
    ...
    const rows=[["spawnDistanceOverrideWorldUnits","Spawn distance (world units)",...gameSetupBounds.normalSpawnDistanceWorldUnits,.1],["noseCameraRangeXWorldUnits","Camera horizontal range",...gameSetupBounds.noseCameraRangeXWorldUnits,.01],["noseCameraRangeYWorldUnits","Camera vertical range",...gameSetupBounds.noseCameraRangeYWorldUnits,.01],["timingWindowMs","Collider timing window (ms)",...gameSetupBounds.timingWindowMs,1],["colliderRadius","Collider radius",...gameSetupBounds.colliderRadius,.01],["directionToleranceDegrees","Direction tolerance (degrees)",...gameSetupBounds.directionToleranceDegrees,1]];
    ...
    const note=document.createElement("p");note.className="game-setup-note";note.textContent="Gameplay timing and collider changes apply on next Start/Test.";section.append(note);content.prepend(section);
  }
  applyGameSetupControl(input){const current=getGameSetupSnapshot(),field=input.dataset.gameSetupField;if(!field)return;let next=current;if(input instanceof HTMLInputElement&&input.type==="number"){if(!input.validity.valid||!Number.isFinite(input.valueAsNumber)){this.renderGameSetupError(field,"Enter a value within the allowed range and step.");return;}const value=input.valueAsNumber;if(field==="spawnDistanceOverrideWorldUnits")next={...current,spawnDistanceOverride:{...current.spawnDistanceOverride,normalSpawnDistanceWorldUnits:value}};else if(["noseCameraRangeXWorldUnits","noseCameraRangeYWorldUnits","timingWindowMs","colliderRadius","directionToleranceDegrees"].includes(field))next={...current,[field]:value};else return;}else if(input instanceof HTMLSelectElement&&field==="guidanceBandMode")next={...current,guidanceBandMode:input.value};else if(input instanceof HTMLInputElement&&["showGameplayGrid","noseCameraParallaxEnabled","enforceAuthoredDirection"].includes(field))next={...current,[field]:input.checked};else if(input instanceof HTMLInputElement&&field==="spawnDistanceOverrideEnabled")next={...current,spawnDistanceOverride:{...current.spawnDistanceOverride,enabled:input.checked}};else return;try{setGameSetupSnapshot(next);this.gameSetupDrafts.delete(field);this.renderGameSetupError(field,"");}catch{this.gameSetupDrafts.delete(field);this.renderGameSetupControls();this.renderGameSetupError(field,"Enter a value within the allowed range and step.");}}
