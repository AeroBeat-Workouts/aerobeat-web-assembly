...
import { beatBounceConfigArtifactFilename, beatBounceConfigArtifactMimeType, defaultBeatBounceConfig, maximumBeatBounceConfigBytes, normalizeBeatBounceConfig, serializeBeatBounceConfig } from "@aerobeat/web-renderer";
...
export class AeroGame extends HTMLElement {
  constructor() {
    ...
    this.beatBounceConfig = defaultBeatBounceConfig;
    this.beatBouncePickerRequest = null;
    this.beatBouncePickerGeneration = 0;
    this.beatBouncePickerToken = null;
    this.beatBounceAuthoringEnabled = false;
    this.beatBounceStatus = "";
    this.boundBeatBounceFile = (event) => { void this.handleBeatBounceFile(event); };
    this.boundVisibility = () => { if (document.hidden) { this.releaseDebugCameraControls(); this.invalidateBeatBouncePicker(); this.beatBounceAuthoringEnabled=false; } void this.applyVisibility(); };
    ...
    this.beatBouncePicker = document.createElement("input"); this.beatBouncePicker.type = "file"; this.beatBouncePicker.accept = ".json,application/json"; this.beatBouncePicker.hidden = true; this.beatBouncePicker.dataset.role = "beat-bounce-config-picker"; root.prepend(this.beatBouncePicker);
  }
  ...
  connectedCallback() {
    ...
    try {
      ...
      if (typeof graph.renderer.setBeatBounceConfig === "function") graph.renderer.setBeatBounceConfig(this.beatBounceConfig);
  ...
  bindHostLifecycle() {
    ...
    this.beatBounceInput().addEventListener("change", this.boundBeatBounceFile);
  ...
  rendererFrame() {
    ...
    if (events !== this.renderEventSource || this.renderBounceConfig !== this.beatBounceConfig) {
      this.renderEventSource = events; this.renderBounceConfig = this.beatBounceConfig;
      ...
      this.renderEventIndex = createSessionTargetIndex(events, { mapBeatToTimelineMs:mapBeatToTimelineMs ?? undefined, leadBeats:this.beatBounceConfig.leadBeats });
      if (this.renderEventIndex.timingMismatchCount > 0) this.setBeatBounceStatus("Bounce timing mismatch; straight approach retained.", true);
      else if (this.renderEventIndex.leadLimited) this.setBeatBounceStatus("Lead limited to 10 s for this tempo.");
    }
  ...
  syncDebugCameraPresentation() {
    ...
    this.renderEnvironmentControls(); const bounceEnabled=this.beatBounceAuthoringSnapshot().enabled;if(this.beatBounceAuthoringEnabled&&!bounceEnabled)this.invalidateBeatBouncePicker();this.beatBounceAuthoringEnabled=bounceEnabled;this.renderBeatBounceControls();this.syncDebugCameraControlState(snapshot);
  }
  ...
  applyBeatBounceControls() {
    if(!this.graph||!this.beatBounceAuthoringSnapshot().enabled)return;
    const value=(name)=>{const input=this.shadowRoot?.querySelector(`[data-beat-bounce-field='${name}']`);return input instanceof HTMLInputElement?Number(input.value):input instanceof HTMLSelectElement?input.value:undefined;};
    try{const config=normalizeBeatBounceConfig({schema:"aerobeat/beat_bounce_config",version:1,leadBeats:value("leadBeats"),heightWorldUnits:value("heightWorldUnits"),apexFraction:Number(value("apexPercent"))/100,riseEasing:value("riseEasing"),fallEasing:value("fallEasing")});this.commitBeatBounceConfig(config,"Bounce updated.");}catch{/* invalid intermediate text leaves prior config unchanged */}
  }

  commitBeatBounceConfig(config,status){const normalized=normalizeBeatBounceConfig(config);if(!this.graph||!this.beatBounceAuthoringSnapshot().enabled)return false;this.graph.renderer.setBeatBounceConfig(normalized);this.beatBounceConfig=normalized;this.renderBounceConfig=null;this.beatBounceStatus=status;this.renderBeatBounceControls();return true;}
  resetBeatBounce(){if(this.commitBeatBounceConfig(defaultBeatBounceConfig,"Bounce reset."))this.renderGameplay();}
  exportBeatBounce(event){if(!event?.isTrusted||!this.beatBounceAuthoringSnapshot().enabled)return;const text=serializeBeatBounceConfig(this.beatBounceConfig),blob=new Blob([text],{type:beatBounceConfigArtifactMimeType}),url=URL.createObjectURL(blob),anchor=document.createElement("a");anchor.hidden=true;anchor.download=beatBounceConfigArtifactFilename;anchor.href=url;this.shadowRoot?.append(anchor);try{anchor.click();this.beatBounceStatus="Bounce config saved.";}finally{anchor.remove();URL.revokeObjectURL(url);this.renderBeatBounceStatus();}}
  invalidateBeatBouncePicker(){this.beatBouncePickerGeneration+=1;this.beatBouncePickerToken=null;this.beatBouncePickerRequest=null;this.beatBounceInput().value="";}
  openBeatBouncePicker(event){if(!event?.isTrusted||!this.graph||!this.beatBounceAuthoringSnapshot().enabled)return;this.invalidateBeatBouncePicker();const token=Object.freeze({}),request=Object.freeze({generation:this.beatBouncePickerGeneration,token,connectionGeneration:this.connectedGeneration,sessionGeneration:this.sessionGeneration,graph:this.graph,purpose:"visual_test",lifecycle:"connected"});this.beatBouncePickerToken=token;this.beatBouncePickerRequest=request;this.beatBounceStatus="";this.renderBeatBounceStatus();this.beatBounceInput().click();}
  async handleBeatBounceFile(event){const input=event.currentTarget;if(!(input instanceof HTMLInputElement))return;const request=this.beatBouncePickerRequest,file=input.files?.[0]??null;input.value="";if(!event.isTrusted){this.invalidateBeatBouncePicker();return;}if(!file||!request||!this.isBeatBounceRequestCurrent(request)){this.invalidateBeatBouncePicker();return;}try{if(file.size>maximumBeatBounceConfigBytes)throw new Error("invalid");const bytes=await file.arrayBuffer();if(!this.isBeatBounceRequestCurrent(request)||bytes.byteLength>maximumBeatBounceConfigBytes)throw new Error("invalid");const text=new TextDecoder("utf-8",{fatal:true}).decode(bytes);if(!this.isBeatBounceRequestCurrent(request))return;const value=JSON.parse(text);if(!this.isBeatBounceRequestCurrent(request))return;const config=normalizeBeatBounceConfig(value);if(!this.isBeatBounceRequestCurrent(request))return;this.commitBeatBounceConfig(config,"Bounce config loaded.");}catch{if(this.isBeatBounceRequestCurrent(request))this.setBeatBounceStatus("Bounce config is invalid.",true);}finally{if(this.beatBouncePickerRequest===request)this.invalidateBeatBouncePicker();}}
  isBeatBounceRequestCurrent(request){const state=this.beatBounceAuthoringSnapshot();return request!==null&&request===this.beatBouncePickerRequest&&request.generation===this.beatBouncePickerGeneration&&request.token===this.beatBouncePickerToken&&request.connectionGeneration===this.connectedGeneration&&request.sessionGeneration===this.sessionGeneration&&request.graph===this.graph&&request.purpose==="visual_test"&&request.lifecycle===this.lifecycle&&state.enabled;}
  setBeatBounceStatus(message,error=false){this.beatBounceStatus=String(message).slice(0,96);this.renderBeatBounceStatus(error);}
  renderBeatBounceStatus(error=false){const output=this.shadowRoot?.querySelector("[data-role='beat-bounce-status']");if(output instanceof HTMLOutputElement){output.value=this.beatBounceStatus;output.dataset.error=error?"true":"false";}}
  renderBeatBounceControls(){const config=this.beatBounceConfig,values={leadBeats:config.leadBeats,heightWorldUnits:config.heightWorldUnits,apexPercent:config.apexFraction*100,riseEasing:config.riseEasing,fallEasing:config.fallEasing};for(const [name,value]of Object.entries(values)){const input=this.shadowRoot?.querySelector(`[data-beat-bounce-field='${name}']`);if(input instanceof HTMLInputElement||input instanceof HTMLSelectElement)input.value=String(value);}const {enabled}=this.beatBounceAuthoringSnapshot();for(const control of this.shadowRoot?.querySelectorAll("[data-beat-bounce-field],button[data-beat-bounce-action]")??[])if(control instanceof HTMLInputElement||control instanceof HTMLSelectElement||control instanceof HTMLButtonElement)control.disabled=!enabled;this.renderBeatBounceStatus();}
  ...
  async handleInteractionClick(event) {
    ...
    else if (action === "beat-bounce-reset") this.resetBeatBounce();
    else if (action === "beat-bounce-load" && event.isTrusted) this.openBeatBouncePicker(event);
    else if (action === "beat-bounce-save" && event.isTrusted) this.exportBeatBounce(event);
  ...
  handleInteractionInput(event) {
    ...
    else if ((target instanceof HTMLInputElement || target instanceof HTMLSelectElement) && target.dataset.beatBounceField) this.applyBeatBounceControls();
  }
  ...
  setMenuOpen(open, options = {}) {
    ...
    if (open) {
      this.invalidateBeatBouncePicker(); this.beatBounceAuthoringEnabled=false;
  ...
  beatBounceInput() { return this.beatBouncePicker; }
}
