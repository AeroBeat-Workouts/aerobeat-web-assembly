...
import { beatBounceConfigArtifactFilename, beatBounceConfigArtifactMimeType, createBeatBounceConfig, defaultBeatBounceConfig, maximumBeatBounceConfigBytes, normalizeBeatBounceConfig, parseBeatBounceConfig, serializeBeatBounceConfig } from "@aerobeat/web-renderer";
...
export class AeroGame extends HTMLElement {
  ...
  applyBeatBounceControls() {
    ...
    try{const config=createBeatBounceConfig(value("leadBeats"),value("heightWorldUnits"),Number(value("apexPercent"))/100,value("riseEasing"),value("fallEasing"));this.commitBeatBounceConfig(config,"Bounce updated.");}catch{/* invalid intermediate text leaves prior config unchanged */}
  }
  ...
  async handleBeatBounceFile(event){const input=event.currentTarget;if(!(input instanceof HTMLInputElement))return;const request=this.beatBouncePickerRequest,file=input.files?.[0]??null;input.value="";if(!event.isTrusted){this.invalidateBeatBouncePicker();return;}if(!file||!request||!this.isBeatBounceRequestCurrent(request)){this.invalidateBeatBouncePicker();return;}try{if(file.size>maximumBeatBounceConfigBytes)throw new Error("invalid");const bytes=await file.arrayBuffer();if(!this.isBeatBounceRequestCurrent(request)||bytes.byteLength>maximumBeatBounceConfigBytes)throw new Error("invalid");const text=new TextDecoder("utf-8",{fatal:true}).decode(bytes);if(!this.isBeatBounceRequestCurrent(request))return;const config=parseBeatBounceConfig(text);if(!this.isBeatBounceRequestCurrent(request))return;normalizeBeatBounceConfig(config);if(!this.isBeatBounceRequestCurrent(request))return;this.commitBeatBounceConfig(config,"Bounce config loaded.");}catch{if(this.isBeatBounceRequestCurrent(request))this.setBeatBounceStatus("Bounce config is invalid.",true);}finally{if(this.beatBouncePickerRequest===request)this.invalidateBeatBouncePicker();}}
