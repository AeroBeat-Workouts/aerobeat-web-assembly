...
export class AeroGame extends HTMLElement {
  constructor() {
    ...
    // 0.0.63 C5 (376l): live session-level equipment config. Initialized to
    // the baked build defaults; Test mode can APPLY a modified config via
    // setEquipmentConfigYaml() without a rebuild, and session resets restore
    // the defaults (see connectedCallback / teardown).
    ...
    this.boundVisibility = () => { if (document.hidden) { this.releaseDebugCameraControls(); this.invalidateTestPresentationPicker(); this.testPresentationAuthoringEnabled=false; } void this.applyVisibility(); };
  ...
  exportDebugCameraPose() {
    if (!this.graph || !this.debugCameraSnapshot().enabled || typeof this.graph.renderer.exportDebugCameraPoseArtifact !== "function") return;
  ...
  openDebugCameraPosePicker() {
    ...
    if (!graph || !snapshot.enabled || typeof graph.renderer.loadDebugCameraPose !== "function") return;
  ...
  isDebugCameraPoseRequestCurrent(request) {
    return request?.connectionGeneration === this.connectedGeneration && request?.sessionGeneration === this.sessionGeneration && request?.graph === this.graph && this.debugCameraSnapshot().enabled;
  }
  ...
  async handleInteractionClick(event) {
    ...
    else if (action === "equipment-config-apply") this.applyEquipmentConfig();
  ...
  handleInteractionInput(event) {
    ...
    else if (target instanceof HTMLTextAreaElement && target.dataset.equipmentConfigField === "yaml") this.validateEquipmentConfigDraft(target.value);
  }
  ...
  equipmentConfigTextarea() { const el = this.shadowRoot?.querySelector("[data-equipment-config-field='yaml']"); if (!(el instanceof HTMLTextAreaElement)) throw new Error("Equipment config textarea missing"); return el; }
  ...
  /**
   * 0.0.63 C5 (376l): live session-level equipment config state.
   *
   * Parse `text` as the strict equipment-config YAML subset and validate it
   * against the schema (missing keys merge from the build defaults). On
   * success the config REPLACES `this.equipmentConfig` — C2/C3/C4 pick it up
   * on the next rendered frame — and `ok` is true. On any parse or schema
   * error the prior state is left untouched and the error message is
   * returned so the Test-mode panel can show it without applying.
   *
   * @param {string} text - Full equipment-config YAML text.
   * @returns {{ok: true, config: Readonly<Record<PropertyKey, unknown>>} | {ok: false, error: string}}
   */
  setEquipmentConfigYaml(text) {
    try {
      const config = parseEquipmentConfigYaml(text);
      this.equipmentConfig = config;
      this.equipmentConfigStatus = "Equipment config applied.";
      this.renderEquipmentConfigStatus();
      return { ok: true, config };
    } catch (e) {
      const error = e instanceof Error ? e.message : String(e);
      this.equipmentConfigStatus = error;
      this.renderEquipmentConfigStatus();
  ...
  /**
   * 0.0.63 C5 (376l): Test-mode equipment config panel behavior.
   * Live-validate the textarea contents WITHOUT applying: the status output
   * shows the error message (or a valid indicator) and `this.equipmentConfig`
   * is left untouched.
   *
   * @param {string} text - Current textarea contents.
   * @returns {{ok: boolean, error: string | null}}
   */
  validateEquipmentConfigDraft(text) {
    try {
      parseEquipmentConfigYaml(text);
      this.equipmentConfigStatus = "Valid. Use Apply to make it active.";
      this.renderEquipmentConfigStatus();
  ...
  /**
   * 0.0.63 C5 (376l): APPLY button — commit the textarea contents through
   * setEquipmentConfigYaml; invalid input stays on the error and leaves the
   * live config untouched. The new config takes effect on the next rendered
   * frame (C2/C3/C4 read `this.equipmentConfig`).
   *
   * @returns {boolean} True when the config was applied.
   */
  applyEquipmentConfig() {
    if (!this.testPresentationAuthoringSnapshot().enabled) return false;
    const text = this.equipmentConfigTextarea().value;
    // The next displayed frame reads the live config through the normal
    // render loop — no direct render call here.
    return this.setEquipmentConfigYaml(text).ok;
  }

  /**
   * 0.0.63 C5 (376l): RESET button — restore the baked build defaults into
   * the live state and refresh the textarea.
   *
   * @returns {boolean}
   */
  resetEquipmentConfig() {
    if (!this.testPresentationAuthoringSnapshot().enabled) return false;
    this.equipmentConfig = validateEquipmentConfig(equipmentConfigDefaults);
    this.equipmentConfigTextarea().value = serializeEquipmentConfigYaml(this.equipmentConfig);
    this.equipmentConfigStatus = "Reset to build defaults.";
    this.renderEquipmentConfigStatus();
    // The next displayed frame reads the baked defaults through the render loop.
    return true;
  }

  /**
   * 0.0.63 C5 (376l): EXPORT button — the textarea content is only exported
   * when it is currently valid; it downloads as
   * `aerobeat-equipment-config.yaml` via a temporary `<a download>` click
   * (no server, no network).
   *
   * @param {{isTrusted?: boolean}} event
  ...
  exportEquipmentConfig(event) {
    ...
    const text = this.equipmentConfigTextarea().value;
    const result = this.validateEquipmentConfigDraft(text);
    if (!result.ok) { this.equipmentConfigStatus = `${result.error} (fix the YAML before exporting)`; this.renderEquipmentConfigStatus(true); return false; }
    const blob = new Blob([text], { type: "text/yaml" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.hidden = true; anchor.download = "aerobeat-equipment-config.yaml"; anchor.href = url;
  ...
  /**
   * 0.0.63 C5 (376l): render the equipment-config status line.
   *
   * @param {boolean} [error]
   * @returns {void}
   */
  renderEquipmentConfigStatus(error = false) {
    ...
    if (output !== null && typeof output === "object") { /** @type {{value: string, dataset: Record<string,string>}} */ (output).value = this.equipmentConfigStatus; /** @type {{dataset: Record<string,string>}} */ (output).dataset.error = error ? "true" : "false"; }
  }

  /**
   * 0.0.63 C5 (376l): keep the panel in sync — preload the textarea with the
   * current config's YAML on first show, and enable/disable the controls
   * with the Visual Test authoring state.
   *
   * @returns {void}
   */
  renderEquipmentConfigControls() {
    const { visible, enabled } = this.testPresentationAuthoringSnapshot();
    const textarea = this.equipmentConfigTextarea();
    // Preload the serialized live config on first VISUAL Test surface show;
    // once populated the user's edits own the text. When the panel is hidden
    // (non-test state) always reset to empty so idle never leaks a stale edit.
    if (!visible) { textarea.value = ""; return; }
    if (textarea.value === "") textarea.value = serializeEquipmentConfigYaml(this.equipmentConfig);
    for (const control of this.shadowRoot?.querySelectorAll("[data-equipment-config-field], button[data-equipment-config-action]") ?? []) {
      if ("disabled" in control && typeof control.disabled === "boolean") control.disabled = !enabled;
    }
    this.renderEquipmentConfigStatus();
  }
}
