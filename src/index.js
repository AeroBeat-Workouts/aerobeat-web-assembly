...
export class AeroGame extends HTMLElement {
  ...
  installGameSetupControls() {
    ...
    const rows=[["timingWindowMs","Collider timing window (ms)",...gameSetupBounds.timingWindowMs,1],["colliderRadius","Collider radius",...gameSetupBounds.colliderRadius,.01],["directionToleranceDegrees","Direction tolerance (degrees)",...gameSetupBounds.directionToleranceDegrees,1]];
    ...
    const note=document.createElement("p");note.className="game-setup-note";note.textContent="Timing, collider, and spawn-distance changes apply on next Start/Test. Grid and visibility changes apply live.";section.append(note);content.prepend(section);
  }
  applyGameSetupControl(input){const current=getGameSetupSnapshot(),field=input.dataset.gameSetupField;if(!field)return;let next=current;if(input instanceof HTMLInputElement&&input.type==="number"){if(!input.validity.valid||!Number.isFinite(input.valueAsNumber)){this.renderGameSetupError(field,"Enter a value within the allowed range and step.");return;}const value=input.valueAsNumber;if(field==="spawnDistanceOverrideWorldUnits")next={...current,spawnDistanceOverride:{...current.spawnDistanceOverride,normalSpawnDistanceWorldUnits:value}};else if(["noseCameraRangeXWorldUnits","noseCameraRangeYWorldUnits","timingWindowMs","colliderRadius","directionToleranceDegrees","noteScalePercent","obstacleScalePercent","bombScalePercent","markerScalePercent"].includes(field))next={...current,[field]:value};else return;}else if(input instanceof HTMLSelectElement&&field==="guidanceBandMode")next={...current,guidanceBandMode:input.value};else if(input instanceof HTMLInputElement&&["showGameplayGrid","noseCameraParallaxEnabled","visibleToleranceRange","visibleColliderRadius"].includes(field))next={...current,[field]:input.checked};else if(input instanceof HTMLInputElement&&field==="spawnDistanceOverrideEnabled")next={...current,spawnDistanceOverride:{...current.spawnDistanceOverride,enabled:input.checked}};else return;try{setGameSetupSnapshot(next);this.gameSetupDrafts.delete(field);if(field==="guidanceBandMode")this.renderGuidanceModeFrame(next);this.renderGameSetupError(field,"");}catch{this.gameSetupDrafts.delete(field);this.renderGameSetupControls();this.renderGameSetupError(field,"Enter a value within the allowed range and step.");}}
