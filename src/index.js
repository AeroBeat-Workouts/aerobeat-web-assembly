...
import {
  ...
  judgementDiagnosticCodes,
  maximumObstaclesPerChart,
...
import { gameSetupBounds, getGameSetupSnapshot, setGameSetupSnapshot, subscribeGameSetup } from "./game-setup-coordinator.js";
...
const INTERNAL_RENDERER_EFFECTIVE_PALETTE = Symbol.for("aerobeat.web-renderer.internal-effective-palette");
...
const FLOW_REIMPORT_MESSAGES = Object.freeze({
  ...
  flow_colliders_reimport_required: "Package predates explicit flow_colliders_v1 authoring and must be reimported"
});
// 0.0.54 W2-E: pre-Boxing-collider downloaded packages (the four-chart Lanes/Grid
// pair predates collider authoring) cannot bind the sole boxing_collider_v1
// variant; selecting Boxing on them surfaces this bounded reimport reason.
const BOXING_REIMPORT_MESSAGES = Object.freeze({
  boxing_collider_reimport_required: "This downloaded song predates Boxing collider authoring. Reimport it to play."
});
...
function equipmentControlMarkup() {
  return EQUIPMENT_CONTROL_GROUPS.map((group) => `<fieldset class="equipment-control-group"><legend>${group.label}</legend><div class="equipment-fields">${group.controls.map((control) => {
    const id = `equipment-${control.path.replaceAll(".", "-")}`;
    if (control.kind === "select") return `<label for="${id}">${control.label}</label><select id="${id}" data-equipment-config-field="${control.path}">${EQUIPMENT_EASE_OPTIONS.map((value) => `<option value="${value}">${value}</option>`).join("")}</select>`;
    return `<label for="${id}">${control.label}</label><input id="${id}" data-equipment-config-field="${control.path}" type="number" step="any" inputmode="decimal"${control.nullable ? " placeholder=\"Neutral\"" : ""}>`;
  }).join("")}</div></fieldset>`).join("");
}

function equipmentConfigValue(config, path) {
  return path.split(".").reduce((value, key) => value?.[key], config);
}

function equipmentConfigCandidate(config, path, value) {
  const candidate = structuredClone(config); const keys = path.split("."); let target = candidate;
  for (const key of keys.slice(0, -1)) target = target[key];
  target[keys.at(-1)] = value;
  return validateEquipmentConfig(candidate);
}
...
export class AeroGame extends HTMLElement {
  constructor() {
    ...
    this.debugCameraPosePickerRequest = null;
    this.testPresentationConfig = defaultTestPresentationConfig;
    this.testPresentationPickerRequest = null;
    this.testPresentationPickerGeneration = 0;
    this.testPresentationPickerToken = null;
    this.testPresentationAuthoringEnabled = false;
    this.testPresentationStatus = "";
    // Private session-level equipment authoring state. Grouped Test controls
    ...
    this.boundTestEquipmentPointerMove = (event) => this.handleTestEquipmentPointerMove(event);
    this.boundTestPresentationFile = (event) => { void this.handleTestPresentationFile(event); };
    this.boundVisibility = () => { if (document.hidden) { this.resetTestEquipmentAuthoringState({ render:false }); this.invalidateTestPresentationPicker(); this.testPresentationAuthoringEnabled=false; } void this.applyVisibility(); };
    ...
    this.boundInteractionBlur = (event) => this.handleInteractionBlur(event);
    ...
    this.testPresentationPicker = document.createElement("input"); this.testPresentationPicker.type = "file"; this.testPresentationPicker.accept = ".json,application/json"; this.testPresentationPicker.hidden = true; this.testPresentationPicker.dataset.role = "test-presentation-config-picker"; root.prepend(this.testPresentationPicker);
  }
  ...
  connectedCallback() {
    ...
    try {
      ...
      if (typeof graph.renderer.setDebugCameraAuthoringInputEnabled === "function") graph.renderer.setDebugCameraAuthoringInputEnabled(true);
  ...
  async selectGameplayAxes(rulesetId, recipeId = this.lastBoxingRecipeId) {
    ...
    if (boxingGameplayRulesetIds.includes(rulesetId) && !conversionRecipeIds.includes(recipeId)) throw new TypeError("Boxing conversion intent is invalid");
    if (!boxingGameplayRulesetIds.includes(rulesetId) && !flowGameplayRulesetIds.includes(rulesetId)) throw new TypeError("Gameplay ruleset intent is invalid");
    ...
    return this.enqueueLifecycleIntent("gameplay-mode-select", async (owner) => {
      ...
      if (!target?.variantId) {
        if (rulesetId === gameplayRulesetIds.boxingCollider) {
          // 0.0.54 W2-E: the selected package predates Boxing collider authoring
          // (no boxing_collider_v1 variant exists in it). Surface the bounded
          // reimport reason through the same lastError/info-action UX path the
          // flow reimport reasons use instead of the generic unavailable throw.
          this.lastError = Object.freeze({ code: "boxing_collider_reimport_required", message: BOXING_REIMPORT_MESSAGES.boxing_collider_reimport_required });
   ...
   */
  describeEquipmentConfig() { return Object.freeze(structuredClone(this.equipmentConfig)); }
  ...
  bindGraph() {
    ...
    if (typeof this.graph.content.subscribe === "function") this.unsubscribe.push(this.graph.content.subscribe((snapshot) => {
      this.applyContentVisualTruth(snapshot);
  ...
  bindHostLifecycle() {
    ...
    this.shadowRoot?.addEventListener("focusout", this.boundInteractionBlur);
  ...
  /** Configure content for the explicit action purpose, preserving active Visual Test truth without entering Play calibration. @param {boolean} futureOnly @param {"play"|"visual_test"} [purpose] @param {boolean} [bindDesiredSetup] */
  ...
  applyContentVisualTruth(snapshot=this.graph?.content.getSnapshot()) {
    if(!this.graph)return;const graph=this.graph,renderer=/** @type {Record<PropertyKey,unknown>} */(graph.renderer),setPalette=renderer[INTERNAL_RENDERER_EFFECTIVE_PALETTE];
    if(snapshot?.state!=="ready"){if(typeof setPalette==="function")setPalette.call(graph.renderer,null,null);return;}
    if(snapshot.theme)graph.renderer.setTheme(snapshot.theme);
    const paletteAccessor=/** @type {Record<PropertyKey,unknown>} */(graph.content)[INTERNAL_CONTENT_EFFECTIVE_PALETTE],palette=typeof paletteAccessor==="function"?paletteAccessor.call(graph.content,snapshot.generation):null;
    if(typeof setPalette==="function")setPalette.call(graph.renderer,typeof palette?.left==="string"?palette.left:null,typeof palette?.right==="string"?palette.right:null);
  }
  ...
  applyActiveVisualProfile() {
    ...
    const activeScaleSetup = this.activeSessionSetup ?? this.desiredGameSetup;
    if (typeof this.graph.renderer.setVisualScales === "function") this.graph.renderer.setVisualScales(rendererVisualScales(activeScaleSetup));
  }
  ...
  reconcileTerminalServices(graph = this.graph) {
    ...
    this.resetTestEquipmentAuthoringState({ render:false });
    this.stopFrameLoop(); graph.video.pause(this.videoElement());
  ...
  applyGameSetup(graph=this.graph,setup=this.desiredGameSetup) {
    if(!graph)return;
    ...
    if(typeof graph.renderer.setVisualScales==="function")graph.renderer.setVisualScales(rendererVisualScales(setup));
    this.lastAppliedScaleId=rendererVisualScalesId(setup);
    this.renderSpawnDistanceWorldUnits=null;
  }

  /** @param {import("./game-setup-coordinator.js").AeroGameSetupSnapshot} setup */
  renderGuidanceModeFrame(setup) {
    // er3m: push the live guidance band mode into the next renderer frame immediately.
    if (this.graph && typeof this.graph.renderer.renderGameplayScene === "function") {
      const frame = this.rendererFrame();
      frame.guidanceBandMode = setup.guidanceBandMode;
      void this.graph.renderer.renderGameplayScene(frame, null, null);
    }
  }
  ...
  rendererFrame() {
    ...
    if (events !== this.renderEventSource || this.renderPresentationConfig !== this.testPresentationConfig || !Object.is(this.renderSpawnDistanceWorldUnits,spawnDistanceWorldUnits)) {
      ...
      this.renderEventIndex = createSessionTargetIndex(events, { mapBeatToTimelineMs:typeof mapBeatToTimelineMs==="function"?mapBeatToTimelineMs:undefined, songDurationMs:typeof content.song?.durationSec==="number"?content.song.durationSec*1000:undefined, bounceLeadBeats:this.testPresentationConfig.bounceLeadBeats, normalSpawnLeadMs:spawnDistanceWorldUnits===null?null:spawnDistanceWorldUnits/canonicalWorldUnitsPerMs, skyMode:this.testPresentationConfig.skyMode, skyPreludeDurationMs:this.testPresentationConfig.skyPreludeDurationMs });
    ...
    const aftermath = projectAftermathEntries(events, gameplay, nowMs, targets, this.renderEventIndex, isBoxingColliderForAftermath ? Object.freeze({ topRowReachWU: Number(setup.topRowReachWU), bottomRowReachWU: Number(setup.bottomRowReachWU) }) : null, aftermathSaberWristHistory, this.testAutomaticFeedbackEnabled);
    ...
    // 0.0.54 W1-B/W2-C: state-driven hazard-contact presentation — copied from the
    // gameplay session snapshot (flow_colliders_v1 + play purpose only). Absent or
    ...
    return {
      ...
      visibleToleranceRange:Boolean(setup.visibleToleranceRange),
      visibleColliderRadius:Boolean(setup.visibleColliderRadius),
      colliderRadius:Number(setup.colliderRadius),
      directionToleranceDegrees:Number(setup.directionToleranceDegrees),
  ...
  debugCameraSnapshot() {
    ...
    const enabled = visible && !this.menuOpen && this.lifecycle === "connected" && !document.hidden && ["playing","paused_manual","completed"].includes(String(session?.state??""));
  ...
  syncDebugCameraPresentation() {
    ...
    this.renderEnvironmentControls(); const bounceEnabled=this.testPresentationAuthoringSnapshot().enabled;if(this.testPresentationAuthoringEnabled&&!bounceEnabled)this.invalidateTestPresentationPicker();this.testPresentationAuthoringEnabled=bounceEnabled;this.renderTestPresentationControls();this.renderEquipmentConfigControls();this.syncDebugCameraControlState(snapshot);
  }
  ...
  syncDebugCameraControlState(snapshot = this.debugCameraSnapshot()) {
    ...
    const cameraControlEnabled = snapshot.enabled && this.testEquipmentMouseHand === "off";
    this.debugCameraSpeedMode = speedMode;
    const pressedIntents = new Set(this.debugCameraControlPointers.values());
    const signature = JSON.stringify([snapshot.visible, snapshot.enabled, cameraControlEnabled, captureMode, speedMode, boostActive, activeIntentCount, this.environmentLoadState, [...pressedIntents].sort()]);
    ...
    for (const button of this.shadowRoot?.querySelectorAll("button[data-debug-camera-intent]") ?? []) {
      ...
      button.disabled = !cameraControlEnabled; button.setAttribute("aria-pressed", pressed ? "true" : "false"); button.dataset.active = pressed ? "true" : "false";
    }
    const speed = this.shadowRoot?.querySelector("button[data-action='debug-camera-speed']");
    if (speed instanceof HTMLButtonElement) {
      const boost = speedMode === "boost"; speed.disabled = !cameraControlEnabled; speed.setAttribute("aria-pressed", boost ? "true" : "false"); speed.setAttribute("aria-label", `Camera movement speed ${boost ? "Boost" : "Normal"}. Activate ${boost ? "Normal" : "Boost"}.`);
    ...
    if (reset instanceof HTMLButtonElement) reset.disabled = !cameraControlEnabled;
    const loadPose = this.shadowRoot?.querySelector("button[data-action='debug-camera-load']");
    if (loadPose instanceof HTMLButtonElement) loadPose.disabled = !cameraControlEnabled;
    const exportPose = this.shadowRoot?.querySelector("button[data-action='debug-camera-export']");
    if (exportPose instanceof HTMLButtonElement) exportPose.disabled = !cameraControlEnabled;
  ...
  handleDebugCameraPointerDown(event) {
    if (!(event instanceof PointerEvent) || !this.graph || !this.debugCameraAuthoringControlEnabled()) return;
  ...
  debugCameraAuthoringControlEnabled() { return this.debugCameraSnapshot().enabled && this.testEquipmentMouseHand === "off"; }

  toggleDebugCameraSpeed() {
    if (!this.graph || !this.debugCameraAuthoringControlEnabled() || typeof this.graph.renderer.setDebugCameraSpeedMode !== "function") return;
  ...
  resetDebugCamera() {
    if (!this.graph || !this.debugCameraAuthoringControlEnabled() || typeof this.graph.renderer.resetDebugCamera !== "function") return;
    this.releaseDebugCameraControls(); this.graph.renderer.resetDebugCamera();
    this.publish("session_changed");
  }

  exportDebugCameraPose() {
    if (!this.graph || !this.debugCameraAuthoringControlEnabled() || typeof this.graph.renderer.exportDebugCameraPoseArtifact !== "function") return;
    this.releaseDebugCameraControls();
    if (typeof this.graph.renderer.releaseDebugCameraAuthoringInput === "function") this.graph.renderer.releaseDebugCameraAuthoringInput();
    const artifact = this.graph.renderer.exportDebugCameraPoseArtifact();
    const blob = new Blob([new Uint8Array(artifact.bytes)], { type: artifact.mimeType });
    const url = URL.createObjectURL(blob); const anchor = document.createElement("a");
    anchor.hidden = true; anchor.download = artifact.filename; anchor.href = url; this.shadowRoot?.append(anchor);
    try { anchor.click(); } finally { anchor.remove(); URL.revokeObjectURL(url); }
  }

  openDebugCameraPosePicker() {
    const graph = this.graph; const snapshot = this.debugCameraSnapshot();
    if (!graph || !snapshot.enabled || !this.debugCameraAuthoringControlEnabled() || typeof graph.renderer.loadDebugCameraPose !== "function") return;
    const input = this.cameraPoseInput(); input.value = "";
    this.debugCameraPosePickerRequest = Object.freeze({ connectionGeneration:this.connectedGeneration, sessionGeneration:this.sessionGeneration, graph });
    this.setDebugCameraPoseStatus(""); input.click();
  }

  /** @param {Event} event */
  async handleDebugCameraPoseFile(event) {
    const input = event.currentTarget; if (!(input instanceof HTMLInputElement)) return;
    const file = input.files?.[0] ?? null; const request = this.debugCameraPosePickerRequest;
    input.value = ""; this.debugCameraPosePickerRequest = null;
    if (!file || !request || !this.isDebugCameraPoseRequestCurrent(request)) return;
    try {
      if (file.size > MAXIMUM_CAMERA_POSE_FILE_BYTES) throw cameraPoseFileError("Camera pose file exceeds 16 KiB.");
      const bytes = await file.arrayBuffer();
      if (bytes.byteLength > MAXIMUM_CAMERA_POSE_FILE_BYTES) throw cameraPoseFileError("Camera pose file exceeds 16 KiB.");
      const text = new TextDecoder("utf-8", { fatal:true }).decode(bytes);
      let value; try { value = JSON.parse(text); } catch { throw cameraPoseFileError("Camera pose file is invalid."); }
      if (!this.isDebugCameraPoseRequestCurrent(request)) return;
      this.releaseDebugCameraControls();
      await request.graph.renderer.loadDebugCameraPose(value);
      if (!this.isDebugCameraPoseRequestCurrent(request)) return;
      this.setDebugCameraPoseStatus("Camera pose loaded."); this.debugCameraUiSignature = ""; this.syncDebugCameraControlState();
    } catch (error) {
      if (this.isDebugCameraPoseRequestCurrent(request)) this.setDebugCameraPoseStatus(errorMessage(error) === "Camera pose file exceeds 16 KiB." ? "Camera pose file exceeds 16 KiB." : "Camera pose file is invalid.", true);
    }
  }

  isDebugCameraPoseRequestCurrent(request) {
    return request?.connectionGeneration === this.connectedGeneration && request?.sessionGeneration === this.sessionGeneration && request?.graph === this.graph && this.debugCameraAuthoringControlEnabled();
  }

  setDebugCameraPoseStatus(message, error = false) {
    const output = this.shadowRoot?.querySelector("[data-role='debug-camera-pose-status']");
  ...
  testPresentationAuthoringSnapshot() {
    const session=this.graph?.gameplay.getSnapshot().session;
    const visible=Boolean(this.graph&&this.sessionStartRequested&&this.activeSessionAction==="test"&&session?.purpose==="visual_test");
    const enabled=visible&&this.lifecycle==="connected"&&!document.hidden&&!this.menuOpen&&["playing","paused_manual","completed"].includes(String(session?.state??""));
    return Object.freeze({visible,enabled});
  }

  applyTestPresentationControls() {
    if(!this.graph||!this.testPresentationAuthoringSnapshot().enabled)return;
    const value=(name)=>{const input=this.shadowRoot?.querySelector(`[data-test-presentation-field='${name}']`);return input instanceof HTMLInputElement?Number(input.value):input instanceof HTMLSelectElement?input.value:undefined;};
    try{const config=createTestPresentationConfig(value("bounceLeadBeats"),value("bounceHeightWorldUnits"),Number(value("bounceApexPercent"))/100,value("bounceRiseEasing"),value("bounceFallEasing"),value("normalSpawnDistanceWorldUnits"),value("skyMode"),value("skyPreludeHeightWorldUnits"),value("skyPreludeDurationMs"),value("skyPreludeEasing"),value("boxingLaneSeparationWorldUnits"));this.commitTestPresentationConfig(config,"Test presentation updated.");}catch{/* invalid intermediate text leaves prior config unchanged */}
  }

  commitTestPresentationConfig(config,status){const normalized=normalizeTestPresentationConfig(config);if(!this.graph||!this.testPresentationAuthoringSnapshot().enabled)return false;this.graph.renderer.setTestPresentationConfig(normalized);this.testPresentationConfig=normalized;this.renderPresentationConfig=null;this.testPresentationStatus=status;this.renderTestPresentationControls();return true;}
  resetTestPresentation(){if(this.commitTestPresentationConfig(defaultTestPresentationConfig,"Test presentation reset."))this.renderGameplay();}
  exportTestPresentation(event){if(!event?.isTrusted||!this.testPresentationAuthoringSnapshot().enabled)return;const text=serializeTestPresentationConfig(this.testPresentationConfig),blob=new Blob([text],{type:testPresentationConfigArtifactMimeType}),url=URL.createObjectURL(blob),anchor=document.createElement("a");anchor.hidden=true;anchor.download=testPresentationConfigArtifactFilename;anchor.href=url;this.shadowRoot?.append(anchor);try{anchor.click();this.testPresentationStatus="Test presentation saved.";}finally{anchor.remove();URL.revokeObjectURL(url);this.renderTestPresentationStatus();}}
  invalidateTestPresentationPicker(){this.testPresentationPickerGeneration+=1;this.testPresentationPickerToken=null;this.testPresentationPickerRequest=null;this.testPresentationInput().value="";}
  openTestPresentationPicker(event){if(!event?.isTrusted||!this.graph||!this.testPresentationAuthoringSnapshot().enabled)return;this.invalidateTestPresentationPicker();const token=Object.freeze({}),request=Object.freeze({generation:this.testPresentationPickerGeneration,token,connectionGeneration:this.connectedGeneration,sessionGeneration:this.sessionGeneration,graph:this.graph,purpose:"visual_test",lifecycle:"connected"});this.testPresentationPickerToken=token;this.testPresentationPickerRequest=request;this.testPresentationStatus="";this.renderTestPresentationStatus();this.testPresentationInput().click();}
  async handleTestPresentationFile(event){const input=event.currentTarget;if(!(input instanceof HTMLInputElement))return;const request=this.testPresentationPickerRequest,file=input.files?.[0]??null;input.value="";if(!event.isTrusted){this.invalidateTestPresentationPicker();return;}if(!file||!request||!this.isTestPresentationRequestCurrent(request)){this.invalidateTestPresentationPicker();return;}try{if(file.size>maximumTestPresentationConfigBytes)throw new Error("invalid");const bytes=await file.arrayBuffer();if(!this.isTestPresentationRequestCurrent(request)||bytes.byteLength>maximumTestPresentationConfigBytes)throw new Error("invalid");const text=new TextDecoder("utf-8",{fatal:true}).decode(bytes);if(!this.isTestPresentationRequestCurrent(request))return;const config=parseTestPresentationConfig(text);if(!this.isTestPresentationRequestCurrent(request))return;normalizeTestPresentationConfig(config);if(!this.isTestPresentationRequestCurrent(request))return;this.commitTestPresentationConfig(config,"Test presentation loaded.");}catch{if(this.isTestPresentationRequestCurrent(request))this.setTestPresentationStatus("Test presentation is invalid.",true);}finally{if(this.testPresentationPickerRequest===request)this.invalidateTestPresentationPicker();}}
  isTestPresentationRequestCurrent(request){const state=this.testPresentationAuthoringSnapshot();return request!==null&&request===this.testPresentationPickerRequest&&request.generation===this.testPresentationPickerGeneration&&request.token===this.testPresentationPickerToken&&request.connectionGeneration===this.connectedGeneration&&request.sessionGeneration===this.sessionGeneration&&request.graph===this.graph&&request.purpose==="visual_test"&&request.lifecycle===this.lifecycle&&state.enabled;}
  setTestPresentationStatus(message,error=false){this.testPresentationStatus=String(message).slice(0,96);this.renderTestPresentationStatus(error);}
  renderTestPresentationStatus(error=false){const output=this.shadowRoot?.querySelector("[data-role='test-presentation-status']");if(output instanceof HTMLOutputElement){output.value=this.testPresentationStatus;output.dataset.error=error?"true":"false";}}
  renderTestPresentationControls(){const config=this.testPresentationConfig,values={bounceLeadBeats:config.bounceLeadBeats,bounceHeightWorldUnits:config.bounceHeightWorldUnits,bounceApexPercent:config.bounceApexFraction*100,bounceRiseEasing:config.bounceRiseEasing,bounceFallEasing:config.bounceFallEasing,normalSpawnDistanceWorldUnits:config.normalSpawnDistanceWorldUnits,skyMode:config.skyMode,skyPreludeHeightWorldUnits:config.skyPreludeHeightWorldUnits,skyPreludeDurationMs:config.skyPreludeDurationMs,skyPreludeEasing:config.skyPreludeEasing,boxingLaneSeparationWorldUnits:config.boxingLaneSeparationWorldUnits};for(const [name,value]of Object.entries(values)){const input=this.shadowRoot?.querySelector(`[data-test-presentation-field='${name}']`);if(input instanceof HTMLInputElement||input instanceof HTMLSelectElement)input.value=String(value);}const {enabled}=this.testPresentationAuthoringSnapshot();for(const control of this.shadowRoot?.querySelectorAll("[data-test-presentation-field],button[data-test-presentation-action]")??[])if(control instanceof HTMLInputElement||control instanceof HTMLSelectElement||control instanceof HTMLButtonElement)control.disabled=!enabled;this.renderTestPresentationStatus();}
  ...
  applyEnvironmentControls() {
    ...
    try {
      ...
      this.graph.renderer.setEnvironmentTransform(config.transform); this.environmentConfigs.set(entry.descriptor.id, config); this.environmentStatus = ""; this.renderEnvironmentControls();
  ...
  async handleInteractionClick(event) {
    ...
    else if (action === "test-presentation-reset") this.resetTestPresentation();
    else if (action === "test-presentation-load" && event.isTrusted) this.openTestPresentationPicker(event);
    else if (action === "test-presentation-save" && event.isTrusted) this.exportTestPresentation(event);
    else if (action === "equipment-config-reset") this.resetEquipmentConfig();
  ...
  handleInteractionInput(event) {
    ...
    else if (target instanceof HTMLInputElement && target.dataset.testAutomaticFeedbackToggle === "true") this.setTestAutomaticFeedbackEnabled(target.checked);
    else if (target instanceof HTMLInputElement && target.dataset.equipmentMouseHand && target.checked) this.setTestEquipmentMouseHand(target.value);
    else if ((target instanceof HTMLInputElement || target instanceof HTMLSelectElement) && target.dataset.equipmentConfigField) this.applyEquipmentConfigControl(target);
  }

  handleInteractionBlur(event){const target=event.target;if(!(target instanceof HTMLInputElement)||target.type!=="number"||!target.dataset.gameSetupField||!this.gameSetupDrafts.has(target.dataset.gameSetupField))return;if(target.validity.valid&&Number.isFinite(target.valueAsNumber))this.applyGameSetupControl(target);else{this.gameSetupDrafts.delete(target.dataset.gameSetupField);this.renderGameSetupControls();this.renderGameSetupError(target.dataset.gameSetupField,"Enter a value within the allowed range and step.");}}

  handleInteractionKeydown(event) {
    if (!(event instanceof KeyboardEvent)) return;
    const target=event.target;if(event.key==="Enter"&&target instanceof HTMLInputElement&&target.type==="number"&&target.dataset.gameSetupField){event.preventDefault();this.applyGameSetupControl(target);return;}
  ...
  setMenuOpen(open, options = {}) {
    ...
    if (open) {
      this.invalidateTestPresentationPicker(); this.testPresentationAuthoringEnabled=false;
  ...
  installGameSetupControls() {
    ...
    for(const [field,text] of [["showGameplayGrid","Show 4 × 3 grid"],["noseCameraParallaxEnabled","Nose camera parallax"],["visibleToleranceRange","Visible tolerance range"],["visibleColliderRadius","Visible collider radius"]]){const label=document.createElement("label");label.className="environment-option";const input=document.createElement("input");input.type="checkbox";input.dataset.gameSetupField=field;if(field==="showGameplayGrid")input.dataset.action="show-gameplay-grid";const span=document.createElement("span");span.textContent=text;label.append(input,span);section.append(label);}
    // W3-C: Spawn override, camera ranges, bomb/marker scales, and the Guidance
    // bands select are hidden from the drawer (fields stay in the snapshot,
    // data-intact — same pattern as the hidden Note/Obstacle scales and the
    // hidden Lanes/Grid rulesets). Guidance bands are locked to the
    // "target_arrivals" renderer input per the 0.0.53 successor decisions.
  ...
  handleUiIntent(event) {
    ...
    if (detail.type === "session-start") void this.startFromMenu("play");
      ...
      try {
        ...
        if (boxingGameplayRulesetIds.includes(selectedRuleset)) void this.selectGameplayAxes(selectedRuleset, recipeId).catch((error) => this.handleError(error));
      } catch (error) { this.handleError(error); }
    }
  ...
  testPresentationInput() { return this.testPresentationPicker; }
  /** @returns {HTMLOutputElement} */
  equipmentConfigStatusOutput() { const el = this.shadowRoot?.querySelector("[data-role='equipment-config-status']"); if (!(el instanceof HTMLOutputElement)) throw new Error("Equipment config status missing"); return el; }

  testEquipmentAuthoringSnapshot() {
    const session = this.graph?.gameplay.getSnapshot().session;
    const visible = Boolean(this.graph && this.sessionStartRequested && this.activeSessionAction === "test" && session?.purpose === "visual_test");
    const enabled = visible && this.lifecycle === "connected" && !document.hidden && !this.menuOpen && ["playing", "paused_manual"].includes(String(session?.state ?? ""));
    return Object.freeze({ visible, enabled });
  }
  ...
  resetTestEquipmentAuthoringState(options = {}) {
    ...
    this.testEquipmentMouseHand = "off";
    this.testEquipmentPointerPosition = null;
    if (typeof this.graph?.renderer.setDebugCameraAuthoringInputEnabled === "function") this.graph.renderer.setDebugCameraAuthoringInputEnabled(true);
    this.debugCameraUiSignature = "";
    if (options.render !== false) { this.renderEquipmentConfigControls(false); this.syncDebugCameraControlState(); this.renderGameplay(); }
  }
  ...
  setTestEquipmentMouseHand(value) {
  ...
  applyEquipmentConfigControl(control) {
    if (!this.testPresentationAuthoringSnapshot().enabled) return false;
    const path = control.dataset.equipmentConfigField;
    if (!path || !EQUIPMENT_CONTROL_PATHS.has(path)) return false;
    try {
      let value;
  ...
  exportEquipmentConfig(event) {
    if (!event?.isTrusted || !this.testPresentationAuthoringSnapshot().enabled) return false;
    const text = serializeEquipmentConfigYaml(this.equipmentConfig);
    const blob = new Blob([text], { type: "text/yaml" });
    const url = URL.createObjectURL(blob); const anchor = document.createElement("a");
    anchor.hidden = true; anchor.download = "aerobeat-equipment-config.yaml"; anchor.href = url; this.shadowRoot?.append(anchor);
    try { anchor.click(); this.equipmentConfigStatus = "Equipment config exported."; } finally { anchor.remove(); URL.revokeObjectURL(url); this.renderEquipmentConfigStatus(); }
    return true;
  }

  renderEquipmentConfigStatus(error = false) {
    const output = this.shadowRoot?.querySelector("[data-role='equipment-config-status']");
    if (output instanceof HTMLOutputElement) { output.value = this.equipmentConfigStatus; output.dataset.error = error ? "true" : "false"; }
  }

  renderEquipmentConfigControls(updateInputs = true) {
    const { visible, enabled } = this.testPresentationAuthoringSnapshot();
    if (updateInputs) for (const control of this.shadowRoot?.querySelectorAll("[data-equipment-config-field]") ?? []) {
      if (!(control instanceof HTMLInputElement || control instanceof HTMLSelectElement)) continue;
      const value = equipmentConfigValue(this.equipmentConfig, control.dataset.equipmentConfigField ?? "");
      control.value = value === null ? "" : String(value);
    }
    const toggle = this.shadowRoot?.querySelector("[data-equipment-preview-toggle='true']");
    if (toggle instanceof HTMLInputElement) toggle.checked = visible && this.testEquipmentVisible;
    const automatic = this.shadowRoot?.querySelector("[data-test-automatic-feedback-toggle='true']");
    if (automatic instanceof HTMLInputElement) automatic.checked = this.testAutomaticFeedbackEnabled;
    for (const control of this.shadowRoot?.querySelectorAll("[data-equipment-mouse-hand]") ?? []) if (control instanceof HTMLInputElement) control.checked = control.value === this.testEquipmentMouseHand;
    for (const control of this.shadowRoot?.querySelectorAll("[data-equipment-config-field], [data-equipment-preview-toggle], button[data-equipment-config-action]") ?? []) {
      if (control instanceof HTMLInputElement || control instanceof HTMLSelectElement || control instanceof HTMLButtonElement) control.disabled = !enabled;
    }
    const authoringEnabled = this.testEquipmentAuthoringSnapshot().enabled;
    for (const control of this.shadowRoot?.querySelectorAll("[data-test-automatic-feedback-toggle], [data-equipment-mouse-hand]") ?? []) if (control instanceof HTMLInputElement) control.disabled = !authoringEnabled;
    this.renderEquipmentConfigStatus();
  }
...
function cameraPoseFileError(message) { return new Error(message); }
...
function publicScorePartition(value){const base=telemetryFields(value,["partitionId","variantId","chartId","rulesetId","recipeId","modifierIds","profileId","profileVersion","profileHash","profileClass","regenerationRequired","scoringSettingsIdentity","ranked","localOnly","hits","misses","ignored","obstacleContacts","bombContacts","score","maxCombo","combo"]),flowColliderSettingsIdentity=boundedOpaqueIdentity(ownDataValue(value,"flowColliderSettingsIdentity"));return Object.freeze({...base,...(flowColliderSettingsIdentity===null?{}:{flowColliderSettingsIdentity}),mapHash:publicContentHash(ownDataValue(value,"mapHash")),scoreIdentityHash:publicContentHash(ownDataValue(value,"scoreIdentityHash")),scoringSettings:telemetryFields(ownDataValue(value,"scoringSettings"),["comboBonusPerHit","hitPoints","missPenalty"])}); }
function publicSemanticJudgement(value){if(!value||typeof value!=="object")return null;const result=ownDataValue(value,"result"),diagnostics=ownDataValue(value,"diagnostics"),allowed=new Set(judgementDiagnosticCodes);if(!["hit","miss","ignored"].includes(String(result)))return null;return Object.freeze({result,diagnostics:Object.freeze(Array.isArray(diagnostics)?diagnostics.filter((entry)=>allowed.has(entry)).slice(0,16):[])});}
function boundedOpaqueIdentity(value){return typeof value==="string"&&/^[0-9a-zA-Z:_-]{1,256}$/u.test(value)?value:null;}
function publicFlowCollidersSummary(snapshot){const judgements=Array.isArray(snapshot.judgements)?snapshot.judgements.filter((entry)=>ownDataValue(entry,"shadow")!==true):[],hazards=Array.isArray(snapshot.hazardOutcomes)?snapshot.hazardOutcomes:[],cap=(value)=>Math.min(flowCollidersPublicCountMaximum,Math.max(0,value));const hazardCounts=(kind)=>Object.freeze({contact:cap(hazards.filter((entry)=>ownDataValue(entry,"kind")===kind&&ownDataValue(entry,"result")==="contact").length),avoided:cap(hazards.filter((entry)=>ownDataValue(entry,"kind")===kind&&ownDataValue(entry,"result")==="avoided").length),unevaluatedTracking:cap(hazards.filter((entry)=>ownDataValue(entry,"kind")===kind&&ownDataValue(entry,"result")==="unevaluated_tracking").length)});const latest=publicSemanticJudgement(judgements.at(-1));return Object.freeze({schema:"aerobeat/flow_colliders_public_summary",version:1,mode:"flow",rulesetId:gameplayRulesetIds.flow,latestJudgement:latest&&latest.result!=="ignored"?latest:null,notes:Object.freeze({hit:cap(judgements.filter((entry)=>ownDataValue(entry,"result")==="hit").length),miss:cap(judgements.filter((entry)=>ownDataValue(entry,"result")==="miss").length)}),bombs:hazardCounts("bomb"),walls:hazardCounts("wall")});}
...
function flowReimportReason(error) { const code=ownDataValue(error,"code");if(!Object.hasOwn(FLOW_REIMPORT_MESSAGES,String(code)))return null;return Object.freeze({code,message:FLOW_REIMPORT_MESSAGES[code]}); }
/** 0.0.54 W2-E: one bounded reimport reason lookup over both message tables. */
function reimportReason(error) { return flowReimportReason(error) ?? ((()=>{ const code=ownDataValue(error,"code");if(!Object.hasOwn(BOXING_REIMPORT_MESSAGES,String(code)))return null;return Object.freeze({code,message:BOXING_REIMPORT_MESSAGES[code]}); })()); }
...
function rendererTelemetry(snapshot) { return Object.freeze({ serviceId: dataValue(snapshot, "serviceId"), engine: dataValue(snapshot, "engine"), state: dataValue(snapshot, "state"), supported: dataValue(snapshot, "supported"), attached: dataValue(snapshot, "attached"), contextLost: dataValue(snapshot, "contextLost"), widthCssPx: dataValue(snapshot, "widthCssPx"), heightCssPx: dataValue(snapshot, "heightCssPx"), devicePixelRatio: dataValue(snapshot, "devicePixelRatio"), manualRendering: dataValue(snapshot, "manualRendering") === true, debugCameraEnabled: dataValue(snapshot, "debugCameraEnabled") === true, pointerLockActive: dataValue(snapshot, "pointerLockActive") === true, visualProfileIdentity: dataValue(snapshot, "visualProfileIdentity"), visualScalesId: boundedString(dataValue(snapshot,"visualScalesId"),"")||null, iconAtlasReady: dataValue(snapshot, "iconAtlasReady") === true, iconAtlasError: boundedString(dataValue(snapshot, "iconAtlasError"), "") || null, tuningRequiresRegeneration: false, experimental: true, errorMessage: dataValue(snapshot, "errorMessage") }); }
...
function packageCarriesConverterProfile(packageValue, profile) {
  try {
    ...
    // 0.0.54 W1-A rebaseline: new imports carry exactly one Boxing (collider)
    // trace/chart with the profile provenance; legacy stored packages keep the
    // four-chart Lanes/Grid matrix. Mixed shapes fail closed in both directions.
    if (!Array.isArray(boxing) || (boxing.length !== 1 && boxing.length !== 4) || boxing.some((entry) => canonicalPrototypeProfileJson(dataValue(entry, "converterProfile")) !== expected)) return false;
    ...
    return (boxingCharts.length === 1 || boxingCharts.length === 4) && boxingCharts.every((chart) => canonicalPrototypeProfileJson(dataValue(dataValue(chart, "prototype"), "converterProfile")) === expected);
  } catch { return false; }
}
