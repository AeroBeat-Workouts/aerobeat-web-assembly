...
export class AeroGame extends HTMLElement {
  ...
  rendererFrame() {
    ...
    const selected = content.selectedVariant; const nowMs = Number(session.timelinePositionMs ?? 0),runSetup=this.activeSessionSetup??this.desiredGameSetup;const setup={...runSetup,guidanceBandMode:this.desiredGameSetup.guidanceBandMode};
  ...
  installGameSetupControls() {
    ...
    const note=document.createElement("p");note.className="game-setup-note";note.textContent="Timing and collider changes apply on next Start/Test. Scale and guidance changes apply live.";section.append(note);content.prepend(section);
  }
