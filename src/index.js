...
export class AeroGame extends HTMLElement {
  ...
  syncDebugCameraPresentation() {
    ...
    this.renderEnvironmentControls(); this.renderBeatBounceControls(); if(!this.beatBounceAuthoringSnapshot().enabled){this.beatBouncePickerRequest=null;this.beatBounceInput().value="";} this.syncDebugCameraControlState(snapshot);
  }
  ...
  openBeatBouncePicker(event){if(!event?.isTrusted||!this.graph||!this.beatBounceAuthoringSnapshot().enabled)return;const input=this.beatBounceInput();input.value="";this.beatBouncePickerRequest=Object.freeze({connectionGeneration:this.connectedGeneration,sessionGeneration:this.sessionGeneration,graph:this.graph,purpose:"visual_test",lifecycle:"connected"});this.beatBounceStatus="";this.renderBeatBounceStatus();input.click();}
  async handleBeatBounceFile(event){const input=event.currentTarget;if(!(input instanceof HTMLInputElement))return;if(!event.isTrusted){input.value="";this.beatBouncePickerRequest=null;return;}const file=input.files?.[0]??null,request=this.beatBouncePickerRequest;input.value="";this.beatBouncePickerRequest=null;if(!file||!request||!this.isBeatBounceRequestCurrent(request))return;try{if(file.size>maximumBeatBounceConfigBytes)throw new Error("invalid");const bytes=await file.arrayBuffer();if(bytes.byteLength>maximumBeatBounceConfigBytes||!this.isBeatBounceRequestCurrent(request))throw new Error("invalid");const value=JSON.parse(new TextDecoder("utf-8",{fatal:true}).decode(bytes)),config=normalizeBeatBounceConfig(value);if(!this.isBeatBounceRequestCurrent(request))return;this.commitBeatBounceConfig(config,"Bounce config loaded.");}catch{if(this.isBeatBounceRequestCurrent(request))this.setBeatBounceStatus("Bounce config is invalid.",true);}}
  isBeatBounceRequestCurrent(request){const state=this.beatBounceAuthoringSnapshot();return request?.connectionGeneration===this.connectedGeneration&&request?.sessionGeneration===this.sessionGeneration&&request?.graph===this.graph&&request?.purpose==="visual_test"&&request?.lifecycle===this.lifecycle&&state.enabled;}
