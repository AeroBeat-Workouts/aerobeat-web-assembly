 ...
 * @param {Readonly<{left: Readonly<{orientation:Readonly<{x:number,y:number,z:number,w:number}>}>,right: Readonly<{orientation:Readonly<{x:number,y:number,z:number,w:number}>}>}> | null} [flowQuaternionTargets]
 * @param {unknown} [equipmentConfig]
 * @param {unknown} [configIdentity]
 */
export function gameplayEquipmentRecords(menuOpen, session, input, mode, boxingStateOrientations = null, flowQuaternionTargets = null, equipmentConfig = equipmentConfigDefaults, configIdentity = null) {
  ...
  return Object.freeze(["left_wrist", "right_wrist"].flatMap((role) => {
    ...
    let orientation;
    if (mode === "flow") {
      const target = flowQuaternionTargets?.[hand]?.orientation;
