...
export class AeroGameMediaLeaseCoordinator {
  ...
  async request(participant, callbackContext) {
    ...
    return this.enqueue(async () => {
      ...
      const token = ++this.generation; const previous = this.owner ?? this.candidate; let previousPaused = false; let activationAttempted = false;
  ...
  async release(participant, callbackContext) {
    ...
    return this.enqueue(async () => {
      ...
      const token = ++this.generation; const wasOwner = this.owner === registered;
      this.owner = null; this.candidate = registered; this.transferring = true;
  ...
  async invoke(participant, callback) { const context = Object.freeze({ participantId: participant.instanceId, token: Symbol(participant.instanceId) }); this.callbackParticipant = participant; this.callbackContext = context; try { await callback(context); } finally { if (this.callbackContext === context) { this.callbackParticipant = null; this.callbackContext = null; } } }
}
