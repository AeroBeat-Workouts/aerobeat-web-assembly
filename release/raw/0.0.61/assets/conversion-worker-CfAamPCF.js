(function() {
	...
	*/
	async function normalizeConverterProfile(value) {
		...
		if (!exactKeys$1(record.settings, ["guardRelocationRadius", "reachAllowanceSubcells"])) throw profileError("converter_profile_settings_invalid", "Converter profile settings must contain the exact supported fields");
	...
	*/
	...
	function normalizeV2(map) {
		...
		for (const entry of notes) {
			...
			const type = integer$1(entry._type ?? entry.type, -1);
			const x = integer$1(entry._lineIndex ?? entry.lineIndex, 0);
			const y = integer$1(entry._lineLayer ?? entry.lineLayer, 0);
			if (type === 0 || type === 1) colorNotes.push(noteRecord(colorIndex++, number(entry._time ?? entry.b, 0), x, y, type, integer$1(entry._cutDirection ?? entry.cutDirection, 8), 0, false));
		...
		for (let sourceIndex = 0; sourceIndex < rawV2Obstacles.length; sourceIndex += 1) {
			...
			if (!isPlainRecord(entry)) continue;
			if (integer$1(entry._type ?? entry.type, -1) === 1) continue;
		...
		return {
			...
			sliders: array(map._sliders ?? map.sliders).flatMap((entry) => isPlainRecord(entry) ? [{
				...
				cell: cellFromXY(integer$1(entry._headLineIndex ?? entry.x, 0), integer$1(entry._headLineLayer ?? entry.y, 0)),
				tailCell: cellFromXY(integer$1(entry._tailLineIndex ?? entry.tx, 0), integer$1(entry._tailLineLayer ?? entry.ty, 0)),
				hand: handFromColor(integer$1(entry._colorType ?? entry.c, 0)),
				direction: integer$1(entry._headCutDirection ?? entry.d, 8),
				tailDirection: integer$1(entry._tailCutDirection ?? entry.tc ?? entry._headCutDirection ?? entry.d, 8),
				...
				midAnchorMode: integer$1(entry._sliderMidAnchorMode ?? entry.m, 0)
			}] : []),
			burstSliders: []
		};
	}
	...
	function normalizeV3(map) {
		return {
			colorNotes: array(map.colorNotes).flatMap((entry, sourceIndex) => {
				...
				const x = integer$1(entry.x, 0);
				const y = integer$1(entry.y, 0);
				const color = integer$1(entry.c, 0);
				return [noteRecord(sourceIndex, number(entry.b, 0), x, y, color, integer$1(entry.d, 8), number(entry.a, 0), Object.hasOwn(entry, "a"))];
			}),
			bombNotes: array(map.bombNotes).flatMap((entry) => isPlainRecord(entry) ? [{
				start: number(entry.b, 0),
				x: integer$1(entry.x, 0),
				y: integer$1(entry.y, 0),
				cell: cellFromXY(integer$1(entry.x, 0), integer$1(entry.y, 0))
			}] : []),
			...
			sliders: array(map.sliders).flatMap((entry) => isPlainRecord(entry) ? [{
				...
				cell: cellFromXY(integer$1(entry.x, 0), integer$1(entry.y, 0)),
				tailCell: cellFromXY(integer$1(entry.tx, 0), integer$1(entry.ty, 0)),
				hand: handFromColor(integer$1(entry.c, 0)),
				direction: integer$1(entry.d, 8),
				tailDirection: integer$1(entry.tc ?? entry.d, 8),
				headCurveMultiplier: number(entry.mu, 1),
				tailCurveMultiplier: number(entry.tmu, 1),
				midAnchorMode: integer$1(entry.m, 0)
			}] : []),
			burstSliders: array(map.burstSliders).flatMap((entry) => {
				...
				const result = {
					...
					hand: handFromColor(integer$1(entry.c, 0)),
					direction: integer$1(entry.d, 8),
					sliceCount: Math.max(integer$1(entry.sc, 1), 1)
				};
	...
	function normalizeV4(map) {
		if ([
			...
			"njsEventData",
			"_njsEvents"
		].some((key) => Object.hasOwn(map, key) && Array.isArray(map[key]) && map[key].length > 0)) throw new AuthoringParseError("relative_njs_events_unsupported", "Beat Saber v4.1 relative NJS events are unsupported");
		const noteData = records(map.colorNotesData);
		const colorNotes = array(map.colorNotes).flatMap((entry, sourceIndex) => {
			if (!isPlainRecord(entry)) return [];
			const metadata = metadataAt(noteData, integer$1(entry.i, -1));
		...
		const bombNotes = array(map.bombNotes).flatMap((entry) => {
			if (!isPlainRecord(entry)) return [];
			const metadata = metadataAt(bombData, integer$1(entry.i, -1));
			const x = intField(entry, metadata, "x", 0);
			const y = intField(entry, metadata, "y", 0);
			return [{
				start: number(entry.b, 0),
		...
		const sliders = array(map.arcs).flatMap((entry) => {
			if (!isPlainRecord(entry)) return [];
			const head = metadataAt(noteData, integer$1(entry.hi, -1));
			const tail = metadataAt(noteData, integer$1(entry.ti, -1));
			const metadata = metadataAt(arcData, integer$1(entry.ai, -1));
			return [{
		...
		return {
			...
			burstSliders: array(map.chains).flatMap((entry) => {
				if (!isPlainRecord(entry)) return [];
				const head = metadataAt(noteData, integer$1(entry.i, -1));
				const metadata = metadataAt(chainData, integer$1(entry.ci, -1));
				const result = {
					start: number(entry.hb, 0),
					end: number(entry.tb ?? entry.hb, 0),
					cell: cellFromXY(intField(head, {}, "x", 0), intField(head, {}, "y", 0)),
	...
	function normalizeV2Obstacle(value, sourceIndex) {
		...
		return obstacleRecord(requiredFinite(value, ["_time", "b"], "obstacle_time_invalid"), requiredFinite(value, ["_duration", "d"], "obstacle_duration_invalid"), requiredInteger(value, ["_lineIndex", "x"], "obstacle_geometry_invalid"), type === 1 ? 2 : 0, width, type === 1 ? 3 : 5, sourceIndex, type === 1 ? "v2_type_1" : "v2_type_0");
	}
	/** @param {unknown} value @param {number} sourceIndex */
	...
	function normalizeIndexedObstacle(value, obstacleData, sourceIndex) {
		...
		return obstacleRecord(requiredFinite(value, ["b"], "obstacle_time_invalid"), requiredFinite(metadata, ["d"], "obstacle_duration_invalid"), optionalDefaultInteger(metadata, ["x"], 0, "obstacle_geometry_invalid"), optionalDefaultInteger(metadata, ["y"], 0, "obstacle_geometry_invalid"), requiredInteger(metadata, ["w"], "obstacle_geometry_invalid"), requiredInteger(metadata, ["h"], "obstacle_geometry_invalid"), sourceIndex, "v4_rect");
	}
	...
	function obstacleRecord(start, duration, x, y, width, height, sourceIndex, kind) {
		...
		const sourceGeometry = {
			schema: "aerobeat/obstacle_source_geometry",
			version: 1,
			coordinateSpace: kind.startsWith("v2_") ? "beatsaber_v2_legacy_obstacle" : kind === "v3_rect" ? "beatsaber_v3_obstacle_rect" : "beatsaber_v4_obstacle_rect",
			kind,
			x,
			y,
			width,
	...
	function intField(primary, fallback, key, defaultValue) {
		return integer$1(Object.hasOwn(primary, key) ? primary[key] : fallback[key], defaultValue);
	}
	/** @param {Record<string, unknown>} primary @param {Record<string, unknown>} fallback @param {string} key @param {number} defaultValue */
	...
	/** @param {unknown} value @param {number} fallback */
	function integer$1(value, fallback) {
	...
	*/
	...
	//#endregion
	//#region ../aerobeat-web-contracts/src/obstacle-contracts.js
	/**
	...
	*/
	...
	function isObstacleSourceGeometry(value) {
	...
	function isObstacleGameplayGeometry(value) {
		return hasExactKeys(value, [
			...
			"width",
			"height"
		]) && value.schema === "aerobeat/obstacle_gameplay_geometry" && value.version === 1 && value.coordinateSpace === "aerobeat_top_left_grid" && integer(value.x, 0, 3) && integer(value.y, 0, 2) && integer(value.width, 1, 4) && integer(value.height, 1, 3) && Number(value.x) + Number(value.width) <= 4 && Number(value.y) + Number(value.height) <= 3;
	}
	/** @param {AeroObstacleGameplayGeometry} geometry @returns {readonly number[]} */
	function deriveObstacleGridMask(geometry) {
		if (!isObstacleGameplayGeometry(geometry)) throw new TypeError("obstacle_gameplay_geometry_invalid");
		const cells = [];
		for (let row = geometry.y; row < geometry.y + geometry.height; row += 1) for (let column = geometry.x; column < geometry.x + geometry.width; column += 1) cells.push(row * 4 + column);
		return Object.freeze(cells);
	}
	/** @param {unknown} value @param {AeroObstacleGameplayGeometry} geometry @returns {value is readonly number[]} */
	function isObstacleGridMask(value, geometry) {
	...
	*/
	function isCanonicalOpaqueSrgbColor(value) {
	...
	*/
	function isNotePaletteProvenance(value) {
		if (!hasExactDataObject(value, [
			"kind",
			"infoFormat",
			"infoHash",
			"difficultyHash",
		...
		if (value.kind !== "info_color_scheme" && value.kind !== "difficulty_custom_data" || value.infoFormat !== "v2" && value.infoFormat !== "v4" || typeof value.infoHash !== "string" || !sha256TokenPattern.test(value.infoHash) || typeof value.difficultyHash !== "string" || !sha256TokenPattern.test(value.difficultyHash)) return false;
		...
		return value.kind === "info_color_scheme" ? Number.isSafeInteger(value.schemeIndex) && Number(value.schemeIndex) >= 0 : value.schemeIndex === null;
	}
	/**
	* @param {unknown} value
	* @returns {value is AeroSourceNotePalette}
	*/
	function isSourceNotePalette(value) {
		return hasExactDataObject(value, [
			"schema",
			"version",
			"left",
			"right",
			...
			"alpha",
			"provenance"
		]) && value.schema === "aerobeat/source_note_palette" && value.version === 1 && isCanonicalPair(value) && isNotePaletteProvenance(value.provenance);
	}
	/**
	* @param {unknown} value
	* @returns {value is AeroAuthoredNotePalette}
	*/
	function isAuthoredNotePalette(value) {
		return hasExactDataObject(value, [
			"schema",
			"version",
			"left",
			...
			"paletteHash",
			"provenance"
		]) && value.schema === "aerobeat/authored_note_palette" && value.version === 1 && isCanonicalPair(value) && typeof value.paletteHash === "string" && sha256TokenPattern.test(value.paletteHash) && isNotePaletteProvenance(value.provenance);
	}
	...
	*/
	...
	//#endregion
	//#region ../aerobeat-web-content-authoring/src/note-palette.js
	...
	*/
	function verifySourceNotePalette(value, binding) {
		...
		if (copy.provenance.infoFormat !== binding.infoFormat || copy.provenance.infoHash !== binding.infoHash || copy.provenance.difficultyHash !== binding.difficultyHash) throw paletteError("source_palette_provenance_mismatch", "Source note palette provenance does not match the exact selected source bytes");
		return copy;
	}
	/** @param {AeroSourceNotePalette | null} sourcePalette @returns {Promise<AeroAuthoredNotePalette | null>} */
	async function createAuthoredNotePalette(sourcePalette) {
	...
	async function verifyAuthoredNotePalette(value) {
	...
	function flowPaletteReference(value) {
		return value === null ? null : {
			source: "package",
			paletteHash: value.paletteHash
		};
	}
	/** @param {string} code @param {string} message */
	...
	//#endregion
	//#region ../aerobeat-web-content-authoring/src/spawn-timing.js
	const beatSaberSpawnTimingConstants = Object.freeze({
		...
		minimumHalfJumpDurationBeats: .25
	});
	const exactKeys = Object.freeze([
		...
		"maxHalfJumpDistance",
		"startHalfJumpDurationBeats",
		"minimumHalfJumpDurationBeats",
	...
	*/
	function deriveBeatSaberSpawnTiming(bpm, noteJumpMovementSpeed, noteJumpStartBeatOffset) {
		...
		const reactionTimeMs = secondsPerBeat * halfJumpDurationBeats * 1e3;
		const jumpDistanceMeters = noteJumpMovementSpeed * (reactionTimeMs / 1e3) * 2;
		return deepFreeze({
	...
	function verifyBeatSaberSpawnTiming(value) {
		...
		return deepFreeze(cloneData(expected));
	}
	/** @param {string} code @param {string} message */
	function spawnError(code, message) {
		const error = new Error(message);
		error.name = "AeroBeatSpawnTimingError";
		Object.assign(error, { code });
		return error;
	}
	//#endregion
	//#region ../aerobeat-web-content-authoring/src/flow-contract.js
	/** Successor identities. V5 packages deliberately do not contain this binding. */
	const authoredPackageSchemaId = "aerobeat.song-package.v6";
	const authoredPackageVersion = "6.0.0";
	const flowChartSchemaId = "aerobeat.chart.flow.v5";
	const flowCollidersRulesetId = "flow_colliders_v1";
	/** Sole Flow ruleset variant: the Flow (colliders) ruleset. */
	const flowRulesetVariants = Object.freeze([flowCollidersRulesetId]);
	/**
	...
	*/
	function flowContentIdentity(beats, notePalette) {
		return {
			beats,
			rulesetId: flowCollidersRulesetId,
			rulesetVariants: [...flowRulesetVariants],
			notePalette
		};
	}
	/**
	...
	* @param {unknown} value
	*/
	function hasExactFlowRulesetVariants(value) {
		if (!Array.isArray(value)) return false;
		try {
			return canonicalJson(value) === canonicalJson(flowRulesetVariants) || canonicalJson(value) === canonicalJson(["flow_grid_v2", "flow_colliders_v1"]);
		} catch {
			return false;
		}
	}
	...
	const colliderRulesetId$1 = "boxing_collider_v1";
	...
	*/
	async function convertDifficulty(sourceSummary, options, onProgress = () => void 0) {
		...
		const notePalette = await createAuthoredNotePalette(verifySourceNotePalette(options.notePalette, {
			infoFormat: options.sourceInfoFormat,
			infoHash: options.sourceInfoHash,
			difficultyHash: sourceDifficultyHash
		}));
		const charts = [];
		const traces = [];
		const rowRecipe = recipeDefinitions.find((recipe) => String(recipe.recipeId) === "row_family_balanced_height_v1") ?? recipeDefinitions[0];
		...
		const packageRecord = {
			...
			conversionTrace: {
				notePalette: flowPaletteReference(notePalette),
				spawnTiming: cloneData(spawnTiming),
				boxing: traces,
				flow: [flow.trace],
				...converterProfile ? { converterProfile: cloneData(converterProfile) } : {}
	...
	async function colliderChartFor(generated, recipe, difficulty, songToken, sourceHash, modifiers, suggestion, converterProfile) {
		...
		const rulesetHash = await prefixedSha256(canonicalJson({
			contractId: boxingPrototypeContractId,
			rulesetId: colliderRulesetId$1,
			version: rulesetVersion
		}));
		...
		for (const window of obstacleWindows) {
			...
			if (!safeCells.length) {
				trace.push({
					...
					action: "drop",
					reason: type === "squat" ? "squat_no_reachable_safe_cell" : "weave_no_reachable_safe_cell",
					type,
					gridMask: [...window.gridMask],
					blockedCells,
					noseSafeCells: []
				});
				continue;
			...
			const emitted = {
				...
				sourceEventIds: [sourceId],
				sourceGeometry: cloneData(window.sourceGeometry),
				gameplayGeometry: cloneData(window.gameplayGeometry),
				gridMask: [...window.gridMask],
				checkpoint: {
					kind: "instantaneous",
					freshnessMs: 150,
	...
	function obstaclesFor(obstacles, bpm) {
		if (obstacles.length > 512) throw new Error("flow_obstacle_limit_exceeded");
		return obstacles.map((entry, index) => {
			...
			return {
				...
				gameplayGeometry: geometry.gameplayGeometry,
				gridMask: [...gridMask],
				blockedCells: [...gridMask],
	...
	function normalizedGeometryForObstacle(obstacle) {
		...
		return {
			sourceGeometry: cloneData(sourceGeometry),
			gameplayGeometry: cloneData(gameplayGeometry)
		};
	}
	...
	function gridMaskForObstacle(obstacle) {
		...
		return deriveObstacleGridMask(gameplayGeometry);
	}
	...
	function buildFlowChartBase(summary, difficulty, songToken) {
		const beats = [];
		const events = [];
		...
		for (const obstacle of summary.obstacles ?? []) {
			const emitted = {
				...
				type: "obstacle",
				...normalizedGeometryForObstacle(obstacle),
		...
		return {
			chart: {
				schemaId: "aerobeat.chart.flow.v4",
				schemaVersion: 4,
				recordVersion: 2,
				rulesetId: flowCollidersRulesetId,
	...
	*/
		...
		return {
			...
			charts: packageValue.charts.map((chart) => {
				...
				return {
					...
					...chart.mode === "flow" ? {
						...Object.hasOwn(chart, "rulesetVariants") ? { rulesetVariants: chart.rulesetVariants } : {},
						notePalette: chart.notePalette,
						contentHash: chart.contentHash
					} : {},
	...
	/** @param {Readonly<Record<string, unknown>>} prototype */
	const isColliderPrototype = (prototype) => prototype.rulesetId === colliderRulesetId;
	const legacyMatrixIdentities = Object.freeze([
		"row_family_balanced_height_v1|boxing_semantic_track_v1",
		"row_family_balanced_height_v1|boxing_spatial_grid_v1",
		"cut_family_source_height_v1|boxing_semantic_track_v1",
		"cut_family_source_height_v1|boxing_spatial_grid_v1"
	]);
	/**
	...
	*/
	async function validateAuthoredPackage(packageValue) {
		...
		if ([
			...
			3,
			4
		].some((version) => packageValue.schemaId === `aerobeat.song-package.v${version}` && packageValue.schemaVersion === version)) issue("spawn_timing_reimport_required", "schemaId", "Prior package versions require reimport for source spawn timing");
		else if (packageValue.schemaId === "aerobeat.song-package.v5" && packageValue.schemaVersion === 5 && packageValue.packageVersion === "5.0.0") issue("flow_colliders_reimport_required", "schemaId", "Package predates explicit flow_colliders_v1 authoring and must be reimported");
		else if (packageValue.schemaId !== "aerobeat.song-package.v6" || packageValue.schemaVersion !== 6 || packageValue.packageVersion !== "6.0.0") issue("package_schema_invalid", "schemaId", `Package schema must be ${authoredPackageSchemaId} version 6 / package ${authoredPackageVersion}`);
		...
		if (!isPlainRecord(packageValue.source) || packageValue.source.obstacleContract !== "normalized_obstacle_v2") issue("obstacle_contract_invalid", "source.obstacleContract", "Package source must bind normalized_obstacle_v2");
		const notePalette = packageValue.notePalette;
		if (notePalette !== null && (!isAuthoredNotePalette(notePalette) || !await verifyAuthoredNotePalette(notePalette))) issue("note_palette_invalid", "notePalette", "Package notePalette must be null or an independently hash-verified authored song palette");
		let spawnTiming = null;
		if (isPlainRecord(packageValue.source)) {
			try {
				spawnTiming = verifyBeatSaberSpawnTiming(packageValue.source.spawnTiming);
			} catch (cause) {
				issue(errorCode$1(cause), "source.spawnTiming", cause instanceof Error ? cause.message : "Spawn timing is invalid");
			}
			const infoFormatValid = ["v2", "v4"].includes(String(packageValue.source.sourceInfoFormat));
			const infoVersionValid = packageValue.source.sourceInfoVersion === null || typeof packageValue.source.sourceInfoVersion === "string" && /^\d+\.\d+\.\d+$/u.test(packageValue.source.sourceInfoVersion);
			const beatmapFormatValid = [
				"v2",
				"v3",
				"v4"
			].includes(String(packageValue.source.sourceBeatmapFormat));
			const beatmapVersionValid = packageValue.source.sourceBeatmapVersion === null || typeof packageValue.source.sourceBeatmapVersion === "string" && /^\d+\.\d+\.\d+$/u.test(packageValue.source.sourceBeatmapVersion);
			const formatsCompatible = packageValue.source.sourceInfoFormat === "v2" && (packageValue.source.sourceBeatmapFormat === "v2" || packageValue.source.sourceBeatmapFormat === "v3") || packageValue.source.sourceInfoFormat === "v4" && packageValue.source.sourceBeatmapFormat === "v4";
			if (!infoFormatValid || !infoVersionValid || !validHash$1(packageValue.source.sourceInfoHash) || !beatmapFormatValid || !beatmapVersionValid || !validHash$1(packageValue.source.sourceDifficultyHash) || !formatsCompatible) issue("source_format_provenance_invalid", "source", "Package source must retain compatible exact Info and beatmap format/hash provenance");
			if (isAuthoredNotePalette(notePalette) && (notePalette.provenance.infoFormat !== packageValue.source.sourceInfoFormat || notePalette.provenance.infoHash !== packageValue.source.sourceInfoHash || notePalette.provenance.difficultyHash !== packageValue.source.sourceDifficultyHash)) issue("note_palette_provenance_mismatch", "notePalette.provenance", "Package palette must bind exact source Info and difficulty hashes");
		}
		...
		if (![1, 4].includes(boxingTraces.length)) issue("boxing_trace_count_invalid", "conversionTrace.boxing", "Conversion trace must contain one collider or four legacy Boxing traces");
		if (flowTraces.length !== 1) issue("flow_trace_count_invalid", "conversionTrace.flow", "Conversion trace must contain exactly one Flow trace");
		if (!spawnTiming || !conversionTrace || !canonicalEqual(conversionTrace.spawnTiming, spawnTiming)) issue("spawn_timing_trace_mismatch", "conversionTrace.spawnTiming", "Conversion trace must bind exact source spawn timing");
		const songTiming = isPlainRecord(packageValue.song) && isPlainRecord(packageValue.song.timing) && Array.isArray(packageValue.song.timing.tempoSegments) && isPlainRecord(packageValue.song.timing.tempoSegments[0]) ? packageValue.song.timing.tempoSegments[0] : null;
		if (spawnTiming && (!songTiming || songTiming.bpm !== spawnTiming.bpm)) issue("spawn_timing_bpm_mismatch", "song.timing", "Spawn timing BPM must match authored song timing");
		const expectedPaletteReference = flowPaletteReference(isAuthoredNotePalette(notePalette) ? notePalette : null);
		if (!conversionTrace || !canonicalEqual(conversionTrace.notePalette, expectedPaletteReference)) issue("note_palette_trace_mismatch", "conversionTrace.notePalette", "Top conversion trace must bind the package note palette reference");
		for (let index = 0; index < boxingTraces.length; index += 1) {
			const trace = boxingTraces[index], source = isPlainRecord(packageValue.source) ? packageValue.source : null;
			const traceConverterProfile = isPlainRecord(trace) ? trace.converterProfile : void 0;
			if (isPlainRecord(trace) && (Object.hasOwn(trace, "notePalette") || Object.hasOwn(trace, "paletteHash"))) issue("boxing_palette_trace_forbidden", `conversionTrace.boxing[${index}]`, "Palette presentation data is forbidden from Boxing traces");
			if (!isPlainRecord(trace) || !source || trace.sourceHash !== source.sourceHash || trace.sourceInfoFormat !== source.sourceInfoFormat || trace.sourceInfoVersion !== source.sourceInfoVersion || trace.sourceInfoHash !== source.sourceInfoHash || trace.sourceDifficultyPath !== source.sourceDifficultyPath || trace.sourceBeatmapFormat !== source.sourceBeatmapFormat || trace.sourceBeatmapVersion !== source.sourceBeatmapVersion || trace.sourceDifficultyHash !== source.sourceDifficultyHash || !canonicalEqual(trace.spawnTiming, source.spawnTiming)) issue("boxing_trace_invalid", `conversionTrace.boxing[${index}]`, "Every Boxing trace must bind exact package source provenance");
		...
		for (let index = 0; index < flowTraces.length; index += 1) {
			const trace = flowTraces[index];
			if (isPlainRecord(trace) && trace.converterProfile !== void 0) issue("converter_profile_flow_trace_forbidden", `conversionTrace.flow[${index}].converterProfile`, "Flow traces must not carry Boxing converter profile provenance");
			const legacyFlowTraceAccepted = isPlainRecord(trace) && trace.rulesetId === "flow_grid_v2" && Array.isArray(trace.rulesetVariants) && trace.rulesetVariants.length === 2 && trace.rulesetVariants[0] === "flow_grid_v2" && trace.rulesetVariants[1] === "flow_colliders_v1";
			if (!isPlainRecord(trace) || trace.obstacleContract !== "normalized_obstacle_v2" || !legacyFlowTraceAccepted && trace.rulesetId !== "flow_colliders_v1" || !hasExactFlowRulesetVariants(trace.rulesetVariants) || !isPlainRecord(packageValue.source) || trace.sourceHash !== packageValue.source.sourceHash || trace.sourceInfoFormat !== packageValue.source.sourceInfoFormat || trace.sourceInfoVersion !== packageValue.source.sourceInfoVersion || trace.sourceInfoHash !== packageValue.source.sourceInfoHash || trace.sourceDifficultyPath !== packageValue.source.sourceDifficultyPath || trace.sourceBeatmapFormat !== packageValue.source.sourceBeatmapFormat || trace.sourceBeatmapVersion !== packageValue.source.sourceBeatmapVersion || trace.sourceDifficultyHash !== packageValue.source.sourceDifficultyHash || !canonicalEqual(trace.spawnTiming, packageValue.source.spawnTiming) || !canonicalEqual(trace.notePalette, expectedPaletteReference)) issue("flow_trace_invalid", `conversionTrace.flow[${index}]`, "Flow trace must bind the sole Flow (colliders) ruleset variant, normalized obstacles, source provenance and palette reference");
		}
		const charts = Array.isArray(packageValue.charts) ? packageValue.charts : [];
		if (charts.length !== 2 && charts.length !== 5) issue("chart_count_invalid", "charts", "One difficulty must contain Flow plus one collider or four legacy Boxing charts");
		...
		for (let index = 0; index < charts.length; index += 1) {
			...
			if (chart.mode === "flow") {
				flowCount += 1;
				const legacyFlowGridAccepted = chart.rulesetId === "flow_grid_v2" && Array.isArray(chart.rulesetVariants) && chart.rulesetVariants.length === 2 && chart.rulesetVariants[0] === "flow_grid_v2" && chart.rulesetVariants[1] === "flow_colliders_v1";
				if (chart.schemaId !== "aerobeat.chart.flow.v5" || chart.schemaVersion !== 5 || chart.rulesetId !== "flow_colliders_v1" && !legacyFlowGridAccepted || !hasExactFlowRulesetVariants(chart.rulesetVariants)) issue("flow_chart_schema_invalid", path, "Flow chart must bind the sole Flow (colliders) ruleset variant; historical two-variant Flow Grid binds require reimport");
				if (!canonicalEqual(chart.notePalette, expectedPaletteReference)) issue("flow_note_palette_mismatch", `${path}.notePalette`, "Flow chart must reference the package note palette exactly");
				const flowContentBindingMatches = chart.rulesetId === "flow_colliders_v1" && canonicalEqual(chart.rulesetVariants, flowRulesetVariants);
				const expectedFlowContentHash = await prefixedSha256(canonicalJson(flowContentIdentity(chart.beats, expectedPaletteReference)));
				if (flowContentBindingMatches && chart.contentHash !== expectedFlowContentHash) issue("flow_content_hash_mismatch", `${path}.contentHash`, "Flow content hash must bind beats, exact ruleset variants and palette reference");
				const flowTrace = flowTraces[0];
				if (isPlainRecord(flowTrace) && traceBindingMatchesCurrent(flowTrace) && flowTrace.contentHash !== expectedFlowContentHash) issue("flow_trace_content_hash_mismatch", "conversionTrace.flow[0].contentHash", "Flow trace content hash must bind the current Flow identity exactly");
				if (chart.beats.filter((beat) => isPlainRecord(beat) && beat.type === "obstacle").length > 512) issue("flow_obstacle_limit_exceeded", `${path}.beats`, `Flow chart exceeds the obstacle limit`);
			...
			if (Object.hasOwn(chart, "notePalette") || Object.hasOwn(chart, "paletteHash") || chart.beats.some((beat) => isPlainRecord(beat) && (Object.hasOwn(beat, "notePalette") || Object.hasOwn(beat, "paletteHash") || Object.hasOwn(beat, "appearanceColor")))) issue("boxing_palette_forbidden", path, "Palette presentation data is forbidden from Boxing charts and beats");
			const prototype = chart.prototype;
			const collider = isColliderPrototype(prototype);
			...
			if (collider) {
				if (Object.hasOwn(prototype, "recipeId")) issue("prototype_recipe_invalid", `${path}.prototype.recipeId`, "Collider chart prototype must carry no conversion recipe identity");
			} else {
			...
			if (converterProfile) try {
				...
				const expectedContentHash = await prefixedSha256(canonicalJson(collider ? {
					beats: chart.beats,
			...
			if (!converterProfile) {
				...
				if (prototype.contentHash !== expectedContentHash) issue("prototype_content_hash_mismatch", `${path}.prototype.contentHash`, "Chart content hash must bind the exact authored beat set and ruleset identity");
			}
		...
		const colliderChartCount = charts.filter((chart) => isPlainRecord(chart) && chart.mode === "boxing" && isPlainRecord(chart.prototype) && isColliderPrototype(chart.prototype)).length;
		if (flowCount !== 1 || !(colliderChartCount === 1 && boxingTraces.length === 1 && matrix.size === 0 || colliderChartCount === 0 && boxingTraces.length === 4 && legacyMatrixIdentities.every((identity) => matrix.has(identity)))) issue("prototype_matrix_invalid", "charts", "Charts must be either one Flow plus the sole collider Boxing chart, or one Flow plus all four legacy recipe/ruleset combinations");
	...
	function validateBeat(beat, path, issue) {
		...
		if (/^(squat|weave_)/u.test(String(beat.type))) {
			const geometryValid = Number.isFinite(beat.end) && Number(beat.end) > Number(beat.start) && Number(beat.end) <= 144e3 && isObstacleSourceGeometry(beat.sourceGeometry) && isObstacleGameplayGeometry(beat.gameplayGeometry) && isObstacleGridMask(beat.gridMask, beat.gameplayGeometry);
			...
			if (!isPlainRecord(beat.checkpoint) || beat.checkpoint.kind !== "instantaneous" || beat.checkpoint.timingWindowMs !== 180 || beat.checkpoint.freshnessMs !== 150 || canonicalJson(beat.checkpoint.noseSafeCells) !== canonicalJson(expectedSafeCells)) issue("obstacle_checkpoint_invalid", `${path}.checkpoint`, "Avoidance checkpoint requires frozen timing and the exact normalized safe-cell complement");
		}
	}
	/** @param {unknown} beat @param {string} path @param {(code: string, path: string, message: string) => void} issue */
	function validateFlowBeat(beat, path, issue) {
		if (isPlainRecord(beat) && (Object.hasOwn(beat, "notePalette") || Object.hasOwn(beat, "paletteHash") || Object.hasOwn(beat, "appearanceColor"))) issue("flow_beat_palette_forbidden", path, "Palette presentation data is forbidden from Flow beats");
		...
		if (String(beat.type) === "obstacle") {
			...
			if (Reflect.ownKeys(beat).length !== keys.length || !keys.every((key) => Object.hasOwn(beat, key)) || !Number.isFinite(beat.end) || Number(beat.end) <= Number(beat.start) || Number(beat.end) > 144e3 || !isObstacleSourceGeometry(beat.sourceGeometry) || !isObstacleGameplayGeometry(beat.gameplayGeometry) || !isObstacleGridMask(beat.gridMask, beat.gameplayGeometry)) issue("flow_obstacle_invalid", path, "Obstacle source evidence, normalized gameplay geometry, interval, and derived grid mask must match exactly");
		}
	...
	function obstacleActionForCells(cells) {
		let left = 0, right = 0;
		for (const cell of cells) cell % 4 <= 1 ? left += 1 : right += 1;
		return left > right ? "weave_right" : right > left ? "weave_left" : "squat";
	}
	...
	/** @param {Readonly<Record<string, unknown>> | undefined} trace */
	function traceBindingMatchesCurrent(trace) {
		return isPlainRecord(trace) && trace.rulesetId === "flow_colliders_v1" && canonicalEqual(trace.rulesetVariants, flowRulesetVariants);
	}
	/** @param {unknown} value */
	...
	*/
	...
	function narrowRequest(request) {
		...
		if (record.schema !== "aerobeat/authoring_worker_request" || record.version !== 2 || record.kind !== "convert" || !boundedString(record.jobId, 128) || !record.jobId || !(record.difficultyBytes instanceof Uint8Array) || record.difficultyBytes.byteLength > 67108864) throw workerError("worker_request_invalid", "Worker request shape is invalid");
		...
		if (!boundedString(selected.difficulty, 64) || !selected.difficulty || !boundedString(selected.path, 1024) || !selected.path || typeof selected.beatMapFormat !== "string" || ![
			"v2",
			"v3",
			"v4"
		].includes(selected.beatMapFormat) || selected.beatMapVersion !== null && !semanticVersion(selected.beatMapVersion) || !validHash(selected.contentHash) || manifest.infoFormat === "v2" && selected.beatMapFormat === "v4" || manifest.infoFormat === "v4" && selected.beatMapFormat !== "v4") throw workerError("worker_request_invalid", "Worker selected difficulty values are invalid");
		let selectedPalette;
		try {
			selectedPalette = verifySourceNotePalette(selected.notePalette, {
				infoFormat: manifest.infoFormat,
				infoHash: manifest.infoHash,
				difficultyHash: selected.contentHash
			});
		} catch {
			throw workerError("worker_request_invalid", "Worker source palette provenance is invalid");
		}
		let selectedSpawnTiming;
		try {
			selectedSpawnTiming = verifyBeatSaberSpawnTiming(selected.spawnTiming);
		} catch {
			throw workerError("worker_request_invalid", "Worker source spawn timing is invalid");
		}
		...
		let optionSpawnTiming;
		try {
			optionSpawnTiming = verifyBeatSaberSpawnTiming(conversionOptions.spawnTiming);
		} catch {
			throw workerError("worker_request_invalid", "Worker option spawn timing is invalid");
		}
		...
		let optionPalette;
		try {
			optionPalette = verifySourceNotePalette(conversionOptions.notePalette, {
				infoFormat: manifest.infoFormat,
				infoHash: manifest.infoHash,
				difficultyHash: selected.contentHash
			});
		} catch {
			throw workerError("worker_request_invalid", "Worker option palette provenance is invalid");
		}
		...
		const format = selected.beatMapFormat;
		return {
			jobId: record.jobId,
...
//# sourceMappingURL=conversion-worker-CfAamPCF.js.map