(function() {
	...
	*/
	...
	async function convertFlowChart(summary, difficulty, songToken, notePalette) {
		...
		chart.contentHash = await prefixedSha256(canonicalJson({
			beats: chart.beats,
	...
	*/
	async function validateAuthoredPackage(packageValue) {
		...
		else if (packageValue.schemaId !== "aerobeat.song-package.v5" || packageValue.schemaVersion !== 5 || packageValue.packageVersion !== "5.0.0") issue("package_schema_invalid", "schemaId", "Package schema must be aerobeat.song-package.v5 version 5 / package 5.0.0");
		...
		for (let index = 0; index < charts.length; index += 1) {
			...
			if (chart.mode === "flow") {
				flowCount += 1;
				if (chart.schemaId !== "aerobeat.chart.flow.v4" || chart.schemaVersion !== 4 || chart.rulesetId !== "flow_grid_v2") issue("flow_chart_schema_invalid", path, "Flow chart must use palette-bound normalized obstacle schema/ruleset v4");
				...
				const expectedFlowContentHash = await prefixedSha256(canonicalJson({
					beats: chart.beats,
					rulesetId: chart.rulesetId,
					notePalette: expectedPaletteReference
				}));
				if (chart.contentHash !== expectedFlowContentHash) issue("flow_content_hash_mismatch", `${path}.contentHash`, "Flow content hash must bind beats, ruleset and palette reference");
...
//# sourceMappingURL=conversion-worker-CKy18nBK.js.map