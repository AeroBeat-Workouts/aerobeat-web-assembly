(function() {
	...
	*/
	...
	const flowGridRulesetId = "flow_grid_v2";
	const flowRulesetVariants = Object.freeze([flowGridRulesetId, "flow_colliders_v1"]);
	/**
	...
	*/
	async function validateAuthoredPackage(packageValue) {
		...
		for (let index = 0; index < charts.length; index += 1) {
			...
			if (chart.mode === "flow") {
				flowCount += 1;
				if (chart.schemaId !== "aerobeat.chart.flow.v5" || chart.schemaVersion !== 5 || chart.rulesetId !== "flow_grid_v2" || !hasExactFlowRulesetVariants(chart.rulesetVariants)) issue("flow_chart_schema_invalid", path, "Flow chart must explicitly bind flow_grid_v2 and flow_colliders_v1 in canonical order");
				...
				if (chart.contentHash !== expectedFlowContentHash) issue("flow_content_hash_mismatch", `${path}.contentHash`, "Flow content hash must bind beats, exact ruleset variants and palette reference");
				const flowTrace = flowTraces[0];
				if (isPlainRecord(flowTrace) && flowTrace.contentHash !== chart.contentHash) issue("flow_trace_content_hash_mismatch", "conversionTrace.flow[0].contentHash", "Flow trace must bind exact Flow content hash");
...
//# sourceMappingURL=conversion-worker-BiXEbRAM.js.map