...
- PASS. A new real-Chromium probe against `99c9035` reproduced both unguarded callbacks exactly: hostile collection `deleteCollection` rejected `AeroAuthoringStorageError / indexeddb_transaction_aborted / Uncaught exception in event handler.`, package `delete` rejected `AeroAuthoringStorageError / indexeddb_transaction_failed / Uncaught exception in event handler.`, two `pageerror: Stored collection shape is invalid` events escaped, and the stores remained atomic.
- Current `90d9117` independently passed fake-IDB and Chromium matrices across all 10 valid and 18 hostile public operations. Package, collection, and shared-asset errors retained exact `storage_record_invalid` messages; deletion by key/token retained intentional no-decode semantics; nested collection removal and GC/resolve validation rolled back package+collection+asset stores; each public promise settled once within 1,000/2,000 ms; browser pageerror/unhandledrejection/warning/error capture was empty.
- Decoder-first/cancellation-first/pre-aborted/no-signal/repeated/reentrant/post-completion and service persistence cancel/replace/destroy also passed with first-cause retention, one effective abort where observable, terminal listener cleanup, and exact rollback/commit boundaries. A separate fake-IDB and Chromium completion-boundary injection delivered cancellation after native commit but immediately before the product `oncomplete` callback; both fulfilled once, retained exact committed data, tolerated the one inactive abort attempt without noise, removed the listener, and ignored later delivery. Full authoring and sequential assembly gates, immutable raw policy, pin/tree/fingerprint, repository alignment, port/tag/raw absence, and dry packs passed as detailed in the active plan.
