...
Remaining uncertainty: Derrick's final physical tuning review and the separate jbhb audit; no implementation-path uncertainty remains.
```

## Independent QA — PASS (`q25l`, 2026-09-08)

Independent QA verified exact pushed source identities: content `cce1ee215d428358863798c00459054c54898ce4` / tree `cd3808b595397dcc52deaed612fdba1a30a9a74d`, renderer `cbf91252d9e1b6b76d3b4dad558e5235338b9b63` / tree `43bd4ab8f464ac1d795366c59c6c5e79b0412ee0`, and assembly `b050e9c628fffa1377a7cf2437350f2a71e89d95` / tree `89f5fdab42fc8ed1244ca263c1f89b6768bf1eb9`; each exact commit/tree equals `origin/main`.

A one-off read-only Playwright oracle loaded the already-running immutable raw `0.0.44` at `127.0.0.1:5173` without rebuilding or changing its server. It passed exact `3c9d` Hard bytes (`89,424`; SHA-256 `4db5b3393a389c7bcaba6d7a02aec57c10801bcfd74de91523b8e9cdad859b55`) through the raw app's actual module Worker, IndexedDB write plus package reload, downloaded collection selection, trusted Test click, transport seeks, and actual Test input/change controls. It independently reproduced null-prototype timing and segment records, all three trajectory timestamps `null`, `timingMismatchCount 0`, and byte-equivalent default/extreme model samples: absent at `5899`, then `(1.5,0,-15)` at `5900`, `(1.5,0,-9.6)` at `6800`, `(1.5,0,-4.8)` at `7600`, and `(1.5,0,0)` at `8400`. The served proof remained SHA-256 `26ce02f790178d6604a651f13239b29e428bfb3926523671e96e0676ca3bc01e` and the listener remained the pre-existing loopback-only process.

The repaired `npm run test:exact-3c9d-browser` and complete `npm run test:browser` both passed. The real matrix covered direct and genuine cross-origin iframe at desktop `844×390` and mobile `390×844`; every row used Worker conversion → IndexedDB reload → downloaded selection → trusted Test → input/change. Default beat-50 timestamps were exactly `[18400,17500,17500]`; extremes were `[16800,8000,0]`. Mapper-unavailable and mismatch counts were zero, null-prototype source records remained intact, current event/index identity was exact, stale generation returned `null`, reconnect created a fresh service/current mapper, and no console/page noise occurred. Sky start, continuous normal join, `Z=-72` normal spawn, bounce apex, exact landing `(0.5,0,0)`, materially different framebuffers, real Lanes separation `|X|=2` at configured separation `4`, and bombs/walls/checkpoints exclusions all passed. The renderer also retained explicit trajectory visibility before the generic ten-second cull.

Source/unit inspection confirmed the mapper symbol property is non-enumerable, non-writable, non-configurable, current-ready-generation-only, absent before readiness/loading/after destroy, and absent from public exports, snapshots, JSON, events, iframe messages, persistence and telemetry. Missing mapper and center mismatch produce distinct truthful private counters/status messages while retaining only the configured straight approach and emitting no partial trajectory fields. Content's public authored-timing validator still rejects the canonical null-prototype record; only the already-validated private mapper crosses the seam.

Fresh bounded gates passed: content `npm test`, `npm run test:browser`, dry pack (`9` files, `27,837` packed bytes); renderer `npm test`, `npm run test:browser`, dry pack (`40` files, `158,844` packed bytes); assembly `npm test`, complete `npm run test:browser`, `npm run build` (`1,319` modules), and dry pack (`124` files, `16,308,675` packed bytes). Assembly gates included all `19` immutable raw snapshots, raw mutation rejection, release-target policy, environment/gameplay asset verification, dependency/hash provenance fingerprint `5fd9a166ff01826106390211928a4cd46d676f08b23ced5d1e4bf8bd116299ea`, exact `6,003`-event projection parity, pause/seek/resume/restart/terminal/reconnect matrices, direct/iframe portrait/landscape DPR rows, framebuffer/model movement, privacy and exclusions. One first assembly `npm test` attempt correctly rejected the temporary QA oracle file as escaping the exact pack roots; QA removed that self-created temporary file and the unchanged canonical command then passed completely.

**Disposition:** independent QA PASS. No source fix, version, release build, serve change, raw mutation, asset mutation, or release authorization occurred. Close only `q25l`; leave `zrmj` for the independent `jbhb` audit and Derrick's later physical tuning authority.

## Final audit — PASS (`jbhb`, 2026-09-08)

Final audit independently verified the pushed source authorities and trees: content `cce1ee215d428358863798c00459054c54898ce4` / `cd3808b595397dcc52deaed612fdba1a30a9a74d`, renderer `cbf91252d9e1b6b76d3b4dad558e5235338b9b63` / `43bd4ab8f464ac1d795366c59c6c5e79b0412ee0`, assembly product `b050e9c628fffa1377a7cf2437350f2a71e89d95` / `89f5fdab42fc8ed1244ca263c1f89b6768bf1eb9`, and QA documentation `3731546a35f8fab3853b1d35a607201b9f079066` / `d686989136a4ede5be3143b37ea9e3b53c5e88b8`. Content and renderer `HEAD` equal `origin/main`; assembly product is the exact ancestor of the QA docs-only tip, whose diff changes only this diagnosis and the active plan.

Source inspection PASS. The content symbol property is non-enumerable, non-writable, non-configurable, not exported, and returns only the exact current ready generation's already-validated mapper; loading, stale generation, replacement, disconnect/destroy all return `null`, while public null-prototype timing remains unchanged and still rejects the ordinary-object authored validator. Assembly consumes the seam with the exact snapshot generation, preserves event/index object identity, distinguishes nonzero mapper-unavailable from mapper-mismatch diagnostics, emits only complete all-or-none trajectory fields, and retains configured straight visibility without restoring the raw fixed-`2500 ms` fallback. Bombs, obstacles, squat/weave checkpoints, static surfaces, feedback, environment, cursors, scoring, timing, persistence, snapshots, events, iframe messages and telemetry remain excluded. Renderer culling uses complete explicit trajectory start when present, including the greater-than-ten-second prelude, while retaining generic future culling for targets without complete trajectory metadata; lane separation remains an independent branded scalar applied consistently to Lanes targets, timing tiles, walls and shadows.

The regression is not the synthetic path that missed raw `0.0.44`. It verifies the exact `89,424`-byte `3c9d` Hard fixture and SHA-256 `4db5b3393a389c7bcaba6d7a02aec57c10801bcfd74de91523b8e9cdad859b55` through the real browser module Worker, IndexedDB persistence/reload, downloaded collection selection, trusted Test click, real input/change controls and seeks. Four rows cover direct and genuine cross-origin iframe at `844×390` desktop and `390×844` mobile. The repaired rows retain null-prototype timing but produce exact beat-50 default timestamps `[18400,17500,17500]`, extremes `[16800,8000,0]`, zero unavailable/mismatch counts, `Z=-72` normal spawn, continuous sky join, nonzero bounce apex, exact `(0.5,0,0)` landing, materially different framebuffers, Lanes `|X|=2` at separation `4`, exclusion/privacy and fresh-reconnect truth. This directly contrasts the recorded unchanged raw `0.0.44` reproduction: every trajectory timestamp `null`, mismatch count misleadingly `0`, and byte-equivalent default/extreme samples first visible at `5900 ms / Z=-15`, then straight at `Y=0` through hit.

Fresh final-audit commands exited `0`: content `node scripts/validate-content-runtime.js`; renderer `node scripts/validate-test-presentation.js && node scripts/validate-renderer-facade.js`; assembly `npm run test:exact-3c9d-browser`; and assembly immutable/release gates `node scripts/validate-immutable-raw-snapshot.js && node scripts/validate-immutable-raw-mutations.js && npm run test:release-pack-policy`. The browser gate reported ten exact obstacle/pixel/AABB variant rows plus four repaired trajectory rows. The immutable gate reported all `19` snapshots and preserved raw `0.0.44` tree `691aca00aae5ec23b5004b6df70c49ac9364fd9e`; mutation rejection covered `0.0.37`–`0.0.41`, `0.0.43`, and `0.0.44`; release-target append-only and complete bounded pack-policy adversaries passed. Recorded independent full QA remains applicable after this source inspection. No source, version, release target, immutable raw, build-release, serving process/route, asset, tag, publication or physical-PASS state changed; protected `.beads/interactions.jsonl` dirt remains excluded.

**Disposition and authorization:** PASS. Close implementation `zrmj`, then audit `jbhb`, and push Dolt. Authorize exactly one canonical append-only web `release/raw/0.0.45` build from the exact pushed docs-only final-audit tip named in the Bead closure comments, and from no other commit/tree. The builder must preserve immutable raw `0.0.44` and every predecessor. This authorization is build-only: do not serve `0.0.45` and do not change any serving process/route, tag, GitHub Release, npm publication, public route, or physical approval before a separate independent immutable release audit passes.
