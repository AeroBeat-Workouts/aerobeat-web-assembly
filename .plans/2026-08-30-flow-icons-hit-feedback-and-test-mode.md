...
**Assembly coder status (2026-08-30):** implemented and locally validated. Canonical atlas WebGL pixels plus bounded failure/reconnect stale-upload suppression, real committed-timeline projection through late miss commit + `350 ms`, stable sorted unscored alternating Test projection, audio-before-advance Test lifecycle, direct Test resume, side-effect-free fresh menu close, pending-run invalidation on selection/profile changes, exact cross-song Test→Play restart, same-owner resource-aware lease transitions, UI-only downloaded gating, and null/exact host routing are covered. Independent QA/audit and release remain Task 5 work; `aerobeat-web-assembly-vf5` intentionally stays open.
