...
- Later physical menu/start repair landed at assembly `4c0329f`, compact Music UI at `b777bdb` with QA closure `fc7e869`, and contrasted close/camera calibration compositor at assembly `37a35a6` with QA closure `5c70ddd`. Derrick's next phone pass identified misleading singleton Version/Difficulty controls and requested remote/local Preview/Stop. Exact diagnosis and media/privacy design are committed at `23b6e76` in `docs/task12-music-preview-select-debug.md`; linked P0s `aerobeat-web-ui-92u` and `aerobeat-web-assembly-3ef` are in progress. Provider map `4BE5E` has one version and only Hard, while comparison map `47FB6` proves multi-option native selection still works. Physical confirmation remains pending after implementation, independent QA, release regeneration, and handoff refresh.
