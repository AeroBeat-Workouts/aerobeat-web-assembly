...
**Post-audit option-semantics gap:** Parent review found that the seven-radio QA fixture covered only Gameplay and Visuals while Music was empty. In populated Music, BeatSaver map selection and library package selection still render as ordinary buttons despite being mutually exclusive options; Version/Difficulty are already correct selects, and Search/Latest/ZIP/Import/Export/Delete/Cancel are true actions that should remain buttons. `docs/task12-prototype-start-debug.md` now records this exact gap. P0s `aerobeat-web-ui-24a` and `aerobeat-web-assembly-3cw` must add checked current/first Music radios and populated responsive/integration evidence before physical retest.
