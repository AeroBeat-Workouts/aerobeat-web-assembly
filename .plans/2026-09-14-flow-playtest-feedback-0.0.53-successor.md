...
- **NEXT: W2 Renderer** (render the entry cone + collider overlay, driven by new per-frame flags `frame.visibleToleranceRange` / `frame.visibleColliderRadius` / `frame.colliderRadius` / `frame.directionToleranceDegrees`; idle/off → nothing).
