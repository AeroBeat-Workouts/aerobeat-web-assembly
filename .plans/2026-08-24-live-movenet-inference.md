...
**Date:** 2026-08-24
**Status:** Blocked
**Last Updated:** 2026-08-24 17:17 EDT
**Blocked Reason:** Derrick's physical Android Chrome retest of `0.0.7` passed for skeleton correctness, camera switching, and tracking-profile switching, but both available tracking profiles remain too slow for keeping track of the athlete. The current blocker is deciding and planning the next CV performance optimization slice, likely camera/input resolution, inference downscale, frame pacing, or ROI work rather than preview smoothing.
**Agent:** cookie
...
Auditor pass completed on 2026-08-24 16:51 EDT. Independent source inspection confirmed the visible phone-route controls are composed in the first-viewport assembly topbar, the camera options are populated through `aerobeat-web-video` `listCameraDevices()` over browser `enumerateDevices()` `videoinput` entries, selected camera constraints use `video.deviceId.exact`, and the live camera restart path stops the retained stream/CV route before re-requesting the selected device. `aero-media-pose-preview` reports `trackingProfile`, limits rendered landmarks to `nose`, `left_wrist`, `left_elbow`, `left_shoulder`, `right_shoulder`, `right_elbow`, and `right_wrist`, preserves the prior smoother alpha `0.42`, uses fast alpha `1`, and connects nose to both shoulders through skeleton connections `[0,5]` and `[0,6]`. The live route returned HTTP/2 200 with `aerobeat-release-proof` `0.0.7`; assembly `package.json`, `package-lock.json`, and `index.html` all prove `0.0.7`; assembly, UI, video, CV, and vendor MoveNet repos were clean/up to date. Auditor validation passed in `aerobeat-web-assembly`: `npm test`, `npm run build`, and `npm run test:browser`. Audit did not prove real Android Chrome latency/readability.

Derrick completed the physical Android Chrome retest after audit. Confirmed successful on `0.0.7`: the skeleton looks visually correct, changing cameras worked, and changing latency/tracking settings worked. Remaining failure: tracking is still too slow for athlete use even in the faster profile. Follow-up analysis identified that the current `Fast` profile only disables preview smoothing; it does not reduce MoveNet inference cost. Current CV behavior already uses MoveNet `SINGLEPOSE_LIGHTNING`, WebGL backend, single-pose inference, latest-frame-wins scheduling, and `requestAnimationFrame` sampling, but does not currently cap camera resolution, downscale inference input, crop to an athlete ROI, use `requestVideoFrameCallback`, move inference off the main thread, expose inference quality presets, or instrument inference duration separately from media/pose delta. The next implementation slice should be planned explicitly before changing behavior.
