...
- **Combined package QA (parent, 2026-09-15)** — all 8 repos green: assembly (test:unit 0, check 0, test:browser 0 incl. visual-correction + 0.0.55-visual-proof e2e harness), renderer (check 0, test:browser 0), contracts/gameplay/content/content-authoring/ui/vendor-mediapipe (npm test 0 each). Bead `mzwt` (W4) in progress.
 - **W4 Pipeline — COMPLETE:**
   - **Source stage:** exact-once `version:patch` 0.0.54→0.0.55 + version-literal oracle repair (commit `0f3b08f`). All assembly gates green (test:unit 0, check 0, test:browser 0).
   - **ABCCBA:** PASS — bound to `b79037d1…` (218 inputs, 12 runs, RTX 3080 target-hardware headed-x11, no drift, noise 0, privacy all false; evidence at `.plans/evidence/2026-09-15-raw-0.0.55-target-abccba.json`).
   - **Build:** exactly one immutable MediaPipe-only build (40 files, 28 MB, proofVersion 0.0.55, sourceFingerprint `b79037d1…`, index.html SHA `9faf5159…`, poseBackends ["mediapipe"]). Independent immutable audit: PASS on items 2–7 (raw integrity, fingerprint binding, provenance pins, prior raws immutable, MediaPipe-only, ABCCBA evidence); item 1 (exactly-one build commit) resolved by force-tracking release/raw/0.0.55 in this commit (mirroring ef079df for 0.0.54).
   - **Serving switch:** collected the old 5173 job, started a new job serving raw 0.0.55 on 127.0.0.1:5173; tailnet :8443 proxy unchanged (same upstream port); shadow-aware smoke byte-identical local + tailnet; hand link delivered to Derrick for the physical retest.
