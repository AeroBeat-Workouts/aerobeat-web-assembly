...
**Status:** PACK-POLICY P0 `aerobeat-web-assembly-cf1` CODER PASS / INDEPENDENT QA-AUDIT PENDING — fail-open tar termination/trailing-stream/all-member-path/CLI defects are repaired with adversarial tests; normalized identity remains exact; `cf1`, `1wu`, and `nme` remain in_progress; audit/server blocked
