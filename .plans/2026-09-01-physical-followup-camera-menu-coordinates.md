...
**Status:** PACK-POLICY AND IMMUTABLE RELEASE FINAL AUDIT PASS / EXACT `0.0.33` SERVED — all seven release Beads closed and Dolt-pushed; managed job `bash-5` serves audited raw bytes on `127.0.0.1:5173`; physical gate remains open

- Preserved raw `0.0.32` and every older release byte-exact. The pre/post aggregate through `0.0.32` is `349` files / `706,996,083` bytes with recursive inventory SHA-256 `9e1ca712a88825a6ab880aeb3b4426c21faf963ee8b591ae8d4f92d777e5bfd8`; `0.0.32` alone remains `13` files / `11,518,186` bytes with inventory hash `5b57e7fac19f406b57ebdc7a05c764d511b15edae3aa999952b5f6feacddef96` and proof hash `0962d46169ce7834033aaacefbcb777cacc7dfea43e8f7fbfa9351e832db47c7`. Orchestrator rechecked all eight predecessor version directory Git tree IDs between pre-release baseline `1cd194a` and current head: `8` checked, `0` mismatches.
...
- Orchestrator repair applies that contract directly after two coder children made no progress. Targeted Node syntax checks and `npm run test:release-pack-policy` pass, including forged diagnostics for exits `0`, `7`, `124`, SIGTERM, and SIGKILL plus genuine TERM/KILL process-group deadlines.
- Independent policy QA PASS: the full policy self-test exits `0`; source/diff inspection confirms no child stderr or locale authority, outer-watchdog errors precede monotonic deadline classification, immediate forged outcomes remain ordinary errors/signals under two seconds, and genuine test-only TERM/KILL deadlines preserve descendant/temp/target/lookalike invariants.
- Independent release QA PASS: two fresh exact-`55f4088` worktrees started with distinct wrong modes (`0600`/`0664`), normalized to `214×0644`, and produced `4` dry plus `4` actual packs. All eight JSON hashes are `cbbdb639a9a091b946b2764988e3c34daf57c882fa511919a93c295fc99da65c`; all four archives/manifests reproduce `e2746b16168c03b056e628e66e33ea687bd3ad6db8b45b505c43069ebbc7dcf6` / `939693741365c836abd9e7f1279c9888f4bf03c9e809347094da69d3b44e7c97` / `ca299ccaaceb12edfc00c1811cf86c89a77b96c46a30c849b4ee5a952d94b957`; internal verification is `4/4`, `97×0644`, zero PAX, exact two-block termination/EOF. Raw `0.0.33`, source fingerprint, and all eight predecessor release trees remain exact.
...
**Status:** FINAL AUDIT PASS — reviewed shared default and private Visual-Test-only Load→fine-tune→Export workflow audited at renderer `75dc6f2` and assembly `ab297cd`; `2c6` closed, while physical gate `7f9` and baseline cadence bug `4fo` remain open and separate.
...
- Implementation result: renderer owns the exact `{0.05,1,5}` / zero-Euler / `48,0.1,80` default and strict async live-debug-camera application authority; it clears authoring input, waits for confirmed capture release, rejects stale/inactive state, retains loaded projection during later movement/look, and canonically re-exports. Assembly owns a trusted Visual-Test-only hidden `.json,application/json` picker, 16 KiB pre/post-read bounds, fatal UTF-8 decode, local bounded status, and connection/session/graph generation guards with no storage/bridge/event/snapshot/telemetry route.
- Orchestrator verification: renderer `npm test` and `npm run test:browser` pass. Assembly `npm test` passes. A focused out-of-tree shell-matrix run (only pre-existing cadence assertions bypassed) passes direct portrait DPR1 and direct landscape requested-DPR3 across Flow/Lanes/Grid, proving deterministic export SHA-256 `b04663252307ae8be9ba0f0f542a88f6660b6bc09fbe88c658f098b103b67bee`, loader lifecycle behavior, and revised canvas-wide pixel validity.
- Shell-matrix diagnostic: unchanged quiet runs and a detached clean baseline `3c7a558` worktree reproduce the same high-DPR cadence p90 `46.875 ms` failure before camera-loader checks. This is tracked separately as `4fo`; no cadence source or threshold change is included in `2c6`. See `.plans/debug/2026-09-02-shell-matrix-cadence.md`. The authorized closer camera also exposed a stale fixed-corner pixel assumption; the test now retains DPR/dimensions/full-opacity/positive-background/exact-partition checks without requiring one specific pixel to remain uncovered. See `.plans/debug/2026-09-02-camera-default-corner-sample.md`.
- Independent QA: the first bounded QA found one stale old-default sentence in the renderer README and no implementation defect. Diagnostic `.plans/debug/2026-09-02-renderer-readme-stale-default.md` records the root cause; renderer `75dc6f2` corrects it. Fresh re-QA PASS confirms docs/source/tests match the reviewed values and artifact hash; renderer `npm run check:renderer`, assembly `npm run check:components`, both diff checks, and exact clean/upstream heads pass.
- Final independent audit PASS: exact renderer `b1f6627..75dc6f2` and assembly `3c7a558..ab297cd` diffs confirm the reviewed `{0.05,1,5}` / zero-Euler / `48,0.1,80` shared default, strict bounded/fatal-UTF-8 private Visual-Test loader, confirmed capture/input release, generation-safe stale rejection, projection-preserving fine-tune/export, revoked download URL, and no storage/snapshot/event/telemetry/bridge/upload/persistence path. Renderer `npm run check:renderer`, assembly `npm run check:components`, both diff checks, and exact clean upstream heads pass. Baseline cadence remains solely `4fo`; physical work remains solely `7f9`.
...
- Release identity checks PASS: proof/package/lock/index all `0.0.34`; `raw-unminified-browser-proof`; `minified:false`; MediaPipe GPU-WebGL measured input at a 15fps submission target; no runtime WASM; all six required runtime markers present; all eight forbidden runtime markers absent; no forbidden runtime asset; camera-pose schema/artifact markers absent from the proof document.
- Fresh coder gates PASS: `npm test`; production `npm run build` (`1300` modules; only known Vite worker-externalization and chunk advisories); `npm run test:v4-integration` (`vendor=7b14eec`, golden `96e68173fffd6454bfb38740acaf58653da11320`, `3` packages / `5` variants); `npm run test:release-pack-policy`; `npm ls --all` with declared platform/feature-optional absences only; and source/plan `git diff --check` excluding the immutable generated raw payload (whose unminified vendored shader strings contain upstream trailing spaces byte-identically in both deterministic builds). `test:live-flow-obstacles` was not run because its validator directly fetches BeatSaver map `3C9D` and is therefore network-dependent.
- Focused immutable-raw Chromium proof PASS in direct portrait `390×844` requested DPR1 and direct landscape `844×390` requested DPR3 (renderer-capped DPR2), across Flow/Lanes/Grid. Both contexts proved the shared default, trusted local Load, strict invalid/extra/oversize rejection, cleared capture/movement, continued fine tuning, deterministic Export (`478` bytes; SHA-256 `b04663252307ae8be9ba0f0f542a88f6660b6bc09fbe88c658f098b103b67bee`), lifecycle cleanup, scalar privacy, full opaque canvas partition, and zero unexpected console noise. As prescribed by the existing `4fo` diagnostic, this used an untracked `/tmp` copy converting only the two pre-existing cadence timing asserts into diagnostics; the tracked cadence source and thresholds are unchanged, all remaining cadence assertions ran, and neither focused run emitted a cadence diagnostic. The known all-context baseline reliability reproduction remains documented under `4fo` and was not absorbed or weakened here.
- No npm publish, upload, GitHub Release, server change, visual/environment/Gaussian work, authored-default change, or unrelated source edit occurred.

#### Immutable camera-loader successor `0.0.34` independent release QA — 2026-09-02

- **QA PASS at exact candidate `6957f42e94e2c5716d89bc3479828320b8deda6a`; `khx` closed and Dolt-pushed.** Two fresh detached sibling worktrees were normalized from the Git index to `238×0644`, then only `release/raw/0.0.34` was cleared in each before one independent `npm run build-release` round. Both rebuilt trees and the tracked canonical tree match completely by relative path, Git mode, filesystem mode, size, and bytes: `13` files / `11,565,989` bytes, QA inventory SHA-256 `98667352658a94d747cb5527a89f0a5ea1e85cb9c500f301d822fee3b5a44a09`; proof SHA-256 `46ca87869bae0016639277832520eec48031532c5ab509a2cfca36f8fc3d12a2`. Each detached worktree remained clean after rebuilding.
- Pre/post predecessor inventories through `0.0.33` are byte-identical in both worktrees: `114` files / `79,791,684` bytes, QA path/Git-mode/filesystem-mode/size/content inventory SHA-256 `c3e7941dbda17cd72f4d6f7766943e940571bd07d4712b260b38ffcf2dbafa8d`. The eight predecessor Git tree IDs `0.0.26` through `0.0.33` are unchanged across the payload commit.
- Each fresh exact-candidate worktree then removed its dependency symlink, re-normalized cleanly, and ran one dry plus one actual npm pack followed by independent policy verification. All four metadata JSON records are byte-identical at SHA-256 `9478f2bb6d7e09cd1d9f3f4e415a02b5ae5e2b36c3b46e01b4659bd67e00a493`: `108` files, `457,868` packed bytes, `1,627,764` unpacked bytes, npm shasum `f5fbe39a8e3e34decb5848309bb01ce0bb24450c`, integrity `sha512-Gin7MLr/Cm3inAYuZupPf1yGkICMABQxYY2DfH4Wma41ahKjfHcBrZdTV9JHWVBuGqQbAfAZYN51KIdB+OckMA==`. Both TGZ archives are byte-identical at SHA-256 `fd4f89f541f08c975807bce733d2d55197ccb30bd0e14a1c7bc3815bab3e2cf9`; decompressed TAR SHA-256 is `ded29a66cd9bf910560611c95f0f31d28ecab83ca25b99b5f52b0672f1dcced8`; manifest SHA-256 is `28a1c2f87ecd7f51f6e75c7b0c0cc66fc18d8d7f09976674fe56bf585ea02ccf`; verifier reports `108×0644`, zero PAX, and exact two-zero-block termination/EOF.
- Per Derrick's bounded QA recovery directive, broad source tests and the all-context shell matrix were not repeated; their independent prior PASS evidence remains above. Canonical source/raw bytes were not modified, and no server, publish, upload, GitHub Release, visual, or environment action occurred. `sfs`, `7f9`, and cadence bug `4fo` remain open.

#### Immutable camera-loader successor `0.0.34` corrected final audit — 2026-09-02
