...
import { createEquipmentConfigIdentity,createResolvedEquipmentPose,equipmentEulerDegreesToQuaternion,saberCapsuleGeometry } from "@aerobeat/web-contracts";
import { createAeroGameplaySessionCoordinator } from "@aerobeat/web-gameplay";

const HASH="a".repeat(64);
const CONFIG_IDENTITY=createEquipmentConfigIdentity({schema:"aerobeat/equipment_config_identity",version:1,algorithm:"sha256",value:"1".repeat(64)});const ORIENTATION=equipmentEulerDegreesToQuaternion({x:0,y:0,z:0});
...
function equipmentPoses(evidenceValue){return ["left_wrist","right_wrist"].map((role)=>{const anchor=evidenceValue.anchors.find((entry)=>entry.anchor===role);return createResolvedEquipmentPose({role,mode:"flow",anchor:{x:anchor.x*4-.5,y:2.5-anchor.y*3,z:0},scale:2,orientation:ORIENTATION,geometryIdentity:saberCapsuleGeometry.identity,configIdentity:CONFIG_IDENTITY});});}
function frame(timestampMs,positionMs,evidenceValue,playing=true){return {timestampMs,clock:clock(positionMs,playing),input:{calibration:{calibrationId:"cal-1",readiness:"countdown"},tracking:{gameplayPaused:false,freshCalibrationRequired:false},sourceIdentity:"assembly-source",countdownFrozen:false,latestEvidence:evidenceValue??null,straightQualifications:[]},...(playing&&evidenceValue?{equipmentPoses:equipmentPoses(evidenceValue)}:{})};}
