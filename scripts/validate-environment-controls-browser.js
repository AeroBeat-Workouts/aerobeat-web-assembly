...
try {
  ...
  const staleBouncePicker=await game.evaluate(async(element)=>{const input=element.beatBounceInput(),originalClick=input.click.bind(input),before=JSON.stringify(element.beatBounceConfig),candidate={...element.beatBounceConfig,heightWorldUnits:0.4};let resolveRead;const readPromise=new Promise((resolve)=>{resolveRead=resolve;});Object.defineProperty(input,"files",{configurable:true,value:[{size:JSON.stringify(candidate).length,arrayBuffer:()=>readPromise}]});input.click=()=>{};element.openBeatBouncePicker({isTrusted:true});input.click=originalClick;const request=element.beatBouncePickerRequest,task=element.handleBeatBounceFile({currentTarget:input,isTrusted:true});await Promise.resolve();element.setMenuOpen(true);await element.menuPauseTail;element.setMenuOpen(false);await element.menuPauseTail;const deadline=performance.now()+6000;while(!element.beatBounceAuthoringSnapshot().enabled&&performance.now()<deadline)await new Promise((resolve)=>setTimeout(resolve,25));const reenabled=element.beatBounceAuthoringSnapshot().enabled,generationAdvanced=element.beatBouncePickerGeneration>request.generation;resolveRead(new TextEncoder().encode(JSON.stringify(candidate)).buffer);await task;delete input.files;return{before,after:JSON.stringify(element.beatBounceConfig),reenabled,generationAdvanced,requestDiscarded:element.beatBouncePickerRequest===null};});assert.deepEqual(staleBouncePicker,{before:staleBouncePicker.before,after:staleBouncePicker.before,reenabled:true,generationAdvanced:true,requestDiscarded:true},"disable then re-enable must never resurrect an in-flight bounce picker read");

  const rendererBoundary=await game.evaluate((element)=>{const renderer=element.graph.renderer,before=renderer.beatBounceConfig,record={...before};let stableGetterCalls=0,selfErasingGetterCalls=0;const stable={...record};Object.defineProperty(stable,"leadBeats",{enumerable:true,get(){stableGetterCalls+=1;return 4;}});const selfErasing={...record};Object.defineProperty(selfErasing,"leadBeats",{configurable:true,enumerable:true,get(){selfErasingGetterCalls+=1;Object.defineProperty(selfErasing,"leadBeats",{configurable:true,enumerable:true,writable:true,value:4});return 4;}});const traps={getPrototypeOf:0,ownKeys:0,getOwnPropertyDescriptor:0,get:0,has:0,isExtensible:0},proxy=new Proxy(record,{getPrototypeOf(){traps.getPrototypeOf+=1;return Object.prototype;},ownKeys(){traps.ownKeys+=1;return Reflect.ownKeys(record);},getOwnPropertyDescriptor(target,key){traps.getOwnPropertyDescriptor+=1;return Reflect.getOwnPropertyDescriptor(target,key);},get(target,key,receiver){traps.get+=1;return Reflect.get(target,key,receiver);},has(){traps.has+=1;return true;},isExtensible(){traps.isExtensible+=1;return true;}});for(const value of[stable,selfErasing,proxy,structuredClone(before),Object.freeze({...before})])try{renderer.setBeatBounceConfig(value);}catch{}return{stableGetterCalls,selfErasingGetterCalls,traps,atomic:renderer.beatBounceConfig===before};});assert.deepEqual(rendererBoundary,{stableGetterCalls:0,selfErasingGetterCalls:0,traps:{getPrototypeOf:0,ownKeys:0,getOwnPropertyDescriptor:0,get:0,has:0,isExtensible:0},atomic:true},"assembly renderer mutation boundary must reject arbitrary objects without inspection");
