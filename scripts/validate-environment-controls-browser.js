...
try {
      ...
      for (const dpr of [1, 3]) {
        ...
        try {
          ...
          const evidence = await target.locator("aero-game").evaluate((game) => {
            game.debugCameraSnapshot = () => Object.freeze({ visible:true, enabled:true }); game.testPresentationAuthoringSnapshot = () => Object.freeze({ visible:true, enabled:true });
            game.menuOpen = false;
            ...
            const controls = [root.querySelector("[data-action='debug-controls-collapse']"), ...root.querySelectorAll(".environment-authoring button,.environment-authoring select,.environment-authoring input,.test-presentation-authoring button,.test-presentation-authoring select,.test-presentation-authoring input")].filter((node) => !node.hidden).map((node) => ({ tag:node.tagName, type:node.type, action:node.dataset.action ?? "", ...rect(node) }));
  ...
  await game.evaluate((element) => {
    ...
    element.testPresentationAuthoringSnapshot = () => Object.freeze({ visible:true, enabled:true });
    ...
    element.shadowRoot.addEventListener("click", (event) => { const action=event.composedPath().find((node)=>node?.dataset?.action)?.dataset?.action;if(action==="environment-config-load")globalThis.__environmentAudit.trustedLoadActivations.push(event.isTrusted);if(action==="test-presentation-load")globalThis.__environmentAudit.trustedBounceActivations.push(event.isTrusted); }, true);
    ...
    element.testPresentationInput().removeEventListener("change", element.boundTestPresentationFile);
  ...
  const behavior = await game.evaluate(async (element) => {
    ...
    peer.remove();
    const lead=element.shadowRoot.querySelector("[data-test-presentation-field='bounceLeadBeats']"),height=element.shadowRoot.querySelector("[data-test-presentation-field='bounceHeightWorldUnits']"),beforeBounce=JSON.stringify(element.testPresentationConfig);lead.value="6.1234567";lead.dispatchEvent(new Event("change",{bubbles:true,composed:true}));const updatedBounce=JSON.stringify(element.testPresentationConfig);height.value="99";height.dispatchEvent(new Event("change",{bubbles:true,composed:true}));const invalidAtomic=JSON.stringify(element.testPresentationConfig)===updatedBounce;element.resetTestPresentation();const resetBounce=JSON.stringify(element.testPresentationConfig),publicLeak=JSON.stringify(element.getSnapshot()).includes("bounceLeadBeats")||JSON.stringify(element.getSnapshot()).includes("beat_bounce");
  ...
  await game.locator("[data-action='test-presentation-save']").click();const bounceSaved=await game.evaluate(async()=>{const audit=globalThis.__environmentAudit,created=audit.urls.created.at(-1),anchor=audit.urls.anchors.at(-1);return{type:created.type,text:await created.blob.text(),download:anchor.download,revoked:audit.urls.revoked.at(-1)===created.value};});assert.equal(bounceSaved.type,"application/json");assert.equal(bounceSaved.download,"aerobeat-test-presentation-config.v1.json");assert.equal(bounceSaved.revoked,true);assert.ok(bounceSaved.text.endsWith("\n"));assert.deepEqual(Object.keys(JSON.parse(bounceSaved.text)),["schema","version","bounceLeadBeats","bounceHeightWorldUnits","bounceApexFraction","bounceRiseEasing","bounceFallEasing","normalSpawnDistanceWorldUnits","skyMode","skyPreludeHeightWorldUnits","skyPreludeDurationMs","skyPreludeEasing","boxingLaneSeparationWorldUnits"]);
  const bounceLoadButton=game.locator("[data-action='test-presentation-load']"),loadBounce=async(name,buffer)=>{const chooserPromise=page.waitForEvent("filechooser");await bounceLoadButton.click();const chooser=await chooserPromise;await chooser.setFiles({name,mimeType:"application/json",buffer});await game.evaluate(async(element)=>{const trusted=globalThis.__environmentAudit.trustedBounceActivations.at(-1)===true;await element.handleTestPresentationFile({currentTarget:element.testPresentationInput(),isTrusted:trusted});});};const bounceCandidate={...JSON.parse(bounceSaved.text),bounceHeightWorldUnits:1.2},bounceText=JSON.stringify(bounceCandidate),bounceExact=Buffer.from(bounceText+" ".repeat(16*1024-Buffer.byteLength(bounceText)));await loadBounce("exact-bounce.json",bounceExact);assert.equal(await game.evaluate(element=>element.testPresentationConfig.bounceHeightWorldUnits),1.2);for(const [name,bytes]of [["oversize-bounce.json",Buffer.alloc(16*1024+1,0x20)],["utf8-bounce.json",Buffer.from([0xc3,0x28])],["unknown-bounce.json",Buffer.from(JSON.stringify({...bounceCandidate,extra:1}))]]){const before=await game.evaluate(element=>JSON.stringify(element.testPresentationConfig));await loadBounce(name,bytes);const after=await game.evaluate(element=>({config:JSON.stringify(element.testPresentationConfig),status:element.shadowRoot.querySelector("[data-role='test-presentation-status']").value}));assert.equal(after.config,before);assert.match(after.status,/invalid/u);}

  const staleBouncePicker=await game.evaluate(async(element)=>{const input=element.testPresentationInput(),originalClick=input.click.bind(input),before=JSON.stringify(element.testPresentationConfig),candidate={...element.testPresentationConfig,heightWorldUnits:0.4};let resolveRead;const readPromise=new Promise((resolve)=>{resolveRead=resolve;});Object.defineProperty(input,"files",{configurable:true,value:[{size:JSON.stringify(candidate).length,arrayBuffer:()=>readPromise}]});input.click=()=>{};element.openTestPresentationPicker({isTrusted:true});input.click=originalClick;const request=element.testPresentationPickerRequest,task=element.handleTestPresentationFile({currentTarget:input,isTrusted:true});await Promise.resolve();element.setMenuOpen(true);await element.menuPauseTail;element.setMenuOpen(false);await element.menuPauseTail;const deadline=performance.now()+6000;while(!element.testPresentationAuthoringSnapshot().enabled&&performance.now()<deadline)await new Promise((resolve)=>setTimeout(resolve,25));const reenabled=element.testPresentationAuthoringSnapshot().enabled,generationAdvanced=element.testPresentationPickerGeneration>request.generation;resolveRead(new TextEncoder().encode(JSON.stringify(candidate)).buffer);await task;delete input.files;return{before,after:JSON.stringify(element.testPresentationConfig),reenabled,generationAdvanced,requestDiscarded:element.testPresentationPickerRequest===null};});assert.deepEqual(staleBouncePicker,{before:staleBouncePicker.before,after:staleBouncePicker.before,reenabled:true,generationAdvanced:true,requestDiscarded:true},"disable then re-enable must never resurrect an in-flight bounce picker read");

  const rendererBoundary=await game.evaluate((element)=>{const renderer=element.graph.renderer,before=renderer.testPresentationConfig,record={...before};let stableGetterCalls=0,selfErasingGetterCalls=0;const stable={...record};Object.defineProperty(stable,"bounceLeadBeats",{enumerable:true,get(){stableGetterCalls+=1;return 4;}});const selfErasing={...record};Object.defineProperty(selfErasing,"bounceLeadBeats",{configurable:true,enumerable:true,get(){selfErasingGetterCalls+=1;Object.defineProperty(selfErasing,"bounceLeadBeats",{configurable:true,enumerable:true,writable:true,value:4});return 4;}});const traps={getPrototypeOf:0,ownKeys:0,getOwnPropertyDescriptor:0,get:0,has:0,isExtensible:0},proxy=new Proxy(record,{getPrototypeOf(){traps.getPrototypeOf+=1;return Object.prototype;},ownKeys(){traps.ownKeys+=1;return Reflect.ownKeys(record);},getOwnPropertyDescriptor(target,key){traps.getOwnPropertyDescriptor+=1;return Reflect.getOwnPropertyDescriptor(target,key);},get(target,key,receiver){traps.get+=1;return Reflect.get(target,key,receiver);},has(){traps.has+=1;return true;},isExtensible(){traps.isExtensible+=1;return true;}});for(const value of[stable,selfErasing,proxy,structuredClone(before),Object.freeze({...before})])try{renderer.setTestPresentationConfig(value);}catch{}return{stableGetterCalls,selfErasingGetterCalls,traps,atomic:renderer.testPresentationConfig===before};});assert.deepEqual(rendererBoundary,{stableGetterCalls:0,selfErasingGetterCalls:0,traps:{getPrototypeOf:0,ownKeys:0,getOwnPropertyDescriptor:0,get:0,has:0,isExtensible:0},atomic:true},"assembly renderer mutation boundary must reject arbitrary objects without inspection");
