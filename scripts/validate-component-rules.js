...
for (const root of ["src", "test", "demo"]) {
  ...
  for (const file of collectFiles(root)) {
    ...
    const withoutAssemblyShellButtons = source.replace(/<button\b(?=[^>]*\bdata-action="(?:menu-toggle|menu-close|menu-backdrop|debug-camera-reset)")[^>]*>/gu, "").replace(/<input\b(?=[^>]*\bdata-action="environment-select")[^>]*>/gu, "");
