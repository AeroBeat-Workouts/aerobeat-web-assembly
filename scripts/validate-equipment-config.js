...
assert.deepEqual(canonical.flow.saber.zones.edgeTop.localRotationEulerDeg, { x:0, y:0, z:0 });
assert.deepEqual(canonical.flow.saber.zones.edgeBottom.localRotationEulerDeg, { x:0, y:0, z:0 });
assert.deepEqual(canonical.flow.saber.zones.edgeLeft.localRotationEulerDeg, { x:0, y:90, z:0 });
assert.deepEqual(canonical.flow.saber.zones.edgeRight.localRotationEulerDeg, { x:0, y:-90, z:0 });
assert.equal(canonical.flow.saber.ease.durationMs, 1);
assert.equal(canonical.flow.saber.blendRadius, 0.5);
...
const fixturePath = resolve("scripts/fixtures/aerobeat-equipment-config-2026-09-25.yaml");
const fixtureText = readFileSync(fixturePath, "utf8");
assert.equal(createHash("sha256").update(fixtureText).digest("hex"), "25c487b1b20d31b146b01029e05f7624e1fc54b69feb51a9297596fbd8b9a066", "latest downloaded strict v4 fixture retains exact provenance bytes");
assert.equal(yaml, fixtureText, "deterministic default export exactly reproduces the downloaded strict v4 fixture");
assert.deepEqual(parseEquipmentConfigYaml(fixtureText), canonical, "latest downloaded strict v4 fixture exactly matches baked defaults");
const v3FixtureText = readFileSync(resolve("scripts/fixtures/aerobeat-equipment-config-v3-downloaded.yaml"), "utf8");
const parsedV3 = parseEquipmentConfigYaml(v3FixtureText);
const rawV3 = parseYamlSubset(v3FixtureText);
assert.deepEqual(migrateEquipmentConfigV3(rawV3), parsedV3, "direct full-v3 migration matches parser migration");
