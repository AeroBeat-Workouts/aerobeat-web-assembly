...
try {
  ...
  const evidence = await game.evaluate(async (element) => {
    ...
    const variant = (packageId, id, modifiers = []) => { const flowObstacleAssist = id === "flow" && modifiers.includes("no_obstacles"); return { variantId: `${packageId}-${id}`, chartId: `${packageId}-${id}-chart`, mode: id === "flow" ? "flow" : "boxing", rulesetId: id === "flow" ? "flow_colliders_v1" : id.startsWith("semantic") ? "boxing_semantic_track_v1" : "boxing_spatial_grid_v1", recipeId: id === "flow" ? null : id.endsWith("row") ? "row_family_balanced_height_v1" : "cut_family_source_height_v1", modifierIds: modifiers, ranked: !flowObstacleAssist, localOnly: flowObstacleAssist, mapHash: { schema: "aerobeat/content_hash", version: 1, algorithm: "sha256", value: hash }, scoreIdentityHash: { schema: "aerobeat/content_hash", version: 1, algorithm: "sha256", value: hash }, provenance: { baseVariantId: `${packageId}-${id}` } }; };
    ...
    const state = { content: ready("package-a", "semantic-row", ["no_squats"]), loads: [], selections: [], reads: [], exports: [], deletes: [], resolveA: () => undefined, aStarted: false, stalePackageId: "", staleCode: "flow_orientation_reimport_required", listCalls: 0 };
    const aGate = new Promise((resolve) => { state.resolveA = () => resolve(undefined); });
    const original = element.graph;
    const content = { ...original.content, getSnapshot: () => state.content, async loadPersistenceHandle(handle) { state.loads.push(`content:${handle.packageId}`); state.content = ready(handle.packageId); }, async selectVariant(variantId, options = {}) { state.selections.push({ variantId, modifierIds: [...(options.modifierIds ?? [])] }); const modifiers = [...(options.modifierIds ?? [])]; const selected = state.content.variants.find((item) => item.variantId === variantId); const assist = modifiers.includes("no_obstacles"); state.content = { ...state.content, selectedVariant: selected ? { ...selected, modifierIds: modifiers, ...(assist && selected.mode === "flow" ? { ranked: false, localOnly: true } : {}) } : null }; }, async swapFutureVariant(variantId, options) { return this.selectVariant(variantId, options); }, setPlaybackState() {}, readAsset(path) { state.reads.push({ packageId: state.content.packageId, path }); return new Uint8Array([79, 103, 103, 83]); } };
    ...
    await commitIntent("gameplay-mode-select", { rulesetId: "flow_colliders_v1" }); modeEvidence.push(state.content.selectedVariant.variantId); const flowFrame = element.rendererFrame();
    // boxing-conversion-select while Flow is selected only updates lastBoxingRecipeId; no variant change.
    element.configureGameplayFromContent(false);const lockedBefore=element.activeSessionSetup.timingWindowMs;const currentSetup=element.desiredGameSetup;element.desiredGameSetup={...currentSetup,timingWindowMs:225};const lockedDuring=element.activeSessionSetup.timingWindowMs,desiredDuring=element.desiredGameSetup.timingWindowMs;element.configureGameplayFromContent(false,undefined,true);const lockedAfter=element.activeSessionSetup.timingWindowMs;element.desiredGameSetup=currentSetup;const setupLock={lockedBefore,lockedDuring,desiredDuring,lockedAfter};const colliderFrame=element.rendererFrame();
    ...
    await commitIntent("boxing-conversion-select", { recipeId: "cut_family_source_height_v1" }); modeEvidence.push(state.content.selectedVariant.variantId);
    await commitIntent("gameplay-mode-select", { rulesetId: "boxing_semantic_track_v1" }); modeEvidence.push(state.content.selectedVariant.variantId); const laneFrame = element.rendererFrame();
    await commitIntent("boxing-conversion-select", { recipeId: "row_family_balanced_height_v1" }); modeEvidence.push(state.content.selectedVariant.variantId);
    await commitIntent("boxing-conversion-select", { recipeId: "cut_family_source_height_v1" }); modeEvidence.push(state.content.selectedVariant.variantId);element.renderPresenters(); const selectedProfileId = element.shadowRoot.querySelector("aero-prototype-selector[scope='gameplay']")?.presenterSnapshot?.selectedProfileId;
    let scalarGetterCalls = 0; const hostilePayload = {}; Object.defineProperty(hostilePayload, "rulesetId", { enumerable: true, get() { scalarGetterCalls += 1; return "flow_colliders_v1"; } }); element.handleUiIntent(new CustomEvent("aero-ui-intent", { detail: { type: "gameplay-mode-select", payload: hostilePayload } })); const scalarRejection = { getterCalls: scalarGetterCalls, code: element.lastError?.code, selected: state.content.selectedVariant.variantId }; element.lastError = null;
    let retiredGetterCalls = 0; const retiredGridPayload = {}; Object.defineProperty(retiredGridPayload, "rulesetId", { enumerable: true, get() { retiredGetterCalls += 1; return "flow_grid_v2"; } }); element.handleUiIntent(new CustomEvent("aero-ui-intent", { detail: { type: "gameplay-mode-select", payload: retiredGridPayload } })); const retiredGridRejection = { getterCalls: retiredGetterCalls, code: element.lastError?.code, selected: state.content.selectedVariant.variantId }; element.lastError = null;
    const gameplayAxes = { modeEvidence, selectedProfileId, setupLock,flowFrame, colliderFrame, gridFrame, laneFrame, scalarRejection, retiredGridRejection };
    await element.lifecycleIntentTail;element.graph.gameplay.stop(0);element.graph.gameplay.reset(0);state.content = ready("package-a", "semantic-row", ["no_squats"]);
    ...
    const latest = { packageId: state.content.packageId, desiredPackageId: element.libraryView.selectedPackageId, loads: [...state.loads], selection: state.content.selectedVariant };
    ...
    const stale=[];state.stalePackageId="package-a";
    for(const code of ["flow_orientation_reimport_required","flow_obstacle_reimport_required","flow_colliders_reimport_required"]){const staleLoadsBefore=state.loads.filter((entry)=>entry==="authoring:package-a").length,staleListsBefore=state.listCalls;state.staleCode=code;await element.requestLibrarySelection("collection-rapid","package-a");stale.push({code:element.lastError?.code,message:element.lastError?.message,selectedCollectionId:element.libraryView.selectedCollectionId,selectedPackageId:element.libraryView.selectedPackageId,desired:element.desiredLibrarySelection,collectionCount:element.libraryView.collections.length,listCalls:state.listCalls-staleListsBefore,loadCalls:state.loads.filter((entry)=>entry==="authoring:package-a").length-staleLoadsBefore});await element.exportLibraryPackage({packageKey:"key-a",packageId:"package-a"});}
    ...
    state.content=ready("package-a","flow",["no_obstacles"]);const importedGeneration=++element.librarySelectionGeneration;await element.enqueueLifecycleIntent("new-import-default",owner=>element.selectLibraryPackage({collectionId:"collection-rapid",packageKey:"key-b",packageId:"package-b"},importedGeneration,owner));const importedDefault={packageId:state.content.packageId,variantId:state.content.selectedVariant.variantId,modifierIds:state.content.selectedVariant.modifierIds};await content.selectVariant("package-b-flow",{modifierIds:["no_obstacles"]});element.configureGameplayFromContent(false,undefined,true);await element.requestLibrarySelection("collection-rapid","package-b");const samePackageReload={packageId:state.content.packageId,variantId:state.content.selectedVariant.variantId,modifierIds:state.content.selectedVariant.modifierIds};
    HTMLAnchorElement.prototype.click = nativeClick;
    await element.deleteLibraryCollection("collection-rapid");
    return { gameplayAxes, latest, grouped, previewExact, stale, corrected, importedDefault, samePackageReload, exports: state.exports, deletes: state.deletes, publicText: JSON.stringify(element.getSnapshot()) };
  });
  assert(JSON.stringify(evidence.gameplayAxes.modeEvidence) === JSON.stringify(["package-mode-semantic-cut","package-mode-flow","package-mode-spatial-cut","package-mode-spatial-cut","package-mode-semantic-cut","package-mode-semantic-row","package-mode-semantic-cut"]), `mode/conversion intents must resolve the exact five-identity matrix and retain conversion independently of Flow: ${JSON.stringify(evidence.gameplayAxes)}`);
  assert(JSON.stringify(evidence.gameplayAxes.setupLock)===JSON.stringify({lockedBefore:180,lockedDuring:180,desiredDuring:225,lockedAfter:225}),`active Collider settings must remain run-locked until explicit fresh configuration: ${JSON.stringify(evidence.gameplayAxes.setupLock)}`);
  ...
  assert([evidence.gameplayAxes.flowFrame,evidence.gameplayAxes.laneFrame,evidence.gameplayAxes.gridFrame].every((frame) => frame.timingWindowBeforeMs === 180 && frame.timingWindowAfterMs === 180)&&evidence.gameplayAxes.colliderFrame.timingWindowBeforeMs===225&&evidence.gameplayAxes.colliderFrame.timingWindowAfterMs===225, `all presentations must receive authoritative timing bounds: ${JSON.stringify(evidence.gameplayAxes)}`);
  assert(evidence.gameplayAxes.flowFrame.presentation === "flow" && evidence.gameplayAxes.colliderFrame.presentation === "flow" && evidence.gameplayAxes.laneFrame.presentation === "boxing_lanes" && evidence.gameplayAxes.gridFrame.presentation === "boxing_spatial_grid", `all three renderer presentations must remain selectable: ${JSON.stringify(evidence.gameplayAxes)}`);
