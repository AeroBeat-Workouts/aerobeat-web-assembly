...
try {
  ...
  const evidence = await game.evaluate(async (element) => {
    ...
    const state = { content: ready("package-a", "semantic-row", ["no_squats"]), loads: [], selections: [], reads: [], exports: [], deletes: [], resolveA: () => undefined, aStarted: false };
    ...
    const authoring = { ...original.authoring, async listPackages() { return packages; }, async listCollections() { return collections; }, async estimateStorage() { return { usageBytes: 4, quotaBytes: 1024 }; }, async loadPackage(handle) { state.loads.push(`authoring:${handle.packageId}`); if (handle.packageId === "package-a" && !state.aStarted) { state.aStarted = true; await aGate; } return { handle: { key: handle.key, packageId: handle.packageId }, package: {} }; }, async exportPackage(handle) { state.exports.push(handle.packageId); return { fileName: `${handle.packageId}.aeropkg`, mediaType: "application/octet-stream", byteLength: 1, bytes: new Uint8Array([1]) }; }, async deleteCollection(id) { state.deletes.push(id); return true; } };
    ...
    element.stopPreview(); const nativeClick = HTMLAnchorElement.prototype.click; HTMLAnchorElement.prototype.click = function () {}; await element.exportLibraryPackage({ packageKey: "key-c", packageId: "package-c" }); HTMLAnchorElement.prototype.click = nativeClick;
    await element.deleteLibraryCollection("collection-rapid");
    return { latest, grouped, previewExact, exports: state.exports, deletes: state.deletes, publicText: JSON.stringify(element.getSnapshot()) };
  });
  ...
  assert(JSON.stringify(evidence.exports) === JSON.stringify(["package-c"]) && JSON.stringify(evidence.deletes) === JSON.stringify(["collection-rapid"]), `export/delete ownership mismatch: ${JSON.stringify(evidence)}`);
  ...
  console.log("Grouped Catalyst library, serial latest-wins selection, exact preview, preservation, export/delete, and privacy validation passed.");
