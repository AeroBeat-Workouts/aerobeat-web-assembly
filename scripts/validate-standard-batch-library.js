...
try {
  ...
  const evidence = await game.evaluate(async (element) => {
    ...
    const variant = (packageId, id, modifiers = []) => ({ variantId: `${packageId}-${id}`, chartId: `${packageId}-${id}-chart`, mode: id === "flow"||id==="flow-colliders" ? "flow" : "boxing", rulesetId: id === "flow" ? "flow_grid_v2" : id==="flow-colliders"?"flow_colliders_v1":id.startsWith("semantic") ? "boxing_semantic_track_v1" : "boxing_spatial_grid_v1", recipeId: id === "flow"||id==="flow-colliders" ? null : id.endsWith("row") ? "row_family_balanced_height_v1" : "cut_family_source_height_v1", modifierIds: modifiers, ranked: false, localOnly:id==="flow-colliders", mapHash: { schema: "aerobeat/content_hash", version: 1, algorithm: "sha256", value: hash }, scoreIdentityHash: { schema: "aerobeat/content_hash", version: 1, algorithm: "sha256", value: hash }, provenance: { baseVariantId: `${packageId}-${id}` } });
    const ready = (packageId, selectedId = "flow", modifiers = []) => { const variants = ["flow", "flow-colliders", "semantic-row", "spatial-row", "semantic-cut", "spatial-cut"].map((id) => variant(packageId, id, id === selectedId ? modifiers : [])); return { state: "ready", packageId, selectedVariant: variants.find((item) => item.variantId === `${packageId}-${selectedId}`), variants, resolvedEvents: [], song: { name: packageId }, background: null, lineage: { difficulty: packageId } }; };
    ...
    await commitIntent("gameplay-mode-select", { rulesetId: "flow_grid_v2" }); modeEvidence.push(state.content.selectedVariant.variantId); const flowFrame = element.rendererFrame();
    ...
    const gameplayAxes = { modeEvidence, selectedProfileId, setupLock,flowFrame, colliderFrame, gridFrame, laneFrame, scalarRejection };
  ...
  assert(JSON.stringify(evidence.gameplayAxes.modeEvidence) === JSON.stringify(["package-mode-semantic-cut","package-mode-flow","package-mode-flow","package-mode-flow-colliders","package-mode-flow-colliders","package-mode-spatial-row","package-mode-spatial-cut","package-mode-semantic-cut","package-mode-semantic-row","package-mode-semantic-cut"]), `mode/conversion intents must resolve the exact six-identity matrix and retain conversion independently of Flow: ${JSON.stringify(evidence.gameplayAxes)}`);
