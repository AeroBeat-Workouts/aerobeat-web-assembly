...
try {
  ...
  assert([evidence.gameplayAxes.flowFrame,evidence.gameplayAxes.laneFrame,evidence.gameplayAxes.gridFrame].every((frame) => frame.timingWindowBeforeMs === 180 && frame.timingWindowAfterMs === 180), `all presentations must receive authoritative timing bounds: ${JSON.stringify(evidence.gameplayAxes)}`);
  assert(evidence.gameplayAxes.flowFrame.presentation === "flow" && evidence.gameplayAxes.laneFrame.presentation === "boxing_lanes" && evidence.gameplayAxes.gridFrame.presentation === "boxing_spatial_grid", `all three renderer presentations must remain selectable: ${JSON.stringify(evidence.gameplayAxes)}`);
  ...
  assert(JSON.stringify(evidence.stale.map((entry)=>entry.code))===JSON.stringify(["flow_orientation_reimport_required","flow_obstacle_reimport_required"])&&evidence.stale.every((entry)=>/reimport/iu.test(entry.message)&&entry.message.length<=256),`both stale package reasons must expose their exact bounded reimport-required errors: ${JSON.stringify(evidence.stale)}`);
  assert(evidence.stale.every((entry)=>entry.selectedCollectionId===null&&entry.selectedPackageId===null&&entry.desired===null&&entry.collectionCount===2&&entry.listCalls===1&&entry.loadCalls===1),`both stale selection reasons must clear and refresh exactly once without auto-selecting: ${JSON.stringify(evidence.stale)}`);
  ...
  assert(JSON.stringify(evidence.exports) === JSON.stringify(["package-c","package-a","package-a"]) && JSON.stringify(evidence.deletes) === JSON.stringify(["collection-rapid"]), `both stale reasons must retain export/delete management: ${JSON.stringify(evidence)}`);
