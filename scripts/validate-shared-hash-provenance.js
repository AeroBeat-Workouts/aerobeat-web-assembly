...
assert.equal(packageJson.version, "0.0.50", "lev0 release source must use the exact authorized successor version");
...
for (const pin of releaseDependencyPins) {
  ...
  if (!["@aerobeat/web-hash","@aerobeat/web-input","@aerobeat/web-contracts","@aerobeat/web-ui"].includes(pin.name)) {
