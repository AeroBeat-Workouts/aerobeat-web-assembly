  ...
  for (const embedding of ["direct", "real_cross_origin_iframe"]) for (const viewport of [{name:"portrait",width:390,height:844},{name:"landscape",width:844,height:390}]) for (const requestedDpr of [1,3]) {
    ...
    try {
      ...
      const evidence = await target.evaluate(async({width,height,requestedDpr}) => {
        ...
        game.stopFrameLoop(); (await import("/src/game-setup-coordinator.js")).setGameSetupSnapshot({...game.desiredGameSetup,spawnDistanceOverride:{enabled:true,normalSpawnDistanceWorldUnits:50}}); await game.lifecycleIntentTail; game.setMenuOpen(false); renderer.resize({widthCssPx:width,heightCssPx:height,devicePixelRatio:requestedDpr}); renderer.setEnvironmentVisible(false); renderer.setBackgroundProjection({kind:"solid",colors:["#071426"],angleDeg:180});
        ...
        const renderEvents=events.map((event)=>event.eventId==="pending-right"?{...event,appearanceColor:"#2468AC"}:event.eventId==="hit-visible"?{...event,appearanceColor:"#13579B"}:event.eventId==="miss-continuing"?{...event,appearanceColor:"#FF0000"}:event);
        const gameplay={session:{purpose:"play",state:"playing",timelinePositionMs:1181,rulesetId:"flow_colliders_v1"},selectedVariant:variant,judgements:[
        ...
        const graph={...originalGraph,content:{...originalGraph.content,getSnapshot:()=>({selectedVariant:variant,resolvedEvents:events}),[projectionSymbol]:()=>renderEvents},gameplay:{...originalGraph.gameplay,getSnapshot:()=>gameplay},input:{...originalGraph.input,getSnapshot:()=>input}};
        const cursorCalls=[], originalCombined=renderer.renderGameplayFrameWithCursorsAndEquipment.bind(renderer);
        renderer.renderGameplayFrameWithCursorsAndEquipment=(renderFrame,cursors,cursorOptions,equipment,equipmentOptions)=>{const value=originalCombined(renderFrame,cursors,cursorOptions,equipment,equipmentOptions);cursorCalls.push({cursors:structuredClone(cursors),cursorOptions:structuredClone(cursorOptions),equipment:structuredClone(equipment),equipmentOptions:structuredClone(equipmentOptions),result:{cursorCount:value.cursorCount??null,equipmentCount:value.equipmentCount??null}});return value;};
        game.graph=graph; const frame=game.rendererFrame(); const result=game.renderGameplay(graph); const cursorStatus=renderer.describe().cursors,equipmentStatus=renderer.describe().equipment; game.graph=originalGraph; renderer.renderGameplayFrameWithCursorsAndEquipment=originalCombined;
        ...
        const boxingCases=[["straight_left","#FF0000"],["straight_right","#808080"],["hook_left","#FF0000"],["hook_right","#808080"],["uppercut_left","#FF0000"],["uppercut_right","#808080"]],boxingModeEvidence=[];
        for(const [mode,rulesetId] of [["boxing_lanes","boxing_semantic_track_v1"],["boxing_spatial_grid","boxing_spatial_grid_v1"]]){const boxingVariant={variantId:`visual-contract-${mode}`,chartId:`visual-contract-${mode}-chart`,mode:"boxing",rulesetId,recipeId:"row_family_balanced_height_v1",modifierIds:[],ranked:false},boxingEvents=[...boxingCases.map(([type],index)=>({eventId:`${mode}-${type}`,centerTimestampMs:1211+index,authoredBeat:{type,spatialTarget:{targetCell:index%2===0?5:6,entryDirection:"up"}}})),{eventId:`${mode}-guard`,centerTimestampMs:1300,authoredBeat:{type:"guard",guardTarget:{leftCell:4,rightCell:7}}}],boxingRenderEvents=boxingEvents.map((event,index)=>index<boxingCases.length?{...event,appearanceColor:boxingCases[index][1]}:event),boxingGameplay={...gameplay,selectedVariant:boxingVariant,session:{...gameplay.session,timelinePositionMs:900},judgements:[]};game.graph={...graph,content:{...graph.content,getSnapshot:()=>({selectedVariant:boxingVariant,resolvedEvents:boxingEvents}),[projectionSymbol]:()=>boxingRenderEvents},gameplay:{...graph.gameplay,getSnapshot:()=>boxingGameplay}};const boxingFrame=game.rendererFrame(),boxingObjects=renderer.renderGameplayFrame(boxingFrame).model.objects;boxingModeEvidence.push({mode,punches:boxingCases.map(([type])=>{const object=boxingObjects.find((entry)=>entry.targetId===`${mode}-${type}`&&entry.kind==="icon");return [type,object?.appearanceColor??null];}),guards:boxingObjects.filter((entry)=>entry.targetId===`${mode}-guard`&&entry.kind==="icon").map((entry)=>entry.appearanceColor)});}
        ...
        game.graph={...graph,content:{...graph.content,getSnapshot:()=>({selectedVariant:variant,resolvedEvents:[wallEvent]}),[projectionSymbol]:()=>[wallEvent]},gameplay:{...graph.gameplay,getSnapshot:()=>wallGameplay}};
        ...
        return {origins:{child:location.origin,parent:document.referrer?new URL(document.referrer).origin:null},frame:{before:frame.timingWindowBeforeMs,after:frame.timingWindowAfterMs},timingBounds,renderOrder:result.model.renderOrder,cursorCall:cursorCalls.at(-1),cursorStatus,equipmentStatus,pendingRight:byTarget("pending-right","icon")[0],hitVisible:byTarget("hit-visible","icon")[0],hitGone:byTarget("hit-gone","icon").length,greatVisible:feedback("hit-visible"),greatGone:feedback("hit-gone"),missIcon:byTarget("miss-continuing","icon")[0],missFeedback:feedback("miss-continuing"),missShadows:byTarget("miss-continuing","shadow").length,boxingModeEvidence,wallFrame:{before:wallFrame.timingWindowBeforeMs,after:wallFrame.timingWindowAfterMs,target:wallFrame.targets[0]},wall,wallShadow,appliedDpr:renderer.devicePixelRatio};
      ...
      assert.deepEqual(evidence.renderOrder,["world_opaque","grid_timing_tiles","guidance_bands","targets","world_transparent_shadows_track_walls_feedback"]);
      assert.equal(evidence.cursorCall.cursorOptions.sizeCssPx,32); assert.equal(evidence.cursorCall.cursorOptions.minConfidence,.5); assert.equal(evidence.cursorCall.cursors.length,0,"GATE 1: legacy markers hidden — production passes an empty cursor array"); assert.deepEqual(evidence.cursorCall.equipment,[{role:"left_wrist",x:.25,y:.65,mode:"flow",direction:{x:0,y:1}},{role:"right_wrist",x:.75,y:.65,mode:"flow",direction:{x:0,y:1}}],"both hands must stage flow equipment records with fallback direction"); assert.equal(evidence.cursorCall.result.equipmentCount,2); assert.equal(evidence.cursorCall.result.cursorCount,0); assert.equal(evidence.cursorStatus.instanceCount,0,"no legacy marker may be staged"); assert.equal(evidence.equipmentStatus.instanceCount,2); assert.deepEqual(evidence.equipmentStatus.roles,["left_wrist","right_wrist"]); assert.deepEqual(evidence.equipmentStatus.modes,["left_wrist:flow","right_wrist:flow"]);
      assert.equal(evidence.pendingRight.assetId,"directional-arrow/rounded-outline-v1"); assert.equal(evidence.pendingRight.rotationZRad,-Math.PI/2);assert.equal(evidence.pendingRight.appearanceColor,"#2468AC","assembly must consume the private content projection instead of public resolved events"); assert.equal(evidence.pendingRight.whiteCore,true,"right directional note must retain the white authored outline/timing flash");
