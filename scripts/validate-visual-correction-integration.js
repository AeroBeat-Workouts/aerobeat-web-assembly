  ...
  for (const embedding of ["direct", "real_cross_origin_iframe"]) for (const viewport of [{name:"portrait",width:390,height:844},{name:"landscape",width:844,height:390}]) for (const requestedDpr of [1,3]) {
    ...
    try {
      ...
      const evidence = await target.evaluate(async({width,height,requestedDpr}) => {
        ...
        return {origins:{child:location.origin,parent:document.referrer?new URL(document.referrer).origin:null},frame:{before:frame.timingWindowBeforeMs,after:frame.timingWindowAfterMs},timingBounds,renderOrder:result.model.renderOrder,cursorCall:cursorCalls.at(-1),cursorStatus,pendingRight:byTarget("pending-right","icon")[0],hitVisible:byTarget("hit-visible","icon")[0],hitGone:byTarget("hit-gone","icon").length,greatVisible:feedback("hit-visible"),greatGone:feedback("hit-gone"),missIcon:byTarget("miss-continuing","icon")[0],missFeedback:feedback("miss-continuing"),missShadows:byTarget("miss-continuing","shadow").length,boxingModeEvidence,wallFrame:{before:wallFrame.timingWindowBeforeMs,after:wallFrame.timingWindowAfterMs,target:wallFrame.targets[0]},wall,wallShadow,appliedDpr:renderer.devicePixelRatio};
