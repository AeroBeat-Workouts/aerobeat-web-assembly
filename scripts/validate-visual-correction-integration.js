...
try {
  for (const embedding of ["direct", "real_cross_origin_iframe"]) for (const viewport of [{name:"portrait",width:390,height:844},{name:"landscape",width:844,height:390}]) for (const requestedDpr of [1,3]) {
    ...
    try {
      ...
      assert.equal(evidence.cursorCall.options.sizeCssPx,18); assert.equal(evidence.cursorCall.options.minConfidence,.5); assert.equal(evidence.cursorCall.cursors.length,3); assert.equal(evidence.cursorStatus.sizeCssPx,18); assert.equal(evidence.cursorStatus.instanceCount,3);
      ...
      assert.deepEqual(evidence.wallFrame.target.geometry,{schema:"aerobeat/flow_obstacle_geometry",version:1,coordinateSpace:"beatsaber_lane_layer",x:1,y:2,width:1,height:3}); assert.deepEqual(evidence.wallFrame.target.cells,[1]); assert.equal(evidence.wall.scale.x,1); assert(Math.abs(evidence.wall.scale.y-2.94/.94)<1e-12); assert(Math.abs(evidence.wall.scale.z-.15)<1e-9); assert.deepEqual({x:evidence.wall.position.x,y:evidence.wall.position.y},{x:-.5,y:3}); assert(Math.abs(evidence.wall.position.z+.075)<1e-5); assert(evidence.wallShadow&&evidence.wallShadow.position.y<evidence.wall.position.y&&evidence.wallShadow.targetId===evidence.wall.targetId,"tall wall must retain one directly-below gameplay-inert shadow");
