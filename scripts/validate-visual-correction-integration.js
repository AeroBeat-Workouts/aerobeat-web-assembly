...
try {
  for (const embedding of ["direct", "real_cross_origin_iframe"]) for (const viewport of [{name:"portrait",width:390,height:844},{name:"landscape",width:844,height:390}]) for (const requestedDpr of [1,3]) {
    ...
    try {
      ...
      assert.deepEqual(evidence.renderOrder,["world_opaque","grid_timing_tiles","guidance_ribbon","guidance_halo","targets","guidance_number_plate","guidance_number_glyph","world_transparent_shadows_track_walls_feedback"]);
