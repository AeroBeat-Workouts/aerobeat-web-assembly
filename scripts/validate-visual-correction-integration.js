...
try {
  for (const embedding of ["direct", "real_cross_origin_iframe"]) for (const viewport of [{name:"portrait",width:390,height:844},{name:"landscape",width:844,height:390}]) for (const requestedDpr of [1,3]) {
    ...
    try {
      ...
      assert.equal(evidence.pendingRight.assetId,"directional-arrow/outline-v1"); assert.equal(evidence.pendingRight.rotationZRad,-Math.PI/2);assert.equal(evidence.pendingRight.appearanceColor,"#2468AC","assembly must consume the private content projection instead of public resolved events"); assert.equal(evidence.pendingRight.whiteCore,true,"right directional note must retain the white authored outline/timing flash");
