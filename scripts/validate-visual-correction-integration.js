  ...
  for (const embedding of ["direct", "real_cross_origin_iframe"]) for (const viewport of [{name:"portrait",width:390,height:844},{name:"landscape",width:844,height:390}]) for (const requestedDpr of [1,3]) {
    ...
    try {
      ...
      const evidence = await target.evaluate(({width,height,requestedDpr}) => {
        ...
        const boxingVariant={variantId:"visual-contract-boxing",chartId:"visual-contract-boxing-chart",mode:"boxing",rulesetId:"boxing_semantic_track_v1",recipeId:"row_family_balanced_height_v1",modifierIds:[],ranked:false},boxingEvents=[{eventId:"boxing-punch",centerTimestampMs:1211,authoredBeat:{type:"hook_left",spatialTarget:{targetCell:5,entryDirection:"up"}}},{eventId:"boxing-guard",centerTimestampMs:1300,authoredBeat:{type:"guard",guardTarget:{leftCell:4,rightCell:7}}}],boxingRenderEvents=[{...boxingEvents[0],appearanceColor:"#FF0000"},boxingEvents[1]],boxingGameplay={...gameplay,selectedVariant:boxingVariant,session:{...gameplay.session,timelinePositionMs:900},judgements:[]};
        game.graph={...graph,content:{...graph.content,getSnapshot:()=>({selectedVariant:boxingVariant,resolvedEvents:boxingEvents}),[projectionSymbol]:()=>boxingRenderEvents},gameplay:{...graph.gameplay,getSnapshot:()=>boxingGameplay}};const boxingFrame=game.rendererFrame(),boxingResult=renderer.renderGameplayFrame(boxingFrame),boxingObjects=boxingResult.model.objects,boxingPunch=boxingObjects.find((entry)=>entry.targetId==="boxing-punch"&&entry.kind==="icon"),boxingGuards=boxingObjects.filter((entry)=>entry.targetId==="boxing-guard"&&entry.kind==="icon");
        ...
        return {origins:{child:location.origin,parent:document.referrer?new URL(document.referrer).origin:null},frame:{before:frame.timingWindowBeforeMs,after:frame.timingWindowAfterMs},timingBounds,renderOrder:result.model.renderOrder,cursorCall:cursorCalls.at(-1),cursorStatus,pendingRight:byTarget("pending-right","icon")[0],hitVisible:byTarget("hit-visible","icon")[0],hitGone:byTarget("hit-gone","icon").length,greatVisible:feedback("hit-visible"),greatGone:feedback("hit-gone"),missIcon:byTarget("miss-continuing","icon")[0],missFeedback:feedback("miss-continuing"),missShadows:byTarget("miss-continuing","shadow").length,boxingPunch,boxingGuards,wallFrame:{before:wallFrame.timingWindowBeforeMs,after:wallFrame.timingWindowAfterMs,target:wallFrame.targets[0]},wall,wallShadow,appliedDpr:renderer.devicePixelRatio};
      ...
      assert.equal(evidence.missIcon.appearanceColor,"#7c828c"); assert(evidence.missIcon.position.z>0,"miss must continue past the athlete along authoritative timeline motion");assert.equal(evidence.boxingPunch.appearanceColor,"#FF0000","Boxing punch receives the effective song-side color through the private seam");assert.equal(evidence.boxingPunch.whiteCore,true,"Boxing punch keeps its white authored outline");assert.equal(evidence.boxingGuards.length,2);assert.equal(evidence.boxingGuards.every((entry)=>entry.appearanceColor===null),true,"paired guards remain fixed-color"); assert.equal(evidence.missFeedback.apparentHeightCssPx,42); assert.equal(evidence.missFeedback.animation,"shake"); assert.notEqual(evidence.missFeedback.offsetX,0,"Miss must use deterministic nine-cycle shake motion"); assert.equal(evidence.missShadows,1,"continuing miss keeps one gameplay-inert shadow");
