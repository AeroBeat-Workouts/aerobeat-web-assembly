...
try {
  ...
  const evidence = await game.evaluate(async (element, fixture) => {
    ...
    const loaded = await element.graph.authoring.loadPackage(result.defaultPackage.handle);
    return { collectionId: result.collection.collectionId, resultSourceHash: loaded.package.source.sourceVersionHash, packageCount: packages.length, persisted: packages.some((entry) => entry.packageId === result.defaultPackage.packageId && entry.key === result.defaultPackage.handle.key), contentState: content.state, selectedPackageId: content.packageId, selectedVariantId: content.selectedVariant?.variantId, variantCount: content.variants.length, lineageHash: content.lineage?.sourceVersionHash, cameraRequests: globalThis.__liveV4CameraRequests, rawExposure, snapshotText: JSON.stringify(snapshot) };
  }, { mapId, versionHash });
  ...
  if (importedCollectionId) { try { await page.locator("aero-game").evaluate((element, collectionId) => element.deleteLibraryCollection(collectionId), importedCollectionId); } catch { /* ephemeral browser context is the final cleanup boundary */ } }
