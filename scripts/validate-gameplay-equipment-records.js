...
const flowQuaternionTargets=Object.freeze({left:Object.freeze({orientation:Object.freeze({x:0,y:0,z:0,w:1})}),right:Object.freeze({orientation:Object.freeze({x:0,y:0,z:0,w:1})})});

{
  const poses=gameplayEquipmentRecords(false,session(),input(),"flow",null,flowQuaternionTargets,equipmentConfigDefaults,identity);
...

{
  ...
  const zones={left:{orientation:{x:0,y:0,z:Math.sin(Math.PI/12),w:Math.cos(Math.PI/12)}},right:{orientation:{x:0,y:0,z:Math.SQRT1_2,w:Math.SQRT1_2}}};
  const poses=gameplayEquipmentRecords(false,session("playing","visual_test"),input(),"flow",null,zones,config,identity);
...

{
  ...
  const poses=gameplayEquipmentRecords(false,session(),input(),"boxing",{left:q,right:q},null,equipmentConfigDefaults,identity);
...
for(const [label,menu,state,snapshot] of [
  ...
  assert.equal(gameplayEquipmentRecords(menu,session(state,label==="paused play"?"play":"visual_test"),snapshot,"flow",null,flowQuaternionTargets,equipmentConfigDefaults,configIdentity).length,0,label);
}
assert.equal(gameplayEquipmentRecords(false,session("paused_manual","visual_test"),input(),"flow",null,flowQuaternionTargets,equipmentConfigDefaults,identity).length,2,"paused Visual Test still renders frozen shared poses");
