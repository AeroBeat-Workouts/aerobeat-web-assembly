...
try {
  ...
  assert.equal(toggledOn.visible,true); assert.equal(onRecords.length,2,"toggle-on emits exactly two deterministic preview records"); assert.deepEqual(onRecords.map(({role,x,y})=>({role,x,y})),[{role:"left_wrist",x:0,y:.5},{role:"right_wrist",x:1,y:.5}]); assert.equal(/equipmentConfig|equipment_config|rotationZDeg|testEquipmentVisible/u.test(toggledOn.snapshot),false,"preview/config remain private"); assert.equal(toggledOn.input.anchors.length,0,"preview never mutates input service");
