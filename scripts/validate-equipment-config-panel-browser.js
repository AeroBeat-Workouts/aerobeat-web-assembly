...
try {
  ...
  await game.evaluate((element) => {
    ...
    element.serviceGraphFactory = (options) => {
      ...
      const audioState = { playing:false, positionSeconds:0 };
      const audio = {
  ...
  const capturePointerMove=async(hand,xRatio,yRatio,outside=false)=>game.evaluate((element,{hand,xRatio,yRatio,outside})=>{element.setTestEquipmentMouseHand(hand);const canvas=element.canvasElement(),rect=canvas.getBoundingClientRect(),renderer=element.graph.renderer,original=renderer.renderGameplayFrameWithCursorsAndEquipment;let captured=null;renderer.renderGameplayFrameWithCursorsAndEquipment=(...args)=>{captured=structuredClone(args[3]);return original.apply(renderer,args);};try{canvas.dispatchEvent(new PointerEvent("pointermove",{bubbles:true,clientX:outside?rect.left-50:rect.left+rect.width*xRatio,clientY:outside?rect.bottom+50:rect.top+rect.height*yRatio,pointerType:"mouse",pointerId:91}));}finally{renderer.renderGameplayFrameWithCursorsAndEquipment=original;}return{captured,pointer:structuredClone(element.testEquipmentPointerPosition)};},{hand,xRatio,yRatio,outside});
  const leftClamped=await capturePointerMove("left",0,0,true); assert.deepEqual(leftClamped.pointer,{x:0,y:1}); assert.deepEqual(leftClamped.captured.map(({role,x,y})=>({role,x,y})),[{role:"left_wrist",x:0,y:1},{role:"right_wrist",x:1,y:.5}],"Left mouse mode clamps and moves only left equipment");
  const leftLatched=await capturePointerMove("left",.25,.75,false); await scale.fill("1.75");
  const latchedAfterEdit=await game.evaluate((element)=>{let captured=null;const renderer=element.graph.renderer,original=renderer.renderGameplayFrameWithCursorsAndEquipment;renderer.renderGameplayFrameWithCursorsAndEquipment=(...args)=>{captured=structuredClone(args[3]);return original.apply(renderer,args);};try{element.renderGameplay();}finally{renderer.renderGameplayFrameWithCursorsAndEquipment=original;}return captured;});
  assert.deepEqual(leftLatched.pointer,{x:.25,y:.75}); assert.deepEqual(latchedAfterEdit.map(({role,x,y})=>({role,x,y})),[{role:"left_wrist",x:.25,y:.75},{role:"right_wrist",x:1,y:.5}],"last valid pointer remains latched while editing controls"); assert.equal(latchedAfterEdit[0].scale,1.75,"latched pointer and live transform render together while paused");
  const rightMoved=await capturePointerMove("right",.8,.2,false); assert.deepEqual(rightMoved.captured.map(({role,x,y})=>({role,x,y})),[{role:"left_wrist",x:0,y:.5},{role:"right_wrist",x:.8,y:.2}],"Right mouse mode moves only right equipment");
  ...
  await game.locator("[data-test-automatic-feedback-toggle='true']").uncheck(); await capturePointerMove("right",.6,.4,false);
  const resumed=await game.evaluate(async(element)=>{element.enqueueVisualTestPlay();await element.transportIntentTail;return{state:element.graph.gameplay.getSnapshot().session.state,visible:element.testEquipmentVisible,automatic:element.testAutomaticFeedbackEnabled,hand:element.testEquipmentMouseHand,pointer:structuredClone(element.testEquipmentPointerPosition)};}); assert.deepEqual(resumed,{state:"playing",visible:true,automatic:false,hand:"right",pointer:{x:.6,y:.4}},"transport resume preserves every private authoring state");
  ...
  const lifecycle=await game.evaluate(async(element)=>{const events=[];element.addEventListener("aero-game-event",(event)=>events.push(structuredClone(event.detail)));element.setMenuOpen(false);await element.menuPauseTail;element.setTestEquipmentVisible(true);element.setTestAutomaticFeedbackEnabled(false);element.setTestEquipmentMouseHand("left");const canvas=element.canvasElement(),rect=canvas.getBoundingClientRect();canvas.dispatchEvent(new PointerEvent("pointermove",{bubbles:true,clientX:rect.left+rect.width*.4,clientY:rect.top+rect.height*.6,pointerType:"mouse",pointerId:93}));const before={automatic:element.testAutomaticFeedbackEnabled,hand:element.testEquipmentMouseHand,pointer:structuredClone(element.testEquipmentPointerPosition)};await element.startSession("visual_test",{requireDownloaded:false});const fresh={automatic:element.testAutomaticFeedbackEnabled,hand:element.testEquipmentMouseHand,pointer:element.testEquipmentPointerPosition,visible:element.testEquipmentVisible};element.setMenuOpen(false);await element.menuPauseTail;element.setTestAutomaticFeedbackEnabled(false);element.setTestEquipmentMouseHand("right");await element.stop();const stopped={automatic:element.testAutomaticFeedbackEnabled,hand:element.testEquipmentMouseHand,pointer:element.testEquipmentPointerPosition};const serialized=JSON.stringify({snapshot:element.getSnapshot(),events,input:element.graph.input.getSnapshot(),storage:Object.fromEntries(Object.entries(localStorage))});return{before,fresh,stopped,leak:/testAutomaticFeedback|testEquipmentMouseHand|testEquipmentPointerPosition|automatic GREAT|mouse-controlled hand/iu.test(serialized),judgements:element.graph.gameplay.getSnapshot().judgements.length,scores:element.graph.gameplay.getSnapshot().scorePartitions.length};});
  assert.deepEqual(lifecycle.before,{automatic:false,hand:"left",pointer:{x:.4,y:.6}}); assert.deepEqual(lifecycle.fresh,{automatic:true,hand:"off",pointer:null,visible:false},"accepted fresh Test resets all private authoring defaults"); assert.deepEqual(lifecycle.stopped,{automatic:true,hand:"off",pointer:null},"stop restores private authoring defaults"); assert.equal(lifecycle.leak,false,"snapshot/event/input/storage surfaces remain free of private authoring state"); assert.equal(lifecycle.judgements,0); assert.equal(lifecycle.scores,0);
