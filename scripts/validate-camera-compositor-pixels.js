...
async function validateMovingCameraComposition() {
  ...
  try {
    ...
    const aero = await game.evaluate((element) => { const retained = element.graph.video.getRetainedCameraStream(); element.shadowRoot.querySelector("input[value='aero'][data-action='environment-select']").click(); const video = element.shadowRoot.querySelector("video"); return { mode: element.environmentMode, preview: video.dataset.previewVisible, background: element.graph.renderer.background.colors[0], sameStream: retained === element.graph.video.getRetainedCameraStream() }; });
    assert(aero.mode === "aero" && aero.preview === "false" && aero.background === "#071426" && aero.sameStream, `Aero must restore opaque projection without camera reacquire: ${JSON.stringify(aero)}`);
  } finally { await page.close(); }
}
