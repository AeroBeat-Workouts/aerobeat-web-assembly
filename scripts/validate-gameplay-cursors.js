...
async function runContext(context) {
  ...
  assert(result.dark.call.equipment.every((record)=>JSON.stringify(Object.keys(record).sort())===JSON.stringify(["direction","mode","role","x","y"])&&record.mode==="flow"&&Number.isFinite(record.direction.x)&&Number.isFinite(record.direction.y)),`${label} flow records must be exact bounded records with a finite direction: ${JSON.stringify(result.dark.call.equipment)}`);
