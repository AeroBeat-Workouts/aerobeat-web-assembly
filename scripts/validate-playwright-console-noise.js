...
try {
  ...
  const actual = await page.locator("aero-game").evaluate(async (game, fixture) => {
    ...
    // ruleset that is absent from this package raises the bounded generic
    // unavailable error instead of a variant-specific failure.
    const before = game.getSnapshot(); const target = before.services.content.variants.find((entry) => entry.rulesetId === "boxing_collider_v1");
