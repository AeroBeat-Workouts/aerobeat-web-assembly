...
const parentServer = createHttpServer((_request, response) => {
  ...
  response.end(`<!doctype html><iframe id="game" style="width:640px;height:480px" allow="camera; fullscreen; autoplay; xr-spatial-tracking" src="${childUrl}"></iframe><script>window.messages=[];addEventListener('message',event=>messages.push({origin:event.origin,data:event.data}));</script>`);
});
...
try {
  ...
  if (direct.aliasCount !== 0 || direct.snapshot.lifecycle !== "connected" || !direct.bodyUnchanged || !direct.navigationUnchanged) throw new Error("Public root ownership boundaries failed");
  const startCommands=await page.locator("aero-game").evaluate(async(game)=>{const calls=[];const originalStart=game.start;const originalStartSession=game.startSession;game.start=()=>{calls.push("legacy-play");return null};game.startSession=(purpose,options)=>{calls.push(`${purpose}:${options.requireDownloaded===true}`);return null};game.executeCommand({schema:"aerobeat/game_command",version:1,commandId:"legacy",type:"start",payload:null});game.executeCommand({schema:"aerobeat/game_command",version:1,commandId:"play",type:"start",payload:{schema:"aerobeat/gameplay_session_start",version:1,purpose:"play"}});game.executeCommand({schema:"aerobeat/game_command",version:1,commandId:"test",type:"start",payload:{schema:"aerobeat/gameplay_session_start",version:1,purpose:"visual_test"}});game.start=originalStart;game.startSession=originalStartSession;return calls});
  if(JSON.stringify(startCommands)!==JSON.stringify(["legacy-play","play:false","visual_test:false"])) throw new Error(`Legacy/exact start routing failed: ${JSON.stringify(startCommands)}`);
  ...
  const authoring = await page.locator("aero-game").evaluate((game) => {
    const root=game.shadowRoot,panel=root.querySelector("[data-role='debug-camera-controls']"),collapse=root.querySelector("[data-action='debug-controls-collapse']"),body=root.querySelector("[data-role='visual-test-authoring-body']"),select=root.querySelector("[data-action='environment-asset-select']");game.menuOpen=false;root.querySelector("[data-role='drawer']").hidden=true;root.querySelector("[data-role='menu-backdrop']").hidden=true;panel.hidden=false;panel.setAttribute("aria-hidden","false");
    const labels=[...select.options].map((option)=>option.textContent);const ids=[...select.options].map((option)=>option.value);const fields=[...root.querySelectorAll("[data-environment-field]")].map((input)=>({name:input.dataset.environmentField,min:input.min,max:input.max,step:input.step}));const rect=collapse.getBoundingClientRect();return{labels,ids,selected:select.value,fields,width:rect.width,height:rect.height,controls:collapse.getAttribute("aria-controls"),expanded:collapse.getAttribute("aria-expanded"),bodyHidden:body.hidden,privateSnapshot:!/environment-config|photosphere|selectedEnvironment/u.test(JSON.stringify(game.getSnapshot()))};
  });
  if(authoring.ids.length!==8||new Set(authoring.ids).size!==8||authoring.selected!=="alpine-river-valley-photosphere"||authoring.labels.filter((label)=>label.includes("comparison with source artifacts")).length!==2||authoring.fields.length!==7||authoring.width<44||authoring.height<44||authoring.controls!=="visual-test-authoring-body"||authoring.expanded!=="true"||authoring.bodyHidden||!authoring.privateSnapshot)throw new Error(`Environment authoring UI contract failed: ${JSON.stringify(authoring)}`);
  await page.locator("aero-game").locator("[data-action='debug-controls-collapse']").click();
  const collapsed=await page.locator("aero-game").evaluate((game)=>({collapsed:game.environmentControlsCollapsed,expanded:game.shadowRoot.querySelector("[data-action='debug-controls-collapse']").getAttribute("aria-expanded"),label:game.shadowRoot.querySelector("[data-action='debug-controls-collapse']").getAttribute("aria-label"),bodyHidden:game.shadowRoot.querySelector("[data-role='visual-test-authoring-body']").hidden,pointers:game.debugCameraControlPointers.size}));
  if(!collapsed.collapsed||collapsed.expanded!=="false"||collapsed.label!=="Expand Visual Test controls"||!collapsed.bodyHidden||collapsed.pointers!==0)throw new Error(`Environment authoring collapse contract failed: ${JSON.stringify(collapsed)}`);
  await page.locator("aero-game").locator("[data-action='debug-controls-collapse']").click();
  ...
  const reconnect = await page.locator("aero-game").evaluate(async (game) => {
    const firstGeneration = game.getSnapshot().generation; const canvas = game.shadowRoot.querySelector("canvas[data-role='renderer']"); const video = game.shadowRoot.querySelector("video[data-role='media']"); const parent = game.parentElement;const oldGraph=game.graph;const events=[];game.addEventListener("aero-game-event",(event)=>events.push(event.detail.type));game.destroy();await new Promise((resolve)=>setTimeout(resolve,40));const eventsAfterDestroy=[...events];
    game.remove(); parent.append(game); await new Promise((resolve) => setTimeout(resolve, 40));
    return { firstGeneration, secondGeneration: game.getSnapshot().generation, stable: canvas === game.shadowRoot.querySelector("canvas[data-role='renderer']") && video === game.shadowRoot.querySelector("video[data-role='media']"), lifecycle: game.getSnapshot().lifecycle, freshGraph:game.graph!==oldGraph,eventsAfterDestroy,allEvents:[...events],freshProfiles:game.getSnapshot().services.profiles.active.visual.profileId==="aero.visual.default"&&game.getSnapshot().services.profiles.active.scoring.profileId==="aero.scoring.locked"&&game.getSnapshot().services.profiles.active.converter.profileId==="aero.converter.canonical" };
  });
  if (!reconnect.stable || reconnect.secondGeneration <= reconnect.firstGeneration || reconnect.lifecycle !== "connected" || !reconnect.freshGraph || !reconnect.freshProfiles || JSON.stringify(reconnect.eventsAfterDestroy)!==JSON.stringify(["destroyed"]) || reconnect.allEvents.filter((type)=>type==="destroyed").length!==1) throw new Error("Reconnect/stable surfaces failed");

  const integration = await page.evaluate(async () => {
    document.querySelector("aero-game")?.remove(); const log = []; let resolveLateBrowse; const lateBrowseGate=new Promise((resolve)=>{resolveLateBrowse=resolve}); let providerMap=null; const variants = ["flow", "flow-colliders", "semantic-row", "spatial-row", "semantic-cut", "spatial-cut"].map((variantId) => ({ variantId, chartId: `chart-${variantId}`, mode: variantId.startsWith("flow") ? "flow" : "boxing", rulesetId: variantId === "flow" ? "flow_grid_v2" : variantId==="flow-colliders"?"flow_colliders_v1":variantId.includes("semantic") ? "boxing_semantic_track_v1" : "boxing_spatial_grid_v1", recipeId: variantId.startsWith("flow") ? null : variantId.includes("row") ? "row_family_balanced_height_v1" : "cut_family_source_height_v1", modifierIds: [], ranked: variantId!=="flow-colliders", localOnly:variantId==="flow-colliders", mapHash: { schema: "aerobeat/content_hash", version: 1, algorithm: "sha256", value: "a".repeat(64) }, scoreIdentityHash: { schema: "aerobeat/content_hash", version: 1, algorithm: "sha256", value: "b".repeat(64) } }));
    function fakeProfiles(){const definitions=[profile("aero.visual.default","live_visual","1",{motionIntensity:1,roleScale:1}),profile("aero.scoring.locked","between_run_ruleset","2",{comboBonusPerHit:0,hitPoints:1,missPenalty:0}),profile("aero.converter.canonical","converter_regeneration","3",{guardRelocationRadius:1,reachAllowanceSubcells:0})],active={visual:state(definitions[0]),scoring:state(definitions[1]),converter:state(definitions[2])};const snapshot={schema:"aerobeat/prototype_profile_registry_snapshot",version:1,generation:0,destroyed:false,bundleVersion:"1.0.0",profiles:definitions,active,appliedConverterHash:definitions[2].contentHash,pendingConverterHash:null,regenerationRequired:false,experimental:true};return{subscribe(fn){fn(snapshot);return()=>{}},getSnapshot:()=>snapshot,getActive(profileClass){return profileClass==="live_visual"?active.visual:profileClass==="between_run_ruleset"?active.scoring:active.converter},destroy(){}};function profile(profileId,profileClass,digit,settings){return{schema:"aerobeat/prototype_profile",version:1,profileId,profileVersion:"1.0.0",class:profileClass,label:profileId,experimental:true,settings,contentHash:digit.repeat(64)}}function state(value){return{profile:value,identity:{schema:"aerobeat/prototype_tuning_identity",version:1,profileId:value.profileId,profileVersion:value.profileVersion,contentHash:value.contentHash,class:value.class,regenerationRequired:false},settings:value.settings,regenerationRequired:false,appliedContentHash:value.contentHash}}}
    function factory({ instanceId }) {
      let selected = variants[0], state = instanceId==="autoplay-reject"?"playing":"calibrating", destroyed = false, lease = null, playback = "paused", mix = Object.freeze({ musicVolume:0.5, sfxVolume:0.5 });
      ...
      return {
        instanceId, vendor: { snapshot: () => ({ phase: "idle" }), searchMaps: async () => { if(instanceId==="late-reconnect")await lateBrowseGate;return{maps:instanceId==="lease-second"&&providerMap?[providerMap]:[]} }, listLatestMaps: async (options) => { if(Object.prototype.hasOwnProperty.call(options,"latest"))throw new Error("synthetic latest leaked");return{maps:[]} } },
        ...
        audio: { getMixSnapshot:()=>mix, setMix(value){mix=Object.freeze({musicVolume:value.musicVolume,sfxVolume:value.sfxVolume});return mix}, async activateLease() {}, async pauseForLease() {}, async releaseLease() {}, async play() { if(instanceId==="autoplay-reject")throw new Error("autoplay rejected") }, async pause() {}, async stop() {}, async setDocumentHidden(value) { log.push(`${instanceId}:audio-hidden:${value}`); }, async destroy() { destroyed = true; }, getStatus: () => ({ state: "ready" }), getClockSnapshot: () => ({ positionSeconds: 0, playing: false, durationSeconds: undefined }) },
        ...
        renderer: { attach() {}, resize() {}, clear() {}, renderGameplayFrame() {}, setTheme() {}, setEnvironmentTransform() {}, setEnvironmentAsset() {}, setEnvironmentVisible() {}, setBackgroundProjection() {}, setDebugCameraEnabled() {}, resetDebugCamera() {}, importTuning() {}, getCapabilities: () => ({ playcanvas: true }), describe: () => ({ serviceId:"aero.renderer.playcanvas", state: destroyed ? "destroyed" : "ready" }), destroy() { destroyed = true; } }
      };
    }
    ...
    const childMap={mapId:"abc",mapName:"Child Local",songName:"Child Local",songAuthorName:"Author",levelAuthorName:"Mapper",versions:[{hash:"c".repeat(40),key:"v1",difficulties:[{characteristic:"Standard",difficulty:"Expert"}]}]};providerMap=childMap;await second.browseBeatSaver({text:"child"});let importedMap=null;second.importBeatSaver=async(map)=>{importedMap=map;return{ok:true}};await second.executeCommand({schema:"aerobeat/game_command",version:1,commandId:"iframe-import",type:"import_beatsaver",payload:{mapId:"ABC",versionHash:"c".repeat(40),difficultyId:"Expert"}});await second.browseLatestBeatSaver({pageSize:1});
    const late=document.createElement("aero-game");late.setAttribute("instance-id","late-reconnect");late.serviceGraphFactory=factory;const lateEvents=[];late.addEventListener("aero-game-event",(event)=>lateEvents.push(event.detail.type));host.append(late);const oldLateGraph=late.graph;const pendingBrowse=late.browseBeatSaver({text:"late"});late.destroy();late.remove();host.append(late);resolveLateBrowse();await pendingBrowse;await new Promise((resolve)=>setTimeout(resolve,30));const staleReconnectSafe=late.graph!==oldLateGraph&&!lateEvents.includes("beatsaver_results")&&late.beatSaverView.state==="idle";late.remove();const autoplay=document.createElement("aero-game");autoplay.setAttribute("instance-id","autoplay-reject");autoplay.serviceGraphFactory=factory;host.append(autoplay);autoplay.syncAudioForGameplay();await new Promise((resolve)=>setTimeout(resolve,20));const autoplayRejectionVisible=autoplay.lastError?.message==="autoplay rejected";autoplay.remove();
    const result = { owner: second.getSnapshot().lease.ownerInstanceId, log: [...log], variantCount: second.getSnapshot().services.content.variants.length, selected: second.getSnapshot().services.content.selectedVariant.variantId, duplicateLifecycle: duplicate.getSnapshot().lifecycle, childLocalImport: importedMap===childMap,staleReconnectSafe,autoplayRejectionVisible };
...
assert(faviconConsoleErrors.length === 0, `kl80: zero console errors may mention favicon: ${JSON.stringify(faviconConsoleErrors)}`);
  ...
  await parent.evaluate(({ childOrigin }) => {
    ...
    frame.contentWindow.postMessage(command("configure", "configure", {}), childOrigin);frame.contentWindow.postMessage(command("configure", "configure", {}), childOrigin);
    const exact=command("exact-64k","configure",{padding:[...Array.from({length:7},()=>"x".repeat(8192)),""]});const exactBytes=new TextEncoder().encode(JSON.stringify(exact)).byteLength;exact.payload.command.payload.padding[7]="x".repeat(64*1024-exactBytes);if(new TextEncoder().encode(JSON.stringify(exact)).byteLength!==64*1024)throw new Error("Exact iframe boundary fixture failed");frame.contentWindow.postMessage(exact,childOrigin);const over=structuredClone(exact);over.payload.command.payload.padding[7]+="x";frame.contentWindow.postMessage(over,childOrigin);
    let tooDeep={};for(let index=0;index<14;index+=1)tooDeep={nested:tooDeep};
    for (const [id,payload] of [["raw-case",{ZiP_ByTeS:[1]}],["raw-media",{MEDIA_STREAM:{}}],["raw-screen",{screen_shot:"x"}],["oversize",{value:"x".repeat(70*1024)}],["long-string",{value:"x".repeat(8193)}],["too-many",{value:Array.from({length:257},()=>"x")}],["too-deep",tooDeep]]) frame.contentWindow.postMessage(command(id,"configure",payload),childOrigin);
    ...
    const attacker=document.createElement("iframe");const hostile=command("wrong-source","configure",{});attacker.srcdoc=`<script>parent.frames[0].postMessage(JSON.parse(${JSON.stringify(JSON.stringify(hostile))}),${JSON.stringify(childOrigin)})<\/script>`;document.body.append(attacker);
  }, { childOrigin });
  const childFrame=parent.frames().find((frame)=>frame.url().startsWith(childOrigin));if(!childFrame)throw new Error("Cross-origin child frame unavailable");await childFrame.evaluate(()=>{const data={schema:"aerobeat/iframe_message",version:1,kind:"command",messageId:"wrong-origin",instanceId:"aero-game-1",payload:{command:{schema:"aerobeat/game_command",version:1,commandId:"wrong-origin",type:"configure",payload:{}}}};dispatchEvent(new MessageEvent("message",{source:parent,origin:"https://wrong.invalid",data}))});
  ...
  if (events.filter((entry) => entry.payload?.event?.type === "capabilities_changed").length !== 2 || events.filter((entry) => entry.payload?.event?.type === "ready").length !== 1) throw new Error("Unsafe iframe messages were not rejected exactly");
  const readyProfiles=events.find((entry)=>entry.payload?.event?.type==="ready")?.payload?.event?.payload?.snapshot?.services?.profiles;if(readyProfiles?.active?.visual?.profileId!=="aero.visual.default"||readyProfiles?.active?.scoring?.profileId!=="aero.scoring.locked"||readyProfiles?.active?.converter?.profileId!=="aero.converter.canonical"||readyProfiles.profileCount!==6)throw new Error(`Iframe profile identity telemetry parity failed: ${JSON.stringify(readyProfiles)}`);
  if (JSON.stringify(beforeDestroy).match(/zip.?bytes|media.?stream|screen.?shot|audio.?bytes|video.?frame|pixels|prototype_profile_bundle|guardRelocationRadius|hitPoints|motionIntensity/iu)) throw new Error("Raw payload, profile bundle or profile settings crossed iframe bridge");
...
async function makePackage() {
  {const fixture=JSON.parse(await readFile(new URL("../../aerobeat-web-content/fixtures/flow-colliders-3c9d-successor-v1.json",import.meta.url),"utf8")),audioBytes=makeSilentWav(),audioHash=hashBytes(audioBytes),songPackage={...fixture.package,song:{...fixture.package.song,audio:{filePath:"song.wav",contentHash:`sha256:${audioHash}`}}};return {songPackage,audioBytes:[...audioBytes]};}
  ...
  const flowBeats=[{start:1,type:"note",hand:"left",placement:4,direction:1}],flowPalette=null,flowContentHash=`sha256:${hashJson({beats:flowBeats,rulesetId:"flow_grid_v2",notePalette:flowPalette})}`;
  charts.push({schemaId:"aerobeat.chart.flow.v4",schemaVersion:4,recordVersion:2,rulesetId:"flow_grid_v2",chartId:"chart-flow",chartName:"Flow",mode:"flow",difficulty:"Expert",notePalette:flowPalette,contentHash:flowContentHash,beats:flowBeats});
  const spawnTiming={schema:"aerobeat/beatsaber_spawn_timing",version:1,algorithm:"beatsaber_core_hjd_v1",bpm:120,noteJumpMovementSpeed:10,noteJumpStartBeatOffset:0,maxHalfJumpDistance:17.999,startHalfJumpDurationBeats:4,minimumHalfJumpDurationBeats:.25,halfJumpDurationBeats:2,reactionTimeMs:1000,jumpDistanceMeters:20};
  const songPackage = {schemaId:"aerobeat.song-package.v5",schemaVersion:5,packageVersion:"5.0.0",packageId:"assembly-public-package",songId:"assembly-public-song",songName:"Assembly Public Integration",notePalette:null,source:{provider:"local",sourceId:"assembly-public",sourceVersionHash:"public-version",difficulty:"Expert",sourceInfoFormat:"v2",sourceInfoVersion:"2.1.0",sourceInfoHash:sourceHash,sourceDifficultyPath:"Expert.dat",sourceBeatmapFormat:"v3",sourceBeatmapVersion:"3.3.0",sourceDifficultyHash:sourceHash,sourceHash,obstacleContract:"normalized_obstacle_v2",spawnTiming},song:{schemaId:"aerobeat.song.v1",schemaVersion:1,recordVersion:1,songId:"assembly-public-song",songName:"Assembly Public Integration",durationSec:1,audio:{filePath:"song.wav",contentHash:`sha256:${audioHash}`},timing:{anchorMs:0,tempoSegments:[{startBeat:0,bpm:120}],stopSegments:[],timeSignatureSegments:[{startBeat:0,numerator:4,denominator:4}]}},charts,sets:charts.map((chart,index)=>({schemaId:"aerobeat.set.v1",schemaVersion:1,recordVersion:1,setId:`set-${index}`,setName:chart.chartName,songId:"assembly-public-song",chartId:chart.chartId})),recipeDefinitions:[],rulesetDefinitions:[],conversionTrace:{notePalette:flowPalette,boxing:charts.filter(chart=>chart.mode==="boxing").map(()=>({spawnTiming})),flow:[{notePalette:flowPalette,contentHash:flowContentHash,spawnTiming}],spawnTiming},presentationSuggestion:null};
