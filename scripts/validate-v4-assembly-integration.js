...
try {
  ...
  // 0.0.54 W1-A rebaseline: a fresh v4 import authors exactly two variants —
  // the sole flow_colliders_v1 and the sole no-recipe boxing_collider_v1.
  assert.equal(v4Evidence.cameraRequests, 0); assert.equal(v4Evidence.libraryCount, 1); assert.equal(v4Evidence.selectedSourceVersionHash, goldenV4Hash); assert.equal(v4Evidence.selectedDifficulty, "ExpertPlus"); assert.equal(v4Evidence.variantCount,2);assert.equal(v4Evidence.selectedRulesetId,"flow_colliders_v1");assert.deepEqual(new Set(v4Evidence.variantIdentities),new Set(["flow_colliders_v1|none","boxing_collider_v1|none"]));assert.deepEqual(v4Evidence.collider.map(({ranked,localOnly})=>({ranked,localOnly})),[{ranked:true,localOnly:false}]); assert.equal(v4Evidence.checkedLibraryRadios, 1); assert.equal(v4Evidence.musicReady, true); assert.equal(v4Evidence.rawExposure.length, 0);
