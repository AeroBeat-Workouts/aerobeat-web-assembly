...
try {
  ...
  assert.equal(v4Evidence.cameraRequests, 0); assert.equal(v4Evidence.libraryCount, 1); assert.equal(v4Evidence.selectedSourceVersionHash, goldenV4Hash); assert.equal(v4Evidence.selectedDifficulty, "ExpertPlus"); assert.equal(v4Evidence.variantCount,6);assert.equal(v4Evidence.selectedRulesetId,"flow_grid_v2");assert.deepEqual(new Set(v4Evidence.variantIdentities),new Set(["flow_grid_v2|none","flow_colliders_v1|none","boxing_semantic_track_v1|row_family_balanced_height_v1","boxing_semantic_track_v1|cut_family_source_height_v1","boxing_spatial_grid_v1|row_family_balanced_height_v1","boxing_spatial_grid_v1|cut_family_source_height_v1"]));assert.deepEqual(v4Evidence.collider.map(({ranked,localOnly})=>({ranked,localOnly})),[{ranked:false,localOnly:true}]);assert.equal(v4Evidence.collider[0].chartId,v4Evidence.grid[0].chartId);assert.equal(v4Evidence.collider[0].mapHash,v4Evidence.grid[0].mapHash); assert.equal(v4Evidence.checkedLibraryRadios, 1); assert.equal(v4Evidence.musicReady, true); assert.equal(v4Evidence.rawExposure.length, 0);
...
async function integrationSnapshot(game) { return game.evaluate(async (element) => {
  ...
  return { cameraRequests: globalThis.__v4CameraRequests, libraryCount: packages.length, selectedSourceVersionHash: content.lineage?.sourceVersionHash ?? null, selectedDifficulty: content.lineage?.difficulty ?? null, selectedPackageId: content.packageId, selectedVariantId: content.selectedVariant?.variantId,selectedRulesetId:content.selectedVariant?.rulesetId,variantIdentities:content.variants.map((variant)=>`${variant.rulesetId}|${variant.recipeId??"none"}`),collider:content.variants.filter((variant)=>variant.rulesetId==="flow_colliders_v1").map((variant)=>({ranked:variant.ranked,localOnly:variant.localOnly,chartId:variant.chartId,mapHash:variant.mapHash.value})),grid:content.variants.filter((variant)=>variant.rulesetId==="flow_grid_v2").map((variant)=>({chartId:variant.chartId,mapHash:variant.mapHash.value})), variantCount: content.variants.length, checkedLibraryRadios: library.shadowRoot.querySelectorAll("input[type='radio']:checked").length, musicReady: Boolean(content.packageId && content.selectedVariant), rawExposure, publicSnapshot };
}); }
