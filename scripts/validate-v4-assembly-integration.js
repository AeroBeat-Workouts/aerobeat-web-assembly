...
try {
  ...
  assert.equal(v4Evidence.cameraRequests, 0); assert.equal(v4Evidence.libraryCount, 1); assert.equal(v4Evidence.selectedSourceVersionHash, goldenV4Hash); assert.equal(v4Evidence.selectedDifficulty, "ExpertPlus"); assert.equal(v4Evidence.variantCount,5);assert.equal(v4Evidence.selectedRulesetId,"flow_colliders_v1");assert.deepEqual(new Set(v4Evidence.variantIdentities),new Set(["flow_colliders_v1|none","boxing_semantic_track_v1|row_family_balanced_height_v1","boxing_semantic_track_v1|cut_family_source_height_v1","boxing_spatial_grid_v1|row_family_balanced_height_v1","boxing_spatial_grid_v1|cut_family_source_height_v1"]));assert.deepEqual(v4Evidence.collider.map(({ranked,localOnly})=>({ranked,localOnly})),[{ranked:true,localOnly:false}]); assert.equal(v4Evidence.checkedLibraryRadios, 1); assert.equal(v4Evidence.musicReady, true); assert.equal(v4Evidence.rawExposure.length, 0);
