...
const missStart=projectSessionTargets(events,playMiss,1181,straightIndex)[0]; assert.equal(missStart.judgement,"miss"); assert.equal(missStart.feedbackProgress,0,"miss feedback starts at exact committed timeline");
...
const syntheticSecond=projectSessionTargets(events,testTruth,2181,straightIndex); assert.equal(syntheticSecond.length,1); assert.equal(syntheticSecond[0].id,"flow-2"); assert.equal(syntheticSecond[0].judgement,"miss"); assert.equal(syntheticSecond[0].feedbackProgress,0);
