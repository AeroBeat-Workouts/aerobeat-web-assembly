...
try {
  ...
  await game.evaluate((element) => {
    ...
    element.serviceGraphFactory = (options) => {
      ...
      const state = globalThis.__mobileState = { pose: undefined, audioState: "paused", audioPlayCalls: 0, audioPauseCalls: 0, cvStartCalls: 0, cvStopCalls: 0, videoPauseCalls: 0, retained: null, library: [], packageLoadCalls: [], exportCalls: [], importCalls: [], maps: [], contentSnapshot: idleContent() };
  ...
  assert(taxonomy.oneSurface && taxonomy.surfaceAlpha === 1 && taxonomy.contrast >= 4.5 && taxonomy.closeControls === 1 && taxonomy.menuGlyph === "×", `drawer must use one opaque AA surface and one close control: ${JSON.stringify(taxonomy)}`);
  ...
  assert(selection.versionTag === "SELECT" && selection.versionOptions === 1 && selection.difficultyTag === "SELECT" && selection.difficultyOptions === 1, `only playable versions may remain and Version/Difficulty must be nonempty selects: ${JSON.stringify(selection)}`);
  assert(selection.optionButtonCount === 0 && selection.cancelTag === "BUTTON" && ["Search", "Latest", "Choose local ZIP", "Import selected map"].every((label) => selection.browserActions.includes(label)) && JSON.stringify(selection.libraryActions) === JSON.stringify(["Export", "Delete"]) && JSON.stringify(selection.exportCalls) === JSON.stringify(["package-first"]), `Music must expose one selected action row with wired Export and no Play: ${JSON.stringify(selection)}`);
  const drawerCopy = await composedDrawerText(game);
  const allowedDrawerText = new Set(["Calibrate / Start", "Gameplay", "Flow", "Semantic Row", "Spatial Row", "Semantic Cut", "Spatial Cut", "Visuals", "Default", "Compact", "Music", "Search", "Latest", "Choose local ZIP", "First result", "Second result", "Version", "Difficulty", "Expert", "Import selected map", "Idle · 0%", "First library song · Expert", "Current library song · Expert", "Export", "Delete", "Cancel import", "Info", "Enter fullscreen", "Exit fullscreen"]);
  ...
  assert(calibratingVisual.menu && !calibratingVisual.drawer && calibratingVisual.cue && ["T-pose", "Hold T-pose"].includes(calibratingVisual.cueText) && calibratingVisual.hudPresenters === 0 && calibratingVisual.visibleOverlayCount === 2, `calibration may expose only menu plus one minimal T-pose cue: ${JSON.stringify(calibratingVisual)}`);
