...
try {
  ...
  await game.evaluate((element) => {
    ...
    element.serviceGraphFactory = (options) => {
      ...
      const state = globalThis.__mobileState = { pose: undefined, audioState: "paused", audioPositionSeconds: 0, audioPlayCalls: 0, audioPauseCalls: 0, audioSeekCalls: 0, delayNextPause: false, resolvePause: null, delayNextStop: false, resolveStop: null, cvStartCalls: 0, cvStopCalls: 0, videoPauseCalls: 0, videoPlayCalls: 0, gameplayResumeCalls: 0, inputResetCalls: 0, resetReasons: [], testAdvanceBeforeAudio: 0, freshStartResumeBaseline: 0, freshStartResetBaseline: 0, retained: null, library: [], packageLoadCalls: [], exportCalls: [], importCalls: [], maps: [], contentSnapshot: idleContent() };
      ...
      const gameplay = Object.create(original.gameplay); Object.defineProperties(gameplay,{resume:{enumerable:true,value(...args){state.gameplayResumeCalls+=1;return original.gameplay.resume(...args);}},advance:{enumerable:true,value(...args){const session=original.gameplay.getSnapshot().session;if(session.purpose==="visual_test"&&session.state==="playing"&&state.audioState!=="playing")state.testAdvanceBeforeAudio+=1;return original.gameplay.advance(...args);}}});
  ...
  await game.evaluate((element) => { const state=globalThis.__mobileState;state.freshStartResumeBaseline=state.gameplayResumeCalls;state.freshStartResetBaseline=state.inputResetCalls;element.shadowRoot.querySelector("aero-session-actions").shadowRoot.querySelector("button[data-intent='session-test']").click(); });
  ...
  assert(testStart.session.session.purpose === "visual_test" && !testStart.menuOpen && testStart.actions.downloadedPlayable === true && testStart.actions.activeAction === "test" && testStart.actions.pendingAction === "" && testStart.freshCloseResumeDelta === 0 && testStart.freshCloseResetDelta === 0 && testStart.testAdvanceBeforeAudio === 0 && testStart.cameraRequests === 0 && testStart.lease.resources.length === 1 && testStart.lease.resources[0] === "audio" && testFeedback.audioState === "playing" && testFeedback.gameplay.judgements.length === 0 && testFeedback.gameplay.shadowJudgements.length === 0 && testFeedback.gameplay.scorePartitions.length === 0, `Test must start immediately audio-only with zero gameplay truth: ${JSON.stringify({testStart,testFeedback})}`);
  ...
  const started = await game.evaluate((element) => ({ menuOpen: element.getSnapshot().interaction.menuOpen, calibrationId: element.graph.input.getSnapshot().calibration.calibrationId, gameplay:element.graph.gameplay.getSnapshot(), sessionGeneration:element.sessionGeneration, audioSeekCalls:globalThis.__mobileState.audioSeekCalls, cameraRequests:globalThis.__cameraRequests, actions:element.sessionActionsSnapshot(), freshCloseResumeDelta:globalThis.__mobileState.gameplayResumeCalls-globalThis.__mobileState.freshStartResumeBaseline, freshCloseResetDelta:globalThis.__mobileState.inputResetCalls-globalThis.__mobileState.freshStartResetBaseline, freshCloseResetReasons:globalThis.__mobileState.resetReasons.slice(globalThis.__mobileState.freshStartResetBaseline) }));
  assert(!started.menuOpen && started.calibrationId === null && started.gameplay.session.purpose === "play" && started.gameplay.generation > testStart.session.generation && started.sessionGeneration > testStart.sessionGeneration && started.audioSeekCalls >= 2 && started.cameraRequests === 1 && started.gameplay.session.packageId === "package-current" && started.actions.activeAction === "start" && started.actions.pendingAction === "" && started.freshCloseResumeDelta === 0 && !started.freshCloseResetReasons.includes("menu_closed_recalibration_required"), `Start after Test must generation-restart exact downloaded content at zero into a fresh T-pose gate: ${JSON.stringify(started)}`);
