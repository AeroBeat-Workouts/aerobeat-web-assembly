...
try {
  ...
  await game.evaluate((element) => element.shadowRoot.querySelector("aero-session-actions").shadowRoot.querySelector("button[data-intent='session-test']").click());
  ...
  assert(testStart.session.session.purpose === "visual_test" && !testStart.menuOpen && testStart.actions.downloadedPlayable === true && testStart.actions.activeAction === "test" && testStart.actions.pendingAction === "" && testStart.cameraRequests === 0 && testStart.lease.resources.length === 1 && testStart.lease.resources[0] === "audio" && testFeedback.audioState === "playing" && testFeedback.gameplay.judgements.length === 0 && testFeedback.gameplay.shadowJudgements.length === 0 && testFeedback.gameplay.scorePartitions.length === 0, `Test must start immediately audio-only with zero gameplay truth: ${JSON.stringify({testStart,testFeedback})}`);
  ...
  assert(!started.menuOpen && started.calibrationId === null && started.gameplay.session.purpose === "play" && started.gameplay.generation > testStart.session.generation && started.sessionGeneration > testStart.sessionGeneration && started.audioSeekCalls >= 2 && started.cameraRequests === 1 && started.gameplay.session.packageId === "package-current" && started.actions.activeAction === "start" && started.actions.pendingAction === "", `Start after Test must generation-restart exact downloaded content at zero into a fresh T-pose gate: ${JSON.stringify(started)}`);
